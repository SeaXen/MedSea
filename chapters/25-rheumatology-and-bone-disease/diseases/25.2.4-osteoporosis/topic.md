...
```mermaid
flowchart LR
    A[Bone Remodelling Cycle\nOsteoclast resorption → Osteoblast formation] --> B[Coupling Balance]
    B --> C[Peak Bone Mass\nGenetics 60-80%, Nutrition, Exercise, Hormones]
    C --> D[Age-related Loss\nOsteoclast > Osteoblast]
    D --> E[Postmenopausal Acceleration\nOestrogen ↓ → RANKL/OPG ↑ → Osteoclast ↑]
    D --> F[Secondary Causes\nGlucocorticoids, Hypogonadism, Malabsorption, Endocrine]
    E --> G[Microarchitectural Deterioration\nTrabecular thinning, loss of connectivity, cortical porosity]
    F --> G
    G --> H[Bone Fragility\n↑ Fracture Risk]
```
...
mindmap
  root((Osteoporosis))
    Definition
      T_score_≤-2.5
      Fragility_fracture_risk
    Epidemiology
      Women_1_in_3
      Men_1_in_5
      Hip_fx_20-30%_1yr_mortality
    Pathophysiology
      RANK_RANKL_OPG
      Sclerostin
      PTH_intermittent
      Remodelling_imbalance
    Risk_Factors
      Non_modifiable
      Modifiable
      Secondary_causes
