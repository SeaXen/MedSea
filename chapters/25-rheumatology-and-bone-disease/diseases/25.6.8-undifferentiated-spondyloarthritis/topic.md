...
```mermaid
flowchart TD
    A[Patient with SpA Features] --> B{Meets ASAS for AS?}
    B -->|Yes| C[Diagnose AS]
    B -->|No| D{Meets CASPAR for PsA?}
    D -->|Yes| E[Diagnose PsA]
    D -->|No| F{Post-infectious + ReA Criteria?}
    F -->|Yes| G[Diagnose ReA]
    F -->|No| H{IBD + Enteropathic Criteria?}
    H -->|Yes| I[Diagnose Enteropathic]
    H -->|No| J[**Diagnose USpA**\nFeatures present but no full criteria]
```
...
```mermaid
mindmap
  root((Undifferentiated Spondyloarthritis))
    Definition
      SpA_Features_Present
      Insufficient_Criteria
      Not_AS_PsA_ReA_Enteropathic
    Features
      Inflammatory_Back_Pain
      Peripheral_Arthritis
      Enthesitis_Dactylitis
      HLA_B27_Positive
    Evolution
      10-30%_to_Defined_SpA
      AS_Most_Common
      Predictors_HLA_B27_MRI
      Window_2-5_Years
    Management
      Treat_By_Domain
      Axial_NSAIDs_Biologics
      Peripheral_SSD_Biologics
      Treat_To_Target_ASDAS
    Monitoring
      Annual_MRI_SI_Joints
      Clinical_Review
      Skin_Bowel_History
    Prognosis
      50-70%_Stable
      10-30%_Evolve
      Early_Recognition_Key
```
