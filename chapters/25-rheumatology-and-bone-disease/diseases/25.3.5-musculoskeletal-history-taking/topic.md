...
topic: Musculoskeletal history taking
...
```mermaid
flowchart TD
    A[Musculoskeletal Complaint] --> B{Pain Pattern}
    B -->|Inflammatory: worse rest, >30min stiffness, night pain| C{Distribution}
    B -->|Mechanical: worse activity, <15min stiffness| D[OA / Mechanical / Soft Tissue]
    C -->|Monoarticular| E[Septic / Gout / CPPD / Trauma / AVN]
    C -->|Pauciarticular Asymmetrical| F[SpA Spectrum: PsA / Reactive / Enteropathic / AS]
    C -->|Polyarticular Symmetrical| G[RA / SLE / Viral / Parvovirus]
    C -->|Axial Predominant| H[AS / Axial SpA]
    C -->|Polyarticular Asymmetrical| I[PsA / Sarcoid / Lyme / Viral]
    D --> J{Pattern}
    J -->|Bony swelling DIP/PIP/1st CMC| K[Primary OA]
    J -->|Young + traumatic| L[Secondary OA / Post-traumatic]
    J -->|Diffuse + fatigue + sleep| M[Fibromyalgia / Chronic Widespread Pain]
```
...
```mermaid
mindmap
  root((Musculoskeletal History))
    Pain_Pattern
      Inflammatory
        Worse_rest
        Morning_stiffness_gt_30min
        Night_pain
        Improves_activity
      Mechanical
        Worse_activity
        Morning_stiffness_lt_15min
        Better_rest
        End_of_day_pain
    Distribution
      Monoarticular
      Pauciarticular
      Polyarticular_Symmetrical
      Polyarticular_Asymmetrical
      Axial
    Swelling_Type
      Effusion
      Synovitis
