---
tags: [medicine, rheumatology, davidson, regional-msk, oa, fcps, mrcp]
...
topic: Common Regional Musculoskeletal Problems
...
```mermaid
flowchart TD
    A[Risk Factors\nAge, Obesity, Trauma, Genes] --> B[Cartilage Matrix Degradation\nMMPs, Aggrecanases, ADAMTS]
    B --> C[Chondrocyte Stress\nSenescence, ROS, mtDNA damage]
    C --> D[Subchondral Bone Remodelling\nBone marrow lesions, Sclerosis]
    D --> E[Synovial Inflammation\nLow-grade, MMPs, IL-1, TNF]
    E --> F[Structural Failure\nJoint space narrowing, Osteophytes, Deformity]
    F --> G[Regional Clinical Pattern\nHand / Knee / Hip / Spine]
```
...
```mermaid
mindmap
  root((Common Regional MSK))
    Hand_OA
      Nodal_Heberden_Bouchard
      Erosive_gull_wing
      First_CMC_squaring
      PsA_DIP_dactylitis
    Knee_OA
    ...
    Foot
      ...
      Hallux_rigidus_valgus
      Gout_podagra
    Investigations
      Xray_Kellgren_Lawrence
      MRI_for_red_flags
      Joint_aspiration
    Management
      Education_exercise_weight
      Topical_NSAID_first
      Oral_NSAID_PPI
      IA_steroid
      Surgery_arthroplasty
```
