...
```mermaid
flowchart LR
    A[GALS Screen] --> B[Upper Limb]
    A --> C[Lower Limb]
    A --> D[Spine]
    B --> B1[Hands on head]
    B --> B2[Hands behind neck]
    B --> B3[Touch opposite shoulder]
    B --> B4[Make fists / turn palms]
    B --> B5[Precision / power grip]
    C --> C1[Walk / Turn / Heel-toe]
    C --> C2[Stand from chair unaided]
    C --> C3[Lie flat - hip/knee flexion]
    C --> C4[Internal/external hip rotation]
    C --> C5[Patellar tap / ballotement]
    D --> D1[Schober's test]
    D --> D2[Lateral flexion]
    D --> D3[Cervical rotation]
```
...
```mermaid
flowchart TD
    A[Joint Pain] --> B{Active ROM}
    B -->|Limited| C{Passive ROM}
    B -->|Full| D[Muscle/Tendon/Nerve]
    C -->|Limited| E[Joint Pathology: OA, RA, septic, crystal]
    C -->|Full| F[Peri-articular: bursitis, tendonitis, enthesitis]
    D --> G{Resisted Movement}
    G -->|Painful| H[Tendinopathy / Myositis]
    G -->|Weak| I[Neuropathy / Myopathy / Radiculopathy]
```
