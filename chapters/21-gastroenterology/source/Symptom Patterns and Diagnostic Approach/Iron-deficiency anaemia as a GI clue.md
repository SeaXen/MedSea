...
topic_group: Occult blood loss and cancer clues
...
```mermaid
flowchart TD
A[Iron-deficiency anaemia] --> B[Confirm true iron deficiency]
B --> C{At-risk adult / GI clues?}
C -- Yes --> D[GI evaluation for occult bleeding or malabsorption]
C -- No --> E[Consider alternative explanations but reassess carefully]
D --> F[Treat cause + replace iron]
```
