...
heading: Pancreatic Disorders
topic_group: Chronic pancreatic disease
...
```mermaid
mindmap
  root((Chronic Pancreatitis))
    Definition
      Irreversible fibro-inflammatory damage
    ...
    Diagnosis
      ...
      Functional tests (fecal elastase)
    Management
...
```mermaid
flowchart TD
  A[Chronic epigastric pain + weight loss] --> B[Imaging: CT/MRCP/EUS]
  B --> C{Calcification/ductal changes?}
  C -- Yes --> D[Diagnose chronic pancreatitis]
  C -- No --> E[Consider functional tests]
  E --> F[Fecal elastase low?]
  F -- Yes --> D
  F -- No --> G[Alternative diagnoses]
  D --> H[Assess 4 problem streams]
  H --> I[1. Pain: ladder + celiac plexus/endoscopic]
  H --> J[2. Maldigestion: enzyme replacement]
  H --> K[3. Diabetes: monitor + treat]
  H --> L[4. Complications: intervention if needed]
  I --> M[Stop alcohol + smoking]
```
