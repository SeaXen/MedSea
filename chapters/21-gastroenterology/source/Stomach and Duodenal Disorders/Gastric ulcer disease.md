---
tags: [medicine, gastroenterology, davidson, fcps, mrcp, full-note]
chapter: Gastroenterology
heading: Stomach and Duodenal Disorders
topic_group: Helicobacter and ulcer disease
  ...
  root((Gastric Ulcer Disease))
    Anatomy
      Stomach regions
        ...
        Pylorus
      Blood supply
        Lesser curve - watershed
        Greater curve - better perfusion
      Mucosal barrier
        Mucus-bicarbonate
        Tight junctions
    ...
    Physiology
      Acid secretion
        Parietal cells H+/K+-ATPase
        Gastrin/Histamine/ACh
      Defense failure
        Not acid hypersecretion
        Normal/low acid output
    Classification
      Chronicity
        Acute/stress
        Chronic
    ...
    Pathophysiology
      H. pylori
        ...
        Corpus gastritis → ↓acid
        Toxins → defense failure
      NSAIDs
        COX-1 inhibition → ↓PG
    ...
    Clinical
      ...
      Silent in elderly/NSAID
    Investigations
      OGD + biopsy (MANDATORY)
      H. pylori tests
      Gastrin if ZES suspected
    Diagnosis
    ...
    Management
      ...
      PPI BD x 8 weeks
      Stop NSAIDs
      Repeat OGD 6-8 weeks
      Surveillance if atrophy/IM
    Complications
      Bleeding
      Perforation
      Penetration
...
```mermaid
flowchart TD
    A[Suspected Gastric Ulcer] --> B{Urgent indication?}
    B -- Haemodynamic instability / Peritonitis --> C[Resuscitation → Emergency Endoscopy/Surgery]
    B -- Elective --> D[Upper GI Endoscopy + Biopsy 6-8 samples]
    D --> E{Histology}
    E -- Malignant --> F[MDT → Staging → Surgery/Oncology]
    E -- Benign --> G[Assess H. pylori + NSAID History]
    G --> H{H. pylori positive?}
    H -- Yes --> I[14-day Eradication Regimen + PPI BD x 8 weeks]
    H -- No --> J{NSAID use?}
    J -- Yes --> K[Stop NSAID if possible + PPI BD x 8 weeks]
    J -- No --> L[Idiopathic: Consider ZES, Crohn's, Vasculitis, Infection + PPI]
    I --> M[Confirm Eradication UBT/Stool Ag ≥4w post-Rx]
    K --> N[Repeat Endoscopy at 6-8 weeks]
    L --> N
    M --> N
    N --> O{Healed?}
    O -- Yes --> P[Discharge if no atrophy/IM]
    O -- No --> Q[Re-biopsy → Exclude Malignancy/Refractory Causes]
    P --> R{Atrophy/IM on histology?}
    R -- Yes OLGA III-IV --> S[Surveillance Endoscopy 3-yearly]
    R -- No --> T[No routine surveillance]
```
