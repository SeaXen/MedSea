...
## 17. Definitive prevention of recurrence
...
mindmap
  root((Gallstone Pancreatitis))
    Definition
      Acute pancreatitis from gallstones/sludge
    ...
    Diagnosis
      2 of 3 pancreatitis criteria
      ALT elevation
      US abdomen (stones/CBD)
    Key Decisions
      ERCP: cholangitis/obstruction only
...
```mermaid
flowchart TD
  A[Acute pancreatitis] --> B[ALT elevated?]
  B -- Yes --> C[US abdomen for stones]
  B -- No --> D[Consider other causes]
  C -- Stones/CBD dilatation --> E[Gallstone pancreatitis confirmed]
  C -- No stones --> F[Consider sludge/other]
  E --> G{Cholangitis or persistent obstruction?}
  G -- Yes --> H[Urgent ERCP]
  G -- No --> I[Supportive care]
  I --> J[Cholecystectomy after stabilization]
  H --> J
```
