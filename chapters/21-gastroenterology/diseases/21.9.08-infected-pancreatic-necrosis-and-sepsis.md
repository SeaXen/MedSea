...
```mermaid
mindmap
  root((Infected Pancreatic Necrosis))
    Definition
      Necrotic tissue + microbial infection
    Key Features
      Fever, sepsis, CT gas, deterioration
    Diagnosis
      CT + blood cultures + clinical decline
    Management
      Antibiotics + step-up drainage/debridement
    Complications
      Persistent organ failure, death
```

## 23. Flowchart
```mermaid
flowchart TD
  A[Necrotizing pancreatitis patient] --> B[Clinical deterioration + fever?]
  B -- Yes --> C[Start antibiotics + CT]
  B -- No --> D[Continue conservative mgmt]
  C --> E[Step-up drainage if infected]
  D --> E
```
