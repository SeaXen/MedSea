...
```mermaid
mindmap
  root((Whipple Disease))
    Definition
      Whipple = T. whipplei infection; middle-aged men...
    Key Features
      Classic: migratory arthralgia → weight loss → diar...
    Diagnosis
      Duodenal biopsy: PAS+ macrophages in lamina propri...
    Management
      PCR for T. whipplei = confirmatory...
    Complications
      Treatment: ceftriaxone 2g IV 14d → TMP-SMX 1-2 yea...
```

## 14. Flowchart
```mermaid
flowchart TD
  A[Whipple Disease Presentation] --> B{Key Diagnostic Feature?}
