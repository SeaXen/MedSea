...
source: "/mnt/tb/Medicine/Respiratory/Pleural Diseases/Primary spontaneous pneumothorax.md"
...
```mermaid
mindmap
  root((Primary Spontaneous Pneumothorax))
    Definition
    ...
    BTS Size
      ...
      Large ≥2cm hilum
    Management
      Small asympt: observe
      Small sympt: aspirate/observe
      Large: ASPIRATE first
      Aspiration fail: DRAIN
    Procedures
      Aspiration: 16-18G 2nd ICS MCL
      Drain: 10-14F pigtail 5th ICS safe triangle
    Adjuncts
      High-flow O2 (4x reabsorption)
      NSAIDs analgesia
    Recurrence
      30% at 1yr, 50% at 5yr
...
```mermaid
flowchart TD
    A[Young male sudden pleuritic pain] --> B[CXR: measure rim at hilum]
    B --> C{Rim <2cm?}
    C -- YES --> D{Asymptomatic?}
    D -- YES --> E[Discharge, analgesia\nFollow-up 2-4 wks\nNo flight 2wks post-resolution]
    D -- NO --> F[Consider aspiration\nor observe 24h]
    C -- NO (≥2cm) --> G[ASPIRATION 16-18G\nUp to 2.5L]
    G --> H{Success?\nRim <2cm + improved}
    H -- YES --> I[Observe 1-2h\nDischarge if stable\nFollow-up 2-4 wks]
    H -- NO --> J[CHEST DRAIN\n10-14F pigtail\n5th ICS safe triangle\nUnderwater seal]
    J --> K[Admit\nHigh-flow O2\nSuction if leak>48h]
    K --> L[Remove after leak stopped 24h\nCXR confirm]
    L --> M[Follow-up 2-4 wks\nVATS if recurrent]
```
...
> **Auto-generated PasTest/Mediscope-style scenario SBAs** grounded in the authored source. Each scenario tests a real clinical fact (triad, specific sign, contraindication, trial, first-line Rx) extracted from the topic. *Source: Ch 17: Respiratory Medicine — Primary spontaneous pneumothorax*

**Q1.** Which of the following features is most specific or characteristic of Primary spontaneous pneumothorax?
  ...
  > **Answer: A** — CT thorax
  >
  > *Source:* ep costophrenic angle**, **lucent hemithorax**, **visible pleural line**

### Optional / Selected
- **CT thorax**: if diagnostic uncertainty, planned surgery, suspected underlying disease, bilateral, 
