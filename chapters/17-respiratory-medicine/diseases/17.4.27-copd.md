...
source: "/mnt/tb/Medicine/Respiratory/COPD.md"
...
```mermaid
flowchart TD
  A[Chronic cough / sputum / dyspnea / smoking history] --> B[Assess severity and red flags]
  B --> C[Focused exam: wheeze, hyperinflation, cyanosis, cor pulmonale]
  C --> D[Confirm obstruction with post-bronchodilator spirometry]
  D --> E[Assess symptoms and exacerbation history]
  E --> F[Investigate phenotype and comorbidity: CXR, ECG, CBC, eosinophils, alpha-1 if needed]
  F --> G[Stable COPD plan: stop smoking + inhalers + vaccines + rehab]
  G --> H[If acute exacerbation: controlled oxygen + bronchodilators + steroids +/- antibiotics]
  H --> I[Check ABG and decide on NIV / ICU if acidotic or tiring]
```
...
```mermaid
flowchart TD
  A[Suspected COPD] --> B[Exposure history + symptoms]
  B --> C[Examine for hyperinflation, wheeze, cyanosis, edema]
  C --> D[Post-bronchodilator spirometry]
  D -->|FEV1/FVC < 0.70| E[Confirm COPD]
  E --> F[Assess symptom burden + exacerbation frequency]
  F --> G[Stable care: smoking cessation, inhalers, vaccines, rehab]
  G --> H[Monitor for LTOT need, comorbidities, inhaler technique]
  E --> I[If acute worsening]
  I --> J[Controlled oxygen + bronchodilators + steroids +/- antibiotics]
  J --> K[ABG]
  K -->|Acidotic hypercapnia| L[NIV]
  K -->|Failure / contraindication| M[Intubation / ICU]
```
...
> **Auto-generated PasTest/Mediscope-style scenario SBAs** grounded in the authored source. Each scenario tests a real clinical fact (triad, specific sign, contraindication, trial, first-line Rx) extracted from the topic. *Source: Ch 17: Respiratory Medicine — COPD*

**Q1.** What is the most appropriate first-line therapy for COPD?
  ...
  >
  > *Source:* **Systemic corticosteroids**:
   - e.g. prednisolone 40 mg daily for about 5 days in many protocols
4.
