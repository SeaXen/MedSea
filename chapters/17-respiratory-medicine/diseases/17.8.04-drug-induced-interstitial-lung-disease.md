...
source: "/mnt/tb/Medicine/Respiratory/Interstitial and Diffuse Parenchymal Lung Diseases/Drug-induced interstitial lung disease.md"
...
```mermaid
mindmap
  root((Drug-Induced ILD))
    Diagnosis
      Temporal association
      Exclusion (infection, CTD, malignancy, HP)
      Improvement on withdrawal
      NO rechallenge
    High-Risk Drugs
      Bleomycin (dose, O2 toxicity)
      Amiodarone (phospholipidosis)
      Nitrofurantoin (acute/chronic)
      Methotrexate (HP-like)
      ICI (anti-PD1/PD-L1/CTLA4)
      TKIs (EGFR/ALK)
      Amiodarone, Anti-TNF, Sirolimus
    HRCT Patterns
      HP-like (MTX, Nitro, Sulfa)
      NSIP (Amiodarone, ICI, Bleo, TKIs)
      DAD (ICI, Bleo, TKIs, Gem)
      UIP-like (Amiodarone, Nitro chronic, Bleo)
      OP (ICI, Bleo, TKIs, Anti-TNF)
    Management
      STOP drug
      Steroids if symptomatic
      Steroid-sparing if refractory
      NO rechallenge
    Specific
      Bleomycin: O2 toxicity, dose
      Amiodarone: Phospholipidosis, blue skin
      ICI: Grade-based, hold ICI
      TKIs: Early DAD
      MTX: Exclude infection first
```

## Flowchart
```mermaid
flowchart TD
    A[Patient on medication + new dyspnoea/cough] --> B[Temporal Association?\nDrug started before symptoms + known latency]
    B -- NO --> C[Not DILD\nConsider other causes]
    B -- YES --> D[Exclude Infection/Inflammation/Malignancy\nCultures, CTD screen, CT/PET, HRCT, BAL]
    D --> E{Exclusion Complete?}
    E -- NO --> F[Continue Exclusion]
    E -- YES --> G[Drug Withdrawal]
    G --> H{Clinical/Radiological Improvement?}
    H -- YES --> I[DILD CONFIRMED\nMonitor PFTs/HRCT]
    H -- NO --> J[Reconsider Diagnosis\nConsider Biopsy]
    I --> K{Severity}
    K -- Mild/Asymptomatic --> L[Monitor Only\nPFTs/HRCT 3-6mo]
    K -- Moderate/Severe --> M[Steroids\nPred 0.5-1mg/kg → taper 3-6mo]
    M --> N{Response}
    N -- Good --> O[Taper Steroids\nMonitor]
    N -- Refractory --> P[Steroid-Sparing\nMMF, AZA, CYC, RTX\nAnti-fibrotics if progressive]
```
...
> **Auto-generated PasTest/Mediscope-style scenario SBAs** grounded in the authored source. Each scenario tests a real clinical fact (triad, specific sign, contraindication, trial, first-line Rx) extracted from the topic. *Source: Ch 17: Respiratory Medicine — Drug-induced interstitial lung disease*

**Q1.** What is the most appropriate first-line therapy for Drug-induced interstitial lung disease?

  - **A.** Stop offending drug immediately + Do NOT rechallenge
  - **B.** An advanced/surgical therapy reserved for refractory disease
  - **C.** Symptomatic treatment only, no disease-modifying therapy
  - **D.** Empiric broad-spectrum therapy without specific indication

  > **Answer: A** — Stop offending drug immediately + Do NOT rechallenge
  >
  > *Source:* Immediate Drug Withdrawal (First Step)
- **Stop offending drug immediately**
- **Do NOT rechallenge** (fatal recurrence)
