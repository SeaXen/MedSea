...
```mermaid
flowchart TD
    A[Acute severe breathlessness] --> B[ABC + SpO2 + ABG + ECG + CXR]
    B --> C{Unilateral hyperresonance + ↓BS?}
    C -->|Yes| D[Tension PTX → needle decompression + drain]
    B --> E{Sudden + pleuritic + tachycardia?}
    E -->|Yes| F[PE suspected → CTPA + Wells/D-dimer]
    B --> G{Bilateral crackles + cardiomegaly?}
    G -->|Yes| H[Acute pulmonary oedema → diuretics + O2 + NIV]
    B --> I{Silent chest + exhaustion?}
    I -->|Yes| J[Severe asthma → intubation consideration]
    B --> K{Stridor + anaphylaxis?}
    K -->|Yes| L[IM adrenaline + airway]
    B --> M[Otherwise: pneumonia, AECOPD, asthma, effusion, etc.]
```
...
> **Auto-generated PasTest/Mediscope-style scenario SBAs** grounded in the authored source. Each scenario tests a real clinical fact (triad, specific sign, contraindication, trial, first-line Rx) extracted from the topic. *Source: Ch 17: Respiratory Medicine — Acute severe breathlessness approach*

**Q1.** What is the most appropriate first-line therapy for Acute severe breathlessness approach?
  ...
  > **Answer: A** — Anxiety/panic
  >
  > *Source:* **Anxiety/panic**   Reassure, breathing exercises, treat cause
