...
## 3. Aetiology — comprehensive differential
...
## 6. Diagnosis — structured approach

```mermaid
flowchart TD
    A[Dyspnoea] --> B[Acute vs chronic?]
    B -->|Acute| C[ABG, CXR, ECG, SpO2, D-dimer]
    C --> D{Pneumothorax?}
    D -->|Yes| E[Chest drain]
    C --> F{PE suspected?}
    F -->|Yes| G[Wells/D-dimer/CTPA → treat]
    C --> H{Pneumonia/oedema?}
    H -->|Yes| I[Treat infection/diuretics]
    C --> J{Asthma/COPD?}
    J -->|Yes| K[Bronchodilators, steroids]
    C --> L{Cardiac?}
    L -->|Yes| M[ECG, troponin, echo, treat]
    C --> N{Metabolic?}
    N -->|Yes| O[ABG, lactate, treat cause]
    B -->|Chronic| P[Full history + CXR + spirometry + ABG + ECG + FBC + BNP]
    P --> Q{Obstructive spirometry?}
    Q -->|Yes| R[COPD/asthma workup]
    P --> S{Restrictive spirometry?}
    S -->|Yes| T[HRCT → ILD vs NM vs chest wall]
    P --> U{Normal spirometry, low SpO2?}
    U -->|Yes| V[CTPA, echo, anaemia, NM]
    P --> W{Cardiac cause?}
    W -->|Yes| X[Echo, BNP → treat HF]
```
...
> **Auto-generated PasTest/Mediscope-style scenario SBAs** grounded in the authored source. Each scenario tests a real clinical fact (triad, specific sign, contraindication, trial, first-line Rx) extracted from the topic. *Source: Ch 17: Respiratory Medicine — Dyspnea*

**Q1.** What is the most appropriate first-line therapy for Dyspnea?
