...
source: "/mnt/tb/Medicine/Respiratory/Interstitial and Diffuse Parenchymal Lung Diseases/Idiopathic pulmonary fibrosis.md"
...
```mermaid
flowchart TD
A[Progressive exertional dyspnea + dry cough] --> B[Examine for bibasal crackles and clubbing]
B --> C[Do spirometry and pulse oximetry]
C --> D[Restrictive pattern or desaturation?]
D --> E[HRCT chest]
E --> F[Assess for UIP pattern]
F --> G[Exclude CTD, exposure, drug causes]
G --> H{Typical IPF/UIP pattern?}
H -- Yes --> I[Diagnose IPF in MDT context]
H -- No --> J[Consider alternative ILD / further workup]
```
...
```mermaid
flowchart LR
A[Older adult with dry cough and exertional dyspnea] --> B[Crackles/clubbing]
B --> C[Restrictive spirometry + low DLCO]
C --> D[HRCT UIP pattern]
D --> E[Exclude CTD exposure drugs]
E --> F[IPF diagnosis]
F --> G[Antifibrotics + supportive care]
```
