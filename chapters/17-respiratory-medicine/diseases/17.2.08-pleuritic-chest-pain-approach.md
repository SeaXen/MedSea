...
```mermaid
flowchart TD
    A[Pleuritic chest pain] --> B[Examination + CXR + ECG]
    B --> C{Unilateral hyperresonance + ↓BS?}
    C -->|Yes| D[Pneumothorax → chest drain]
    B --> E{Consolidation + fever?}
    E -->|Yes| F[Pneumonia → antibiotics]
    B --> G{Dullness + ↓BS?}
    G -->|Yes| H[Pleural effusion → tap + Light's]
    B --> I{Tachycardia + dyspnoea + normal CXR?}
    I -->|Yes| J[PE → Wells + D-dimer → CTPA]
    B --> K{Pericardial rub + diffuse ST elevation?}
    K -->|Yes| L[Pericarditis → ECG + echo + Rx]
    B --> M{Rib tenderness + trauma?}
    M -->|Yes| N[Rib fracture → CXR + analgesia]
    B --> O[Musculoskeletal, shingles, etc.]
```
...
> **Auto-generated PasTest/Mediscope-style scenario SBAs** grounded in the authored source. Each scenario tests a real clinical fact (triad, specific sign, contraindication, trial, first-line Rx) extracted from the topic. *Source: Ch 17: Respiratory Medicine — Pleuritic chest pain approach*

**Q1.** What is the most appropriate first-line therapy for Pleuritic chest pain approach?
