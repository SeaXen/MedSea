...
## 14. Common Viva Questions (extended)
...
## 15. Confusions & Mnemonics (extended)
...
## 17. Flowchart — Management

```mermaid
flowchart TD
    A[Respiratory failure suspected] --> B[ABG + CXR + clinical assessment]
    B --> C[O2 to target SpO2]
    C --> D{ABG and clinical response}
    D -->|Type 1, improving| E[Continue O2, treat cause, monitor]
    D -->|Type 1, worsening| F[Consider CPAP, ITU review, intubate]
    D -->|Type 2, pH >7.25| G[Controlled O2 + medical Rx + NIV trial]
    D -->|Type 2, pH <7.25| H[Urgent NIV; if fails intubate]
    D -->|Any arrest/exhaustion| I[Intubate + ITU]
```
...
> **Auto-generated PasTest/Mediscope-style scenario SBAs** grounded in the authored source. Each scenario tests a real clinical fact (triad, specific sign, contraindication, trial, first-line Rx) extracted from the topic. *Source: Ch 17: Respiratory Medicine — Respiratory Failure*

**Q1.** What is the most appropriate first-line therapy for Respiratory Failure?
  ...
  > **Answer: A** — Use controlled O₂
  >
  > *Source:* **Use controlled O₂** (Venturi mask) for Type 2 patients
