...
source: "/mnt/tb/Medicine/Respiratory/Respiratory Infections/Pneumocystis jirovecii pneumonia.md"
...
```mermaid
flowchart TD
    A[Immunocompromised + Dyspnoea/Cough/Fever] --> B[ABG + HRCT]
    B --> C{Hypoxaemia + Perihilar GGO?}
    C -- YES --> D[Induced Sputum PCR]
    D --> E{Positive?}
    E -- YES --> F[PCP CONFIRMED\nStart Cotrimoxazole]
    E -- NO --> G[BAL PCR\nβ-D-glucan]
    G --> H{Positive?}
    H -- YES --> F
    H -- NO --> I[Alternative Diagnosis]
    F --> J{PaO2 <9.3 kPa?}
    J -- YES --> K[Adjunctive Steroids\nPred 40mg BD×5d → taper 21d]
    J -- NO --> L[Cotrimoxazole alone]
    K --> M[Monitor Response\nProphylaxis after recovery]
```
