...
source: "/mnt/tb/Medicine/Respiratory/Respiratory Failure and Ventilatory Support/ARDS.md"
...
flowchart TD
    ...
    E --> G[Mild: PF 200-300]
    E --> H[Moderate: PF 100-200]
    E --> I[Severe: PF <100]
    G --> J[Lung-protective ventilation: Vt 6ml/kg PBW, PEEP/FiO2 table]
    H --> J
    I --> K[Lung-protective ventilation + PRONE 12-16h/day]
    K --> L[NMB cisatracurium 48h if PF<150]
    L --> M[Conservative fluid strategy]
    M --> N{Refractory? PF<80 >6h or pH<7.25}
    N -- Yes --> O[ECMO referral]
    N -- No --> P[Continue, monitor for fibrosis, wean when ready]
```
...
```mermaid
flowchart TD
    A[Acute hypoxaemic failure] --> B[Assess ABCDE, CXR, ABG, Echo]
    B --> C{Berlin criteria met?}
    C -- No --> D[Alternative diagnosis]
    C -- Yes --> E[Classify by PF ratio]
    E --> F[Mild 200-300]
    E --> G[Moderate 100-200]
    E --> H[Severe <100]
    F --> I[Lung-protective vent]
    G --> I
    H --> J[Lung-protective vent + PRONE 12-16h]
    J --> K[NMB cisatracurium 48h]
    K --> L[Conservative fluids]
    L --> M{Refractory?}
    M -- PF<80 >6h or pH<7.25 --> N[ECMO referral]
    M -- Improving --> O[Wean readiness assessment]
    O --> P[Extubation criteria met?]
    P -- Yes --> Q[Extubate]
    P -- No --> R[Continue support, monitor fibrosis]
```
...
> **Auto-generated PasTest/Mediscope-style scenario SBAs** grounded in the authored source. Each scenario tests a real clinical fact (triad, specific sign, contraindication, trial, first-line Rx) extracted from the topic. *Source: Ch 17: Respiratory Medicine — ARDS*

**Q1.** What is the most appropriate first-line therapy for ARDS?

  - **A.** PEEP + PEEP/FiO2 table
  - **B.** An advanced/surgical therapy reserved for refractory disease
  - **C.** Symptomatic treatment only, no disease-modifying therapy
  - **D.** Empiric broad-spectrum therapy without specific indication

  > **Answer: A** — PEEP + PEEP/FiO2 table
  >
  > *Source:* **PEEP**   **PEEP/FiO2 table** (higher PEEP for severe)
