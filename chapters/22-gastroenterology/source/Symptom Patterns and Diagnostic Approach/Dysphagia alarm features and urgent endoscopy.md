...
```mermaid
flowchart TD
A[Dysphagia] --> B{Emergency features?}
B -- Yes --> C[Urgent hospital assessment / emergency endoscopy]
B -- No --> D{Oropharyngeal or oesophageal?}
D -- Oropharyngeal --> E[Swallow assessment +/- targeted imaging]
D -- Oesophageal --> F{Alarm features?}
F -- Yes --> G[Urgent upper GI endoscopy]
F -- No --> H[Still investigate promptly if persistent]
G --> I[Biopsy / therapy / staging as needed]
```
