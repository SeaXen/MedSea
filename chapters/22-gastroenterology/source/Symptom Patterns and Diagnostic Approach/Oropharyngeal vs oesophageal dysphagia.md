...
```mermaid
flowchart TD
  A[Dysphagia] --> B{Oropharyngeal signs?}
  B -- Yes --> C[Videofluoroscopy / ENT]
  B -- No --> D[Oesophageal]
  D --> E{Solids > Liquids?}
  E -- Yes --> F[Structural: Stricture/Ring/Cancer]
  E -- No --> G[Motility: Achalasia/Spasm]
  F --> H[Endoscopy + Dilate/Biopsy]
  G --> I[Manometry + Barium]
```
