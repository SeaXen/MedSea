...
heading: Upper Gastrointestinal Bleeding
topic_group: Initial assessment and stabilization
...
## Investigations During Resuscitation
- CBC / haemoglobin
- U&E
- coagulation profile
...
```mermaid
flowchart TD
A[Haematemesis / melaena] --> B[Assess ABCs]
B --> C{Haemodynamic instability?}
C -- Yes --> D[Resuscitate: oxygen if needed, 2 IV lines, bloods, fluids/blood, senior help]
C -- No --> E[Assess severity and monitor closely]
D --> F[Correct coagulopathy / review anticoagulants]
E --> F
F --> G[Urgent/early endoscopy after stabilization]
```
...
```mermaid
flowchart TD
  A[UGIB Presentation] --> B[Resuscitate]
  B --> C[Risk Stratify]
  C --> D{High Risk?}
  D -- Yes --> E[Urgent Endoscopy]
  D -- No --> F[Early Endoscopy]
  E --> G[Endoscopic Haemostasis]
  F --> G
  G --> H[Prevent Rebleeding]
  H --> I[Address Cause]
```
