...
```mermaid
flowchart TD
A[Diarrhoea] --> B{Blood/fever?}
B -- Yes --> C[Stool culture logic]
B -- No --> D{Travel/persistent?}
D -- Yes --> E[Ova/parasite logic]
D -- No --> F{Recent antibiotics/healthcare?}
F -- Yes --> G[C difficile testing]
F -- No --> H[Often supportive care only]
```
