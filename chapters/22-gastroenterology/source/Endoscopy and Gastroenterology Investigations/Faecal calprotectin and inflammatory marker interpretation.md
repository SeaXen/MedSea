...
topic_group: Laboratory and stool testing
...
```mermaid
flowchart TD
A[Lower-GI symptoms] --> B{Inflammatory vs functional uncertainty?}
B -- Yes --> C[Faecal calprotectin]
C --> D{High?}
D -- Yes --> E[Inflammatory workup / colon evaluation]
D -- No --> F[Functional disease more likely if no red flags]
```
