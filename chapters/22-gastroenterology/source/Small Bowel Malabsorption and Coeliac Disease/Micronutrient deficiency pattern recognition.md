...
topic_group: Diarrhoea and malabsorption interpretation
...
```mermaid
flowchart TD
  A[Micronutrient Deficiency Pattern Recognition Presentation] --> B{Key Diagnostic Feature?}
  B -- Yes --> C[Confirm Diagnosis]
  B -- No --> D[Differential Diagnosis]
  C --> E{Severity/Complexity?}
  E -- High --> F[Aggressive/Specialist Management]
  E -- Low --> G[Standard Management]
  F --> H[Monitor Response]
  G --> H
  H --> I{Response?}
  I -- Yes --> J[Maintain/Step Down]
  I -- No --> K[Escalate/Rescue]
```
