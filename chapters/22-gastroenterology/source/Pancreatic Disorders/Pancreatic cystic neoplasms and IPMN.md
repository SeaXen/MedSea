...
topic_group: Pancreatic neoplasia
...
```mermaid
mindmap
  root((Pancreatic Cystic Neoplasms / IPMN))
    Definition
      Epithelial cystic neoplasms; IPMN from ductal epithelium
    Key Features
      Incidental, recurrent pancreatitis, mucin production, malignant potential
    Diagnosis
      CT/MRI/MRCP, EUS, cyst fluid CEA/amylase
    Management
      Surveillance by risk; surgery for high-risk/main duct/mural nodules
    Complications
      Invasive carcinoma, main duct obstruction
```

## Flowchart
```mermaid
flowchart TD
  A[Incidental pancreatic cyst on imaging] --> B[Post-pancreatitis pseudocyst?]
  B -- Yes --> C[Observe if simple fluid collection]
  B -- No --> D[Evaluate as cystic neoplasm]
  C --> E[Risk-stratified surveillance or surgery]
  D --> E
```
