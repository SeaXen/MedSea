...
topic_group: Acute pancreatitis syndromes
...
## Local complication management principles
...
```mermaid
mindmap
  root((Acute Pancreatitis))
    Definition
      2 of 3 criteria
      Pain + enzymes >3x ULN + imaging
    Etiology
      Gallstones
      Alcohol
...
```mermaid
flowchart TD
  A[Epigastric pain + vomiting] --> B[Check amylase/lipase]
  B --> C{>3x ULN?}
  C -- Yes --> D[2 of 3 criteria met?]
  C -- No --> E[Consider alternative diagnosis]
  D -- Yes --> F[Diagnose acute pancreatitis]
  F --> G[Assess severity]
  G --> H{Organ failure?}
  H -- Yes --> I[Severe - ICU, organ support]
  H -- No --> J[Mild/Moderate - ward care]
  J --> K[Gallstones?]
  K -- Yes --> L[US abdomen, consider ERCP if cholangitis]
  K -- No --> M[Alcohol/other, supportive care]
  I --> N[Monitor for necrosis/infection]
  N --> O{Infected necrosis?}
  O -- Yes --> P[Step-up drainage/debridement]
  O -- No --> Q[Conservative management]
```
