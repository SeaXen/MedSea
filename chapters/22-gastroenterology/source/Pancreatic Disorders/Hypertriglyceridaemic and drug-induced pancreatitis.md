...
```mermaid
mindmap
  root((Hypertriglyceridaemic/Drug Pancreatitis))
    Definition
      TG >1000 mg/dL or drug trigger
    Key Features
      Lipemic serum, metabolic syndrome, drug history
    Diagnosis
      Pancreatitis criteria + TG/med review
    Management
      Supportive + TG control + drug withdrawal
    Complications
      Recurrent pancreatitis, organ failure
```

## Flowchart
```mermaid
flowchart TD
  A[Acute pancreatitis with no gallstones/alcohol] --> B[TG >1000 or culprit drug?]
  B -- Yes --> C[Treat cause + supportive care]
  B -- No --> D[Consider other rare causes]
  C --> E[Prevent recurrence]
  D --> E
```
