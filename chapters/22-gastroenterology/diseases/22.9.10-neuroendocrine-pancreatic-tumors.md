...
```mermaid
mindmap
  root((Neuroendocrine Pancreatic Tumors))
    Definition
      Neoplasm from neuroendocrine cells, functional or non-functional
    Key Features
      Insulinoma/gastrinoma/glucagonoma/VIPoma syndromes
    Diagnosis
      CgA, hormone levels, CT/MRI/EUS, DOTATATE PET
    Management
      Functional: hormone control; Non-functional: resection by stage
    Complications
      Metastases, carcinoid syndrome, hypoglycemia death
```

## 21. Flowchart
```mermaid
flowchart TD
  A[Suspected pNET] --> B[Functional syndrome?]
  B -- Yes --> C[Target hormone + localize tumor]
  B -- No --> D[Stage + consider resection]
  C --> E[Syndrome control + oncologic outcome]
  D --> E
```
