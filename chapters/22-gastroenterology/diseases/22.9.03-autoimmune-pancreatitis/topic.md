...
## 21. Answer Key Pearls
- In viva, the one-liner is: **“AIP is a steroid-responsive pancreatic cancer mimic, often linked to IgG4 disease.”**

## 22. Mind Map
```mermaid
mindmap
  root((Autoimmune Pancreatitis))
    Definition
      Immune-mediated fibro-inflammatory disease
      Steroid-responsive
    Types
      Type 1 (IgG4-related)
      Type 2 (Pancreas-predominant, IBD-associated)
    Clinical
      Painless jaundice
      Pancreatic mass/enlargement
      Weight loss
      Other IgG4 organ involvement
    Diagnosis
      Imaging (diffuse enlargement)
      Serum IgG4
      Histology (if needed)
      Steroid response
    Differential
    ...
    Management
      ...
      Steroid therapy
      Malignancy exclusion first
```

## 23. Flowchart
```mermaid
flowchart TD
  A[Painless jaundice / pancreatic mass] --> B[Consider AIP vs Cancer]
  B --> C[CT/MRI/EUS: diffuse enlargement?]
  C -- Yes --> D[Check serum IgG4]
  C -- No --> E[Consider other diagnoses]
  D -- Raised IgG4 --> F[Support type 1 AIP]
  D -- Normal --> G[Consider type 2 / other]
  F --> H[Exclude malignancy thoroughly]
  H --> I[Initiate steroid trial]
