...
```mermaid
mindmap
  root((Acute Severe Ulcerative Colitis))
    Definition
      ASUC = Truelove-Witts: ≥6 bloody stools + systemic...
    Key Features
      Admit + IV hydrocortisone + exclude C. diff/CMV...
    Diagnosis
      Day 3 Oxford: stool >8 or CRP >45 → rescue/colecto...
    Management
      Rescue: infliximab 5mg/kg OR ciclosporin 2mg/kg IV...
    Complications
      Colectomy if fail rescue or toxic megacolon/perfor...
```

## 25. Flowchart
```mermaid
flowchart TD
  A[Acute Severe Ulcerative Colitis Presentation] --> B{Key Diagnostic Feature?}
