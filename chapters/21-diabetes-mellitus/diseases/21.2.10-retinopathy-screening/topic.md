...
```mermaid
flowchart TD
    A[Diabetic Patient] --> B{Age/Type/Duration}
    B -->|T1DM >5y OR T2DM at Dx| C[Annual Digital Photography]
    B -->|T1DM <5y| D[Annual from 5y post-dx]
    C --> E[Grade: R0-R4, M0-M1]
    D --> E
    E --> F{Grade}
    F -->|R0| G[2-yearly screen]
    F -->|R1| H[Annual screen]
    F -->|R2| I[6-12 month screen]
    F -->|R3| J[Urgent refer 2-4 weeks]
    F -->|R4| K[Urgent refer 1-2 weeks]
    F -->|M1| L[Urgent refer 2-4 weeks]
```
