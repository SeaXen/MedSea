...
source: "/mnt/tb/Medicine/Infectious Disease/Typhoid and Paratyphoid Fever.md"
...
```mermaid
flowchart TD
  A[Suspected Enteric Fever] --> B[Blood Culture x2-3 sets BEFORE antibiotics]
  B --> C{Clinical Suspicion High?}
  C -->|Yes| D[Start Ceftriaxone 2g IV OD empirically]
  C -->|No| E[Await Culture]
  D --> F[Blood Culture +ve?]
  F -->|Yes| G[Continue Ceftriaxone 7-10d OR Switch to Azithromycin PO]
  F -->|No| H[Consider Alternatives; Repeat Culture]
  G --> I[Monitor for Complications: Perforation, Haemorrhage, Relapse]
  I --> J{Relapse?}
  J -->|Yes| K[Full Course Different Class]
  J -->|No| L[Screen Carrier if Persistent Shedding]
```
