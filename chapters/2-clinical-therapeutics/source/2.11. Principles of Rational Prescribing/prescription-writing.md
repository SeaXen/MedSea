# Prescription Writing & Legal Requirements
...
## 2. Legal Requirements of a UK Prescription (BNF / GPhC)
...
## 3. Mermaid Algorithm — Safe Prescription Writing

```mermaid
flowchart TD
    A[Patient encounter] --> B[Confirm identity<br/>Name, DOB, address]
    B --> C[Confirm allergy status]
    C --> D[Check drug + dose + route + frequency]
    D --> E{Is this a<br/>controlled drug?}
    E -->|Yes| F[CD requirements:<br/>words + figures, handwritten]
    E -->|No| G[Standard Rx]
    F --> H[Write drug name in full<br/>generic unless narrow TI]
    G --> H
    H --> I[Use accepted abbreviations only<br/>OD, BD, TDS, QDS, PRN, PO, IV, IM, SC]
    I --> J[Add duration and quantity]
    J --> K[Sign + date + GMC number]
    K --> L{Is this a<br/>high-risk drug?<br/>warfarin, methotrexate, insulin, opioids}
...
### 4.1 Common Abbreviations (Safe vs Unsafe)
...
### 4.3 UK Misuse of Drugs Act — Schedule Categories
...
### 4.4 Common LASA Drugs (Look-Alike, Sound-Alike)
