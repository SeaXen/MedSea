...
## 2. 2. Rawlins-Thompson Classification (Type A-F)
...
## 3. 3. DoTS Classification (Dose, Time, Susceptibility)
...
### 1. Susceptibility Factors — High Yield
| Factor | Impact |
|--------|--------|
...
## 4. 4. Key HLA Associations (Exam Favourites)
...
## 5. 5. Cutaneous ADR Spectrum (RegiSCAR / EuroSCAR / SCORTEN)

```mermaid
flowchart TD
    A[Drug Rash] --> B{Mucosal Involvement?}
    B -->|No| C[Morbilliform / Exanthematous]
    B -->|Yes| D{Systemic Features?}
    C --> C1[Most common 90%
Maculopapular, Symmetrical
Spares face initially
Benign, Resolves on withdrawal
Penicillins, Allopurinol, Sulfa, AEDs]
    D -->|Fever, Eosinophilia,
Lymphadenopathy, Organ| E[DRESS / DIHS]
    D -->|Nikolsky +ve,
BSA Detachment| F{BSA Detachment %}
    F -->|<10%| G[SJS]
    F -->|10-30%| H[SJS/TEN Overlap]
    F -->|>30%| I[TEN]
    D -->|Acute Pustules,
Neutrophilia, Fever| J[AGEP]
```

### 1. Cutaneous ADR Comparison
...
### 2. SCORTEN (TEN Prognostic Score) — 1 point each
1. Age >40
2. Malignancy
3. Tachycardia >120
4. BSA detachment >10%
...
### 3. RegiSCAR Criteria for DRESS (Diagnostic)
...
## 6. 6. Hepatic ADRs (DILI Patterns) & Hy's Law
...
### 2. Hy's Law — **Regulatory Red Flag**
...
## 7. 7. Haematological ADRs — HIT (High Yield)
...
mindmap
  root((ADR Classification))
    Rawlins-Thompson (Type A-F)
      A: Augmented (80%, Dose-dep, Predictable)
      B: Bizarre (Idiosyncratic, Rare, Severe)
      C: Chronic (Cumulative, Long-term)
      D: Delayed (Carcinogen, Teratogen)
      E: End of Use (Withdrawal/Rebound)
      F: Failure (Therapeutic failure)
    DoTS
      Dose-relatedness
      Time-relatedness
      Susceptibility (Genetics, Age, Disease, Drugs)
    HLA Associations
      B*15:02 → Carbamazepine SJS/TEN
      B*58:01 → Allopurinol SJS/DRESS
      B*57:01 → Abacavir / Flucloxacillin
    ...
    Cutaneous ADRs
      ...
      Overlap 10-30%
      TEN >30%
      DRESS (Eosinophilia, Organ)
      AGEP (Pustules, Neutrophilia)
    DILI
      Hepatocellular (ALT>ALP)
      Cholestatic (ALP>ALT)
      Mixed
      Hy's Law (ALT>3x+Bili>2x+ALP<2x)
    Haematological
      HIT (4Ts, PF4, SRA, Argatroban)
      Agranulocytosis (Clozapine, Carbimazole)
      Thrombocytopenia (Quinine, Valproate, HIT)
      Haemolytic (Penicillins, Dapsone/G6PD)
```
...
> **Auto-generated PasTest/Mediscope-style scenario SBAs** grounded in the authored source. Each scenario tests a real clinical fact (triad, specific sign, contraindication, trial, first-line Rx) extracted from the topic. *Source: Ch 2: Clinical Therapeutics — ADR Classification*

**Q1.** Which of the following features is most specific or characteristic of ADR Classification?
  ...
  >
  > *Source:* B*13:01 | Dapsone Hypersensitivity Syndrome | Chinese | Consider in high-prevalence populations |
| **Lamotrigine** | HLA-B*38:02 (weak) | SJS/TEN | — | Not routine; Slow titration key |
| **Nevirapin
