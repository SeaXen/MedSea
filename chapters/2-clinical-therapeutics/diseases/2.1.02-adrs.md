...
## 2. 2. Core Concept: ADR Definitions & Taxonomy

```mermaid
flowchart TD
    A[Adverse Drug Reaction] --> B[Type A: Augmented
...
Carcinogenesis, Teratogenesis]
    A --> F[Type E: End of use
Withdrawal syndromes]
    A --> G[Type F: Failure
Ineffectiveness, Resistance]
...
### 1. ADR vs ADE vs Medication Error
...
## 3. 3. ADR Classification Systems

### 1. 2.1 Rawlins & Thompson (Type A-F) — Exam Standard
...
### 2. 2.2 DoTS Classification (Dose, Time, Susceptibility) — Mechanistic

| Dimension | Categories | Example |
|-----------|------------|---------|
| **Dose** | **Toxic** (supratherapeutic) — Paracetamol overdose
**Collateral** (therapeutic dose, off-target) — Opioid constipation
**Hypersusceptibility** (normal dose, exaggerated) — G6PD haemolysis with dapsone | Mechanistic insight for prevention |
| **Time** | **Rapid** (minutes-hours) — Anaphylaxis
**First-dose** — ACEi hypotension
**Early** (days-weeks) — SJS/TEN
**Intermediate** (weeks-months) — DILI
**Late** (months-years) — Tardive dyskinesia, carcinogenesis | Guides monitoring timeline |
| **Susceptibility** | **Genetic** — HLA-B*58:01 (allopurinol SJS), TPMT (azathioprine myelosuppression), CYP2C9*3 (warfarin)
**Physiological** — Renal/hepatic impairment, pregnancy, age
**Disease** — HIV (↑ cutaneous ADRs), EBV (ampicillin rash)
**Drug interactions** — CYP inhibition → toxicity | Personalised prescribing |

> **DoTS Advantage:** *Mechanistic — guides prevention (dose adjustment, genotyping, avoidance in susceptible).*

---

## 4. 4. Causality Assessment Tools

### 1. 3.1 Naranjo Algorithm — General ADR Causality (Exam Favourite)
...
### 2. 3.2 WHO-UMC Standardised Case Causality Assessment
...
### 3. 3.3 RUCAM (Roussel Uclaf Causality Assessment Method) — Hepatotoxicity Specific
...
## 5. 5. Pharmacovigilance & Reporting Systems

### 1. 4.1 UK Yellow Card Scheme (MHRA)
...
### 3. 4.3 Signal Detection Methods
...
## 6. 6. System-Specific ADR Patterns — High-Yield for Exams

### 1. 5.1 Cutaneous ADRs — Spectrum of Severity

```mermaid
flowchart LR
    A[Drug Rash] --> B[Morbilliform
Measles-like
7-14 days
Benign, self-limiting]
    A --> C[FDE
Fixed Drug Eruption
Recurring at same site
Hyperpigmentation]
    A --> D[Urticaria/Angioedema
IgE-mediated
Hours
Anaphylaxis risk]
    A --> E[SJS/TEN
...
<5 days
Fever, sterile pustules, neutrophilia
...
### 2. 5.2 Drug-Induced Liver Injury (DILI)
...
### 3. 5.3 Drug-Induced Renal Injury
...
### 4. 5.4 Drug-Induced Haematological Toxicity
...
## 7. 7. Practical Algorithms

### 1. Algorithm: ADR Encounter — Clinical Approach

```mermaid
flowchart TD
    A[Patient with Suspected ADR] --> B[Assess Severity
ABCDE, Anaphylaxis?]
    B -->|Anaphylaxis| C[ADRENALINE IM 0.5mg 1:1000
Stop drug, O2, Fluids, Antihistamine, Steroid]
    B -->|Non-acute| D[Document: Drug, Dose, Timing, Reaction, Dechallenge, Rechallenge]
    D --> E[Classify: Type A-F, DoTS]
    E --> F[Causality Assessment
Naranjo / WHO-UMC / RUCAM]
    F --> G{Serious?}
    G -->|Yes| H[Yellow Card + Clinical Review]
    G -->|No| I[Yellow Card if ▼ drug or serious]
    H --> J[Management:
Stop/Reduce/Switch/Monitor
Treat symptoms]
    I --> J
    J --> K[Follow-up: Resolution, Re-exposure risk, Documentation]
```

### 2. Algorithm: Cutaneous ADR Triage

```mermaid
flowchart TD
    A[Drug Rash] --> B{Mucosal involvement?}
    B -->|Yes| C{Blisters/Target lesions?}
    B -->|No| D[Morbilliform / Urticaria / FDE
Stop drug if possible
Antihistamines, topical steroids
Monitor]
    C -->|Yes + Nikolsky+| E[SJS/TEN
SCORTEN
ICU/Burns referral
STOP ALL non-essential drugs
IVIG / Ciclosporin / Etanercept debate]
    C -->|No| F{Eosinophilia + Systemic?}
    F -->|Yes| G[DRESS
Stop drug
Systemic steroids
Monitor LFTs, renal, HHV-6]
    F -->|No| H[AGEP if pustules + neutrophilia
