...
## 2. 2. 🧠 Core Concept: Start Smart Then Focus (UK AMS Framework)

```mermaid
flowchart TD
    A[Patient with Suspected Infection] --> B[Start Smart
<b>T = 0</b>]
    B --> C1[Take Cultures
Blood x2, Urine, Sputum,
Wound, CSF, Other]
    B --> C2[Assess Severity
Sepsis 6, NEWS2, qSOFA]
    B --> C3[Choose Empirical Abx
Local formulary + guidelines
Allergy history
Severity-adjusted]
    B --> C4[Document Indication
Dose, Route, Duration,
Review Date]
    C1 & C2 & C3 & C4 --> D[Focus
<b>T = 24-72h</b>]
    D --> E{Review: Culture Results
Clinical Response
Biomarkers (CRP, PCT)}
    E -->|Pathogen ID + Sens| F[De-escalate
Narrowest Spectrum
Targeted Therapy]
    E -->|No Growth / Improving| G[Stop or Switch to Oral
(IVOST) / Short Course]
    E -->|Deteriorating / Resistant| H[Escalate / Broaden
Specialist ID/Micro Advice]
    F & G & H --> I[Define Duration
...
## 3. 3. ️⃣ Empirical Antibiotic Selection — Common Syndromes (UK Guidelines)

### 1. Sepsis (Unknown Source) — NICE NG51 / Surviving Sepsis
...
### 2. Community-Acquired Pneumonia (CAP) — NICE NG138 / BTS
...
### 3. Hospital-Acquired Pneumonia (HAP) / VAP
...
### 5. Skin & Soft Tissue Infection (SSTI)
...
## 4. 4. ️⃣ IV to Oral Switch (IVOST) — Criteria

### 1. Mandatory Criteria (ALL must be met)
| Criterion | Detail |
|-----------|--------|
...
### 2. High Bioavailability Oral Agents (IVOST Candidates)
...
## 5. 5. ️⃣ Duration of Therapy — Evidence-Based Short Courses
...
## 6. 6. ️⃣ De-escalation — From Empirical to Targeted
...
flowchart TD
    ...
    D -->|Yes| E[Switch to Narrowest Effective
IVOST if criteria met
Document new stop date]
    D -->|No| F[Continue Current / Optimise Dose]
    B -->|No (Sterile)| G[Clinical Response?]
    G -->|Improving| H[Stop Antibiotics
(or IVOST if short course needed)]
    G -->|Not Improving| I[Reassess Source / Imaging
Specialist Advice]
```

---

## 7. 7. ️⃣ C. difficile Infection (CDI) Prevention — Antibiotic Stewardship

### 1. High-Risk Antibiotics for CDI (Relative Risk)
...
## 8. 8. ️⃣ Penicillin Allergy De-labelling
...
Morbilliform rash,
SJS/TEN, DRESS, AGEP
>1h to days| E[Delay = Lower Risk
If mild rash >10y ago
→ Direct Oral Challenge
(Supervised)]
    C -->|Non-immune
GI upset, Headache,
Family history only| F[**NOT Allergy**
Remove Label
Use Penicillin Safely]
    C -->|Unknown / Vague| G[**Default: De-label**
Unless clear IgE history
Supervised Challenge if needed]
```

### 3. Cross-Reactivity (True)
...
## 9. 9. ️⃣ Carbapenem-Sparing & Last-Resort Agents
...
### 2. Last-Resort Agents — Reserve for MDR/XDR
...
## 10. 10. ️⃣ Biomarkers to Guide Duration
...
## 11. 11. ️⃣ Outpatient Parenteral Antimicrobial Therapy (OPAT)
| Indication | Examples |
|------------|----------|
...
## 12. 12. 🔟 Practical AMS Checklist (Prescriber)

```mermaid
flowchart TD
    A[Before Prescribing] --> B[1. Is it Infection?
Sepsis 6 / Clinical Criteria]
    B --> C[2. Cultures BEFORE Antibiotics
(Blood x2, Source)]
    C --> D[3. Local Guideline + Allergy
Severity-adjusted Empirical]
    D --> E[4. Document: Indication, Dose, Route,
Duration, Review Date (48-72h)]
    E --> F[At 48-72h Review]
    F --> G[5. Culture Results?
De-escalate / Stop / IVOST]
    G --> H[6. Clinical Improvement?
Stop / Short Course / IVOST]
    H --> I[7. Biomarkers (CRP/PCT)
Trend ↓ = Stop]
    I --> J[8. Discharge: Clear Stop Date,
OPAT Plan, GP Communication]
```
