...
```mermaid
mindmap
  root((NSAID Poisoning))
    Mechanism
      COX Inhibition
      COX-1: GI, Renal, Platelets
      COX-2: Inflammation
    Clinical
      GI: Nausea, Bleeding
      Renal: AKI
      CNS: Drowsiness, Seizures (Mefenamic)
    High Risk Agents
      Mefenamic Acid
      Phenylbutazone
    Management
...
```mermaid
flowchart TD
  A[NSAID Overdose] --> B[ABCDE + Renal Function + GI Assessment]
  B --> C{Large Ingestion\n>100mg/kg or\nMefenamic Acid?}
  C -->|Yes| D[Activated Charcoal 1g/kg\nif <1-2h]
  C -->|No| E[Observation + Supportive]
  D --> E
  E --> F{Complications?}
  F -->|GI Bleed| G[PPI + Fluids + Transfusion if Needed]
  F -->|AKI| H[IV Fluids, Avoid Nephrotoxins\nMonitor Renal Function]
  F -->|Seizures| I[Benzodiazepines]
  F -->|None| J[Discharge if Asymptomatic 6h\nPsych Assessment if DSH]
  G --> J
  H --> J
  I --> J
```
