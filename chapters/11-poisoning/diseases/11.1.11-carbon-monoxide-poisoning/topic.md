...
```mermaid
mindmap
  root((Carbon Monoxide Poisoning))
    Mechanism
      CO-Hb 200-250x Affinity
      Left Shift O2-Hb Curve
      Cytochrome Oxidase Inhibition
      Myoglobin Binding
    Clinical
      Acute: Headache, Nausea, Confusion, Chest Pain
...
```mermaid
flowchart TD
  A[Suspected CO Poisoning] --> B[Immediate 100% O2 NRB 15L/min\nDo NOT Wait for COHb]
  B --> C[COHb Level + ABG + ECG + Trop\nPregnancy Test]
  C --> D{HBO Indicated?}
  D -->|COHb>25%, LOC, Cardiac,\nPregnancy>15%, Neuro,\nAcidosis| E[Hyperbaric Oxygen\n2.5-3.0 ATA 90-120min\n1-3 Sessions]
  D -->|No| F[Continue 100% O2\nUntil Asymptomatic\nCOHb<5-10%]
  E --> F
  F --> G[Monitor Cardiac, Neuro\nDNS Follow-up 4-6 Weeks\nMRI if Symptoms]
```
