...
mindmap
  root((TCA Poisoning))
    Mechanism
      Na Channel Blockade (Class Ia)
      Anticholinergic
      Alpha-1 Blockade
      CNS: Seizures, Coma
    Clinical
      Cardiac: QRS>100, R aVR>3mm, VT/VF, Hypotension
      Anticholinergic: Dry, Mydriasis, Tachy
      CNS: Seizures, Coma
    ECG
      QRS Duration Severity
      Terminal R aVR
      QT Prolongation
    Management
      NaHCO3 (QRS>100)
      Benzo Seizures
      NE Hypotension
      Avoid: Phenytoin, Flumazenil, Class Ia/Ic
    Monitoring
      Continuous ECG
      ABG pH 7.50-7.55
      24h Post-Normalization
```

## 16. Flowchart
```mermaid
flowchart TD
  A[Overdose + QRS Widening + Anticholinergic + Seizures] --> B[TCA Poisoning]
  B --> C[ABCDE + Continuous ECG\nHyperventilate to pH 7.50-7.55 if Intubated]
  C --> D{QRS > 100 ms\nor R aVR > 3mm\nor VT/VF\nor Refractory Hypotension?}
  D -->|Yes| E[NaHCO3 1-2 mEq/kg IV Bolus\nRepeat to pH 7.50-7.55, QRS<100\nInfusion to Maintain]
  D -->|No| F[Monitor ECG, ABG q30min]
  C --> G{Seizures?}
  G -->|Yes| H[Lorazepam/Diazepam IV\nPhenobarbital/Propofol 2nd Line\nNO PHENYTOIN]
  G -->|No| F
  C --> I{Hypotension?}
  I -->|Yes| J[Fluids → Norepinephrine\nNO Beta-Blockers]
  I -->|No| F
  F --> K[Observe 24h Post-QRS Normalization\nPsych Assessment]
  E --> K
```
