...
```mermaid
mindmap
  root((Isopropyl Alcohol Poisoning))
    Metabolism
      ADH → Acetone (Ketone, NOT Acid)
      No Toxic Acid Metabolite
    Clinical
      CNS Depression (Potent)
      Ketosis WITHOUT Acidosis
      Hemorrhagic Gastritis
      Acetone Breath
    Labs
      Ketones POSITIVE
      Anion Gap Acidosis ABSENT
    ...
    Management
      ...
      PPI for Gastritis
    Differentiation
      Methanol: Acidosis + Vision
      EG: Acidosis + Renal + Crystals
      IPA: Ketosis + Gastritis + NO Acidosis
```

## 15. Flowchart
```mermaid
flowchart TD
  A[Intoxicated + Ketones + No Acidosis\n+ Hematemesis + Acetone Breath] --> B[Isopropyl Alcohol Poisoning]
  B --> C[ABCDE: Airway Protection\nFluids for Hypotension\nPPI for Gastritis]
  C --> D[Monitor: Glucose, Electrolytes,\nGCS, Renal Function\nSerial Ketones/ABG]
  D --> E{Complications?}
  E -->|Aspiration| F[Antibiotics, Respiratory Support]
  E -->|Significant GI Bleed| G[Transfusion, Endoscopy if Needed]
  E -->|Prolonged Coma| H[Supportive, Observe 12-24h]
  F --> I[Discharge When Awake,\nStable, Psych Assessment]
  G --> I
  H --> I
```
