...
```mermaid
flowchart TD
  A[Poisoned Patient] --> B{Time Since Ingestion?}
  B -->|< 1-2h| C{Toxin Adsorbed by Charcoal?}
  B -->|> 2h| D{Delayed Absorption?\nAnticholinergic, SR, Opioid,\nBezoar, Large Volume}
  D -->|Yes| C
  D -->|No| E[No Decontamination\nSupportive Care]
  C --> F{Airway Protected?}
  F -->|No| G[Intubate First\nThen Charcoal if Indicated]
  F -->|Yes| H{Contraindications?\nCorrosive, Hydrocarbon,\nPerforation, Obstruction}
  H -->|Yes| I[No Charcoal\nConsider WBI if Indicated]
  H -->|No| J[Activated Charcoal 1g/kg\n1:10 Ratio\nWater Vehicle]
  J --> K{Sustained-Release /\nBody Packer /\nIron /\nLithium?}
  K -->|Yes| L[Whole Bowel Irrigation\nPEG 1-2 L/hr\nUntil Clear Effluent]
  K -->|No| M[Monitor\nSerial Levels if Available]
  I --> L
```
