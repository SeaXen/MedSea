...
flowchart TD
  ...
  B -->|Likely >4| C[CTPA]
  B -->|Unlikely <=4| D{PERC positive?}
  D -->|All 8 absent| E[PE Excluded]
  D -->|Any positive| F[Age-adjusted D-dimer]
  F -->|Negative| E
  F -->|Positive| C
  C -->|Confirmed| G[Risk Stratify]
  G -->|Massive SBP<90| H[Thrombolysis]
  G -->|Sub-massive RV+| I[Anticoag + Monitor]
  G -->|Low risk| J[Anticoag + Discharge]
  H --> K[Surgical/catheter embolectomy if contra]
```

## 18. Flowchart — PE Management
```mermaid
flowchart TD
  A[PE Confirmed] --> B{Haemodynamic Status}
  B -->|SBP<90/shock| C[Massive PE]
  B -->|Stable| D{RV dysfunction?}
  C --> E[Thrombolysis + UFH]
  E --> F{Contraindication?}
  F -->|Yes| G[Embolectomy / ECMO]
  F -->|No| E
  D -->|Yes| H[Sub-massive PE]
  D -->|No| I[Low-risk PE]
  H --> J[Anticoag + ICU monitoring]
  I --> K[Anticoag + Early Discharge]
  J --> L{Deteriorating?}
  L -->|Yes| M[Thrombolysis / CDT]
  L -->|No| J
  K --> N[Duration: Provoked 3mo / Unprovoked extended]
```
