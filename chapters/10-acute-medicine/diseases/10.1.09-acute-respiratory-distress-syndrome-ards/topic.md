...
```mermaid
flowchart TD
  A[ARDS - P/F <=300] --> B{Severity}
  B -->|Mild 200-300| C[TV 6mL/kg PBW + PEEP 5-10]
  B -->|Mod 100-200| D[TV 6mL/kg + High PEEP + Prone >12h + Consider NMJ Blockade]
  B -->|Severe <100| E[All above + Paralysis 48h + ECMO Referral]
  C --> F{Reassess 24h}
  D --> F
  E --> F
  F -->|Worsening P/F| G[Rescue: iNO, HFOV, ECMO]
  F -->|Improving| H[Wean PEEP, then TV, then extubate]
  A --> I[Conservative Fluids + Sedation + VAP prophylaxis]
```
