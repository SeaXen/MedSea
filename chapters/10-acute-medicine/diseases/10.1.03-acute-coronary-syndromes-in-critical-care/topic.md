...
```mermaid
mindmap
  root((ACS in Critical Care))
    Definition
      STEMI: STE + Trop+
      NSTEMI: Trop+ No STE
      Unstable Angina: Trop-
    STEMI
      Reperfusion: PCI 90min / Fibrinolysis 30min
      DAPT: Aspirin + Ticagrelor/Prasugrel
      Anticoagulation: UFH/Enoxaparin/Bivalirudin
    NSTEMI
      Risk: GRACE/TIMI
      Early Invasive: GRACE>140 ≤24h
      DAPT + Anticoagulation (Fondaparinux)
...
```mermaid
flowchart TD
  A[Chest Pain + Trop Rise] --> B{ST Elevation?}
  B -->|Yes| C[STEMI]
  C --> D{PCI Available ≤120min?}
  D -->|Yes| E[Primary PCI\nDoor-to-Balloon ≤90min]
  D -->|No| F[Fibrinolysis\nDoor-to-Needle ≤30min\nPharmacoinvasive PCI 3-24h]
  B -->|No| G[NSTEMI / Unstable Angina]
  G --> H[GRACE / TIMI Score]
  H --> I{High Risk?\nGRACE>140 / Dynamic ST / HF}
  I -->|Yes| J[Early Invasive ≤24h]
  I -->|No| K[Invasive ≤72h or Conservative]
  E --> L[DAPT + Anticoagulation\nMechanical Complications Screen]
  F --> L
  J --> L
  K --> L
  L --> M[Post-MI: Beta-blocker, ACEi, Statin\nCardiac Rehab\nSecondary Prevention]
```
