---
tags: [hepatology, autoimmune_liver, overlap_syndrome, aih_pbc, aih_psc, fcps, mrcp]
davidson_chapter: Chapter 24
status: full-fcps-mrcp-note
priority: high
exam_relevance: "Autoimmune liver overlap syndromes - AIH-PBC, AIH-PSC, diagnostic criteria, management - FCPS/MRCP classic"
see_also: ["Autoimmune Liver Disease/Autoimmune hepatitis (AIH)", "Autoimmune Liver Disease/PBC (Primary Biliary Cholangitis)", "Autoimmune Liver Disease/Primary sclerosing cholangitis (PSC)", "Autoimmune Liver Disease/IgG4-related sclerosing cholangitis"]
created: 2025-06-13
modified: 2025-06-13
---
...
```mermaid
flowchart TD
    A[Suspect AIH-PBC Overlap] --> B{PBC Criteria}
    B -->|ALP >1.5xULN + AMA+| C{AIH Criteria}
    C -->|ALT >5xULN OR IgG >2xULN OR ANA/SMA ≥1:80| D[AIH-PBC OVERLAP]
    C -->|If Liver Biopsy| E[Interface Hepatitis + Florid Duct Lesion]
    E --> D
```
...
```mermaid
flowchart TD
    A[Suspect AIH-PSC Overlap] --> B{PSC Criteria}
    B -->|MRCP Beading / Cholangiography| C{AIH Criteria}
    C -->|ALT >5xULN OR IgG >2xULN OR ANA/SMA ≥1:80| D[AIH-PSC OVERLAP]
    C -->|Interface Hepatitis on Biopsy| D
```
...
## Differentiation: Pure vs Overlap
...
```mermaid
flowchart TD
    A[AIH-PBC Overlap] --> B[UDCA 13-15 mg/kg/day]
    B --> C{Response at 12mo}
    C -->|Paris I/II Criteria Met| D[Continue UDCA Alone]
    C -->|Incomplete Response| E[Add Immunosuppression]
    E --> F[Prednisolone 20-30mg/day → Taper]
    F --> G[Azathioprine 1-2mg/kg for Steroid-Sparing]
    G --> H[Monitor: LFTs, IgG, UDCA Response]
```
...
mindmap
  root((Overlap Syndromes))
    ...
    AIH-PSC
      ...
      Mgmt: UDCA + Pred/Aza
    Differential
      vs Pure PBC: ALT, IgG, ANA
      vs Pure AIH: ALP, AMA
      vs Pure PSC: ALT, IgG, ANA
    Management
      UDCA + Immunosuppression (Pred + Aza)
      Response: Paris I/II + ALT/IgG normalization
```
