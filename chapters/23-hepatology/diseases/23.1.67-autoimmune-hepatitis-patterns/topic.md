...
```mermaid
flowchart TD
    A[Young Woman + Jaundice + Hepatocellular Pattern] --> B{IgG Elevated?}
    B -->|Yes| C{Autoantibodies?}
    C -->|ANA/SMA ≥1:80 or LKM1 ≥1:40| D[AIH Likely]
    C -->|Negative| E[Consider Other Causes]
    B -->|No| F[Consider Viral / DILI / Wilson / Ischaemic]
```
...
```mermaid
flowchart TD
    A[Jaundice + Hepatocellular Pattern R>5] --> B{Age/Sex}
    B -->|Young Woman| C[AIH High on Differential]
    B -->|Other| D[Still Possible but Less Likely]
    C --> E[Check IgG]
    E -->|>ULN| F[Check Autoantibodies]
    F -->|ANA/SMA ≥1:80 or LKM1 ≥1:40| G[AIH Probable/Definite]
    F -->|Negative| H[Consider Seronegative AIH / Other]
    D --> I[Exclude Viral, DILI, Wilson, Ischaemic, Alcohol]
```
...
```mermaid
mindmap
  root((AIH in Jaundice))
    Pattern
      Hepatocellular R>5
      High IgG
    Autoantibodies
      Type 1: ANA/SMA ≥1:80 (80%)
      Type 2: LKM1 ≥1:40 (Children)
      Type 3: SLA/LP (Specific 95%)
    Diagnosis
      IAIHG Simplified ≥6 Probable, ≥7 Definite
      Exclude Viral/Drug/Alcohol/Wilson
    ALF Setting
      Only Steroid-Responsive ALF
      Pred 60mg → Day 7 Assess
      Lille >0.45 = Stop
    Differential
      vs Viral: IgG↑, AutoAb+
      vs Alcoholic: IgG↑, AutoAb+, AST:ALT<1
      vs Wilson: Ceruloplasmin Normal, No Haemolysis
      vs DILI: IgG↑, AutoAb+, Steroid Responsive
```
