...
```mermaid
flowchart LR
    A[Biliary Obstruction] --> B[Bile Stasis]
    B --> C[Bacterial Overgrowth]
    C --> D[Bile Duct Mucosal Injury]
    D --> E[Bacteria/Endotoxin → Portal Circulation]
    E --> F[Systemic Inflammatory Response]
    F --> G[Cholangitis]
```
...
```mermaid
mindmap
  root((Acute Cholangitis))
    Presentation
      Charcot: Fever, RUQ Pain, Jaundice
      Reynolds: + Shock + AMS
    TG18 Diagnosis
      A: Inflammation (Fever, WCC, CRP)
      B: Cholestasis (Bil, ALP, ALT)
      C: Imaging (Dilatation, Stone)
    Severity (TG18)
      Grade I: Mild (Abx Alone)
      Grade II: Moderate (2 Criteria → ERCP 24h)
      Grade III: Severe (Organ Failure → ERCP <12h)
    Treatment
      Antibiotics: Ceftriaxone / Meropenem
      Drainage: ERCP (Grade III <12h, II <24h, I Elective)
      Supportive: Fluids, Vasopressors, RRT
    Organisms
