...
```mermaid
flowchart TD
    A[Isolated Conjugated Hyperbilirubinaemia] --> B{Liver Colour / Urinary Coproporphyrins}
    B -->|Black Liver, Copro I >80%| C[Dubin-Johnson]
    B -->|Normal Liver, Total Copro ↑↑, Copro I 65%| D[Rotor Syndrome]
    B -->|Neither| E[Think: Drugs, Sepsis, Early Obstruction, TPN Cholestasis]
    C --> F[Benign, No Treatment]
    D --> F
    E --> G[Investigate Further]
```
...
```mermaid
mindmap
  root((Dubin-Johnson vs Rotor))
    Dubin-Johnson
      Gene: ABCC2 (MRP2)
      Defect: Canalicular excretion
      Liver: BLACK
      Coproporphyrin I: >80%
      HIDA: No visualization 24h
      BSP: Secondary rise 90min
    Rotor
      Genes: SLCO1B1 + SLCO1B3 (OATP1B1/1B3)
      Defect: Sinusoidal uptake + storage
      Liver: NORMAL
      Total Coproporphyrins: 2.5-5x normal
      Copro I: 65%
      HIDA: Rapid uptake/clearance
      BSP: Normal clearance
    Common
      Autosomal Recessive
      Conjugated Hyperbilirubinaemia
      Benign, Normal Life Expectancy
      NO Treatment
      Pregnancy/OCP may worsen
```
