...
```mermaid
flowchart TD
    A[Start SOF/VEL + RBV] --> B[Weekly: Hb, LFTs, CBC, Renal]
    B --> C{Clinical Status}
    C -->|Stable/Improving| D[Continue 12 weeks]
    C -->|Worsening: Ascites, HE, Bleed| E[Assess: Decompensation vs Drug Toxicity]
    E --> F[Stop RBV if Hb <8.5]
    E --> G[Consider Transplant Referral if MELD ↑]
    D --> H[SVR12 Assessment]
```
...
```mermaid
mindmap
  root((HCV Decompensated Cirrhosis))
    Regimen
      SOF/VEL + RBV 12w ONLY
      GLE/PIB = CONTRAINDICATED
      SOF/VEL/VOX = CONTRAINDICATED
    RBV Dosing
      <75kg: 600mg | ≥75kg: 1000mg
      Hb<10: Reduce
      Hb<8.5: STOP RBV
    Monitoring (Weekly)
      Hb, LFTs, INR, Cr, Na, HE
    ...
    SVR12 Rates
      ...
      Child C: 70-80%
    Post-SVR
      Lifelong HCC Surveillance (6m US)
      Variceal Surveillance
      Transplant if Decompensated
```
