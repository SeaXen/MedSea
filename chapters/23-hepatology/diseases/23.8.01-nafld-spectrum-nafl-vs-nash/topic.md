...
```mermaid
flowchart TD
    A[Hepatic Steatosis >5%] --> B{Alcohol <30g/day M, <20g/day F?}
    B -->|Yes| C{Other Causes Excluded?}
    C -->|Yes| D[NAFLD]
    C -->|No| E[Secondary Steatosis]
    B -->|No| F[Alcoholic Liver Disease]
    D --> G{Histology}
    G --> H[NAFL: Simple Steatosis]
    G --> I[NASH: Steatohepatitis]
```
...
|-------|------------|-----------|------------------|
| **NAFL (Simple Steatosis)** | **Steatosis >5%** WITHOUT significant inflammation or ballooning | • Steatosis >5%<br>• No ballooning<br>• No/minimal lobular inflammation | **Low** (5-10% progress) |
| **NASH (Non-Alcoholic Steatohepatitis)** | **Steatosis >5% + Ballooning + Lobular Inflammation** ± Fibrosis | • Steatosis >5%<br>• **Hepatocyte Ballooning**<br>• **Lobular Inflammation**<br>• ± Fibrosis | **High** (20-30% progress to cirrhosis) |
...
flowchart TD
    ...
    B --> C[Non-Invasive Fibrosis Assessment]
    C --> D{FIB-4 / NFS / FibroScan}
    D -->|Low Risk| E[Monitor: LFTs, US Annually]
    D -->|Intermediate/High Risk| F[Consider Liver Biopsy]
    F --> G{Histology}
    G -->|Steatosis Only| H[NAFL]
    G -->|Steatosis + Ballooning + Inflammation| I[NASH]
```
...
mindmap
  root((NAFLD Spectrum))
    ...
    NASH (Steatohepatitis)
      Steatosis >5%
      Ballooning PRESENT
      Lobular Inflammation PRESENT
      Fibrosis F0-F4
      High Progression Risk
    Fibrosis Stages
      F0: None
      F1: Perisinusoidal
      F2: Periportal + Perisinusoidal
      F3: Bridging
      F4: Cirrhosis
```
