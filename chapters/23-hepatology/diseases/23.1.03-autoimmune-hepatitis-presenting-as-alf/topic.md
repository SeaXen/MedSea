...
```mermaid
flowchart TD
    A[AIH Type] --> B{Antibody}
    B --> C[Type 1: ANA +/and SMA +]
    B --> D[Type 2: Anti-LKM1 +]
    B --> E[Type 3: Anti-SLA/LP +]
    C --> F[Most common in ALF]
    D --> G[Younger, more severe, less steroid responsive]
    E --> H[Specific for AIH; often with Type 1]
```
...
```mermaid
flowchart TD
    A[Suspect AIH-ALF: Woman, high IgG, autoantibodies+, no other cause] --> B{Contraindications to Steroids?}
    B -->|Active sepsis, GI bleed, renal failure| C[Delay/avoid steroids; Transplant eval]
    B -->|No absolute contraindication| D[START PREDNISOLONE 60mg/day IV/PO]
    D --> E[Monitor: LFTs, INR, encephalopathy daily]
    E --> F{Day 3-7: Improving?}
    F -->|INR↓, Encephalopathy↓, Bilirubin↓| G[Continue steroids; Taper over 6-12 months]
    F -->|No improvement / Deteriorating| H[STOP steroids; URGENT Transplant Evaluation]
    G --> H
    H --> I[King's College non-PCM criteria / MELD >25 / CLIF-C ACLF]
    I --> J[Liver Transplant = definitive]
```
...
mindmap
  root((AIH-ALF))
    Epidemiology
      ...
      Steroid-responsive!
    Diagnosis
      IAIHG Simplified >=6 probable, >=7 definite
      IgG >2x ULN
      ANA/SMA >=1:80 or LKM1 >=1:40
      Exclude viral/drugs/Wilson
    Types
      Type 1: ANA/SMA
      Type 2: Anti-LKM1 (younger, severe)
      Type 3: Anti-SLA/LP
    Management
      Prednisolone 60mg IV/PO
      Day 7 assessment
      Response: continue & taper
      No response: STOP + Transplant
    Transplant Indications
      King's College non-PCM
      MELD >25
      Steroid failure Day 7
      Contraindication to steroids
    Post-Transplant
      Recurrence 20-30% at 5y
      Tacrolimus + MMF prophylaxis
```
...
1. A patient with suspected Acute Liver Failure presents with: % of non-PCM ALF — 5-10%; Demographic — Women 70%, age 15-50 (bimodal: young & perimenopausal); Precipitants — Infection, drugs, pregnancy, surgery, withdrawal of immunosuppression. What is the most likely diagnosis?
...
> **Auto-generated PasTest/Mediscope-style scenario SBAs** grounded in the authored source. Each scenario tests a real clinical fact (triad, specific sign, contraindication, trial, first-line Rx) extracted from the topic. *Source: Ch 23: Hepatology — Autoimmune hepatitis presenting as ALF*

**Q1.** What is the most appropriate first-line therapy for Autoimmune hepatitis presenting as ALF?

  - **A.** Prednisolone + Assess at Day + Response
  - **B.** An advanced/surgical therapy reserved for refractory disease
  - **C.** Symptomatic treatment only, no disease-modifying therapy
  - **D.** Empiric broad-spectrum therapy without specific indication

  > **Answer: A** — Prednisolone + Assess at Day + Response
  >
  > *Source:* I --> J[Liver Transplant = definitive]
```
