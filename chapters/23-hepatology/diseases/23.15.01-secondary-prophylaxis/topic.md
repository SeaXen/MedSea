...
```mermaid
flowchart TD
    A[Survived Acute Variceal Bleed] --> B[Day 5-7: Start NSBB]
    B --> C[Week 1-2: First EVL Session]
    C --> D[EVL q2-4w until Eradication]
    D --> E[Continue NSBB Lifelong]
    E --> F[Surveillance Endoscopy 6-12 monthly]
    F --> G{Rebleed or Fail Eradication?}
    G -->|Yes| H[TIPS Evaluation]
    G -->|No| I[Continue Combination]
```
...
```mermaid
mindmap
  root((Secondary Prophylaxis))
    Standard of Care
      NSBB + EVL (combination)
      Superior to monotherapy
    NSBB
      Propranolol -> HR 55-60
      Lifelong
    EVL
      q2-4w till eradication
      Eradication = no varices x2 endoscopies
    Surveillance
      Post-eradication: 6-12 monthly
      Recurrence -> resume EVL
    TIPS Indications
      Rebleed on NSBB+EVL
      Failed eradication (4-6 sessions)
      NSBB intolerance
    NSBB Intolerance
      Reduce dose -> Carvedilol -> Stop -> EVL/TIPS
    Gastric Varices
      GOV1: NSBB+EVL+Glue
      GOV2/IGV1: Glue primary
      BRTO/PARTO options
    Post-TIPS
      Doppler US: 1,3,6,12m
      PSV 60-250 cm/s
```
