---
tags: [medicine, dermatology, davidson, fcps, mrcp, disease]
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 29: Dermatology
heading: Hair and Nail Disorders
topic: Trichoscopy
status: full-fcps-mrcp-note
priority: high
created: 2026-06-16
modified: 2026-06-16
exam_relevance: [FCPS, MRCP Part 1, MRCP Part 2, PACES]
---

# Trichoscopy

Related: [[../../../Dermatology MOC]], [[../../../Davidson Chapter 29 - Dermatology Hierarchy]]

> [!info] Davidson alignment: **Chapter 29: Dermatology** (24th Edition, Ibbotson) — Hair and Nail Disorders

## Definition

Trichoscopy (dermoscopy of the scalp/hair) is a non-invasive technique for diagnosing hair and scalp disorders using a handheld dermatoscope. Key features: hair shaft thickness, hair density, follicular openings, peripilar signs, vessel patterns. Used for: AA (yellow dots, black dots, exclamation marks, tapered hairs), AGA (hair diameter diversity, peripilar signs, no yellow dots), LPP (perifollicular erythema, white dots), FFA (peripilar erythema at frontal hairline), TE (empty follicles, short regrowing hairs), trichotillomania (broken hairs, V-sign, trichoptilosis).

## Clinical Features and Presentation

Trichoscopy presents with characteristic features as defined in Davidson 24e Ch 29. The key clinical features are summarised below. The disease is recognised by its morphology, distribution pattern, and course. Severity may be assessed using disease-specific scoring systems where applicable. Always consider the full differential when assessing a patient.

## Investigation and Management Summary

### Investigations
- Clinical diagnosis based on morphology, distribution, and history
- Skin biopsy (H&E) for histological confirmation in atypical or refractory cases
- Disease-specific tests as clinically indicated (autoantibodies, DIF for autoimmune bullous, culture for infection, KOH for fungal)
- Baseline bloods (FBC, LFT, renal) and pre-biologic screening (TB IGRA, hepatitis B/C, HIV) before systemic therapy

### Management
- **First-line:** General measures (emollients, soap substitutes, trigger avoidance) and topical therapy (corticosteroids, calcineurin inhibitors, vitamin D analogues, antifungals as appropriate)
- **Second-line:** Phototherapy (NB-UVB) or conventional systemic agents (methotrexate, ciclosporin, acitretin, azathioprine) for moderate-to-severe disease
- **Third-line:** Biologics or targeted small molecules (anti-TNF, anti-IL17/23, anti-IL4/13, JAK inhibitors) for refractory disease
- **Supportive:** Patient education, psychological support, quality-of-life assessment

## FCPS/MRCP High-Yield Summary

| Category | Key Points |
|----------|------------|
| Technique | Dermoscopy of scalp/hair (×10-70) |
| AA | Yellow dots, black dots, exclamation marks, tapered hairs, short vellus regrowth |
| AGA | Hair diameter diversity (>20%), peripilar signs (brown halos), anisotrichosis, no yellow dots |
| TE | Empty follicles, predominance of single-hair follicular units, short regrowing |
| LPP | Perifollicular erythema, perifollicular scale, white dots (fibrosis), loss of follicular openings |
| FFA | Peripilar erythema at frontal hairline, lonely hair sign |
| Trichotillomania | Broken hairs at different lengths, V-sign, trichoptilosis, no exclamation marks |
| Fibrosing | Loss of follicular openings = scarring (LPP, FFA, DLE, CCCA) |
| Diameter diversity | AGA hallmark (miniaturisation) |
| Yellow dot | Sebum/keratin in empty follicle (AA) |

## Differential Diagnosis

| Differential | Distinguishing Feature |
|--------------|------------------------|
| Different trichoscopic patterns distinguish | See content |

## Viva Questions

1. **Q:** Define Trichoscopy and outline its classification.
   **A:** Trichoscopy (dermoscopy of the scalp/hair) is a non-invasive technique for diagnosing hair and scalp disorders using a handheld dermatoscope. Key features: hair shaft thickness, hair density, follicul

2. **Q:** What are the key clinical features of Trichoscopy?
   **A:** See the high-yield table above.

3. **Q:** Describe the stepwise management of Trichoscopy.
   **A:** Topical → phototherapy → conventional systemic → biologic.

4. **Q:** What scoring system is used for severity assessment?
   **A:** Disease-specific (see high-yield table).

5. **Q:** What are the key complications to monitor?
   **A:** Secondary infection, treatment toxicity, disease-specific complications.

## Confusions and Mnemonics

1. Trichoscopy = non-invasive diagnosis of alopecia

## Special Situations


| Situation | Consideration |
|-----------|---------------|
| **Pregnancy** | Avoid teratogens (retinoids, MTX, MMF) |
| **Paediatric** | Dosing adjustments; growth monitoring |
| **Elderly** | Polypharmacy caution |


## One-Page Revision Card

| **Topic** | **Trichoscopy** |
|-----------|-----------|
| **Definition** | Trichoscopy (dermoscopy of the scalp/hair) is a non-invasive technique for diagnosing hair and scalp |
| **Key Features** | Dermoscopy of scalp/hair (×10-70) |
| **Investigation** | Clinical ± biopsy + disease-specific tests |
| **Management** | Topical → phototherapy → systemic → biologic |
| **Red Flags** | Secondary infection, systemic involvement |
| **Viva Pearl** | See viva section |

## Spaced Repetition Trackers

- [ ] Day 1, 3, 7, 15, 30, 90

## Self-Test Scorecard

| Section | Score /5 |
|---------|----------|
| Understanding | /5 |
| Recall | /5 |
| MCQ Performance | /5 |
| SBA Performance | /5 |
| Viva Confidence | /5 |
| Total | /25 |

> <35 = weak, 35-44 = acceptable, 45+ = strong

## MCQs (5)
1. **Q:** Which of the following is a key feature of Trichoscopy?
   A. Dermoscopy of scalp/hair (×10-70)
   B. Always self-limiting
   C. Caused by bacterial infection only
   D. Restricted to palms and soles only
   E. No investigation needed
   **Answer: A**  *Technique: Dermoscopy of scalp/hair (×10-70)*
2. **Q:** What is the first-line treatment for Trichoscopy?
   A. Topical therapy + general measures
   B. Surgical excision
   C. IVIG
   D. No treatment
   E. Antibiotics
   **Answer: A**  *Topical therapy first-line*
3. **Q:** Which pre-treatment screening is essential before systemic therapy for Trichoscopy?
   A. TB IGRA, hepatitis B/C, HIV, pregnancy test
   B. Pregnancy test only
   C. ECG only
   D. No screening
   E. Eye exam only
   **Answer: A**  *Pre-biologic screening standard*
4. **Q:** What is the most important complication of Trichoscopy?
   A. Secondary infection and disease-specific complications
   B. Malignancy transformation
   C. Renal failure
   D. Cardiac arrest
   E. No complications
   **Answer: A**
5. **Q:** What is the key histological/serological feature of Trichoscopy?
   A. Sebum/keratin in empty follicle (AA)
   B. Always normal histology
   C. Non-specific findings
   D. Only positive in late disease
   E. No test exists
   **Answer: A**

## SBA Questions (5)
1. **Scenario:** Patient with Trichoscopy; first-line investigation?
   A. Clinical diagnosis ± biopsy if atypical  B. Antibiotics  C. CT  D. None  E. Steroids only
   **Answer: A**
2. **Scenario:** Trichoscopy failed topical therapy; next step?
   A. Phototherapy or systemic  B. More topical  C. Observation  D. Surgery  E. Antifungals
   **Answer: A**
3. **Scenario:** Complication of Trichoscopy?
   A. Secondary infection and complications  B. Malignancy  C. Renal failure  D. Cardiac arrest  E. None
   **Answer: A**
4. **Scenario:** Pre-treatment screening before systemic therapy for Trichoscopy?
   A. TB, hep B/C, HIV  B. Pregnancy  C. ECG  D. None  E. Eye exam
   **Answer: A**
5. **Scenario:** Most characteristic histological finding in Trichoscopy?
   A. Sebum/keratin in empty follicle (AA)
   B. Non-specific  C. Always normal  D. Only late  E. No test
   **Answer: A**

## Flashcards

- **Q:** Key fact about Trichoscopy?
  **A:** Dermoscopy of scalp/hair (×10-70)
- **Q:** Another key point?
  **A:** Yellow dots, black dots, exclamation marks, tapered hairs, short vellus regrowth
- **Q:** Most important complication?
  **A:** Hair diameter diversity (>20%), peripilar signs (brown halos), anisotrichosis, no yellow dots
- **Q:** First-line treatment?
  **A:** Empty follicles, predominance of single-hair follicular units, short regrowing
- **Q:** Key histological/serological feature?
  **A:** Perifollicular erythema, perifollicular scale, white dots (fibrosis), loss of follicular openings

## Answer Key with Explanations

### MCQs
1. **A** — Technique: Dermoscopy of scalp/hair (×10-70)
2. **A** — Topical first-line
3. **A** — Pre-biologic screening
4. **A** — Secondary infection most common
5. **A** — Yellow dot: Sebum/keratin in empty follicle (AA)

### SBAs
1. **A** — Clinical + biopsy if atypical
2. **A** — Phototherapy/systemic for moderate-severe
3. **A** — Secondary infection most common
4. **A** — Pre-biologic screening
5. **A** — Sebum/keratin in empty follicle (AA)

## Local Navigation

**Parent Heading Hub:** [[../../Hair and Nail Disorders Hub]]
**Chapter Hierarchy:** [[../../../Davidson Chapter 29 - Dermatology Hierarchy]]
**Chapter MOC:** [[../../../Dermatology MOC]]

---

> [!tip] **Exam focus:** Know definition, key clinical features, first-line management, and the most important complication. Always compare with the main differential.
