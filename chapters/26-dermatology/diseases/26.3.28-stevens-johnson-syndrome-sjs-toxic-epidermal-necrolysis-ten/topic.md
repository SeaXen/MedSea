...
```mermaid
flowchart TD
    A[Acute Mucocutaneous Reaction] --> B{Mucosal Sites ≥2?}
    B -->|Yes| C{BSA Detachment?}
    C -->|<10%| D{Target Lesions?}
    D -->|Yes, classic| E[Erythema Multiforme Major]
    D -->|No, macules→necrosis| F[SJS]
    C -->|10-30%| G[SJS/TEN Overlap]
    C -->|>30%| H[TEN]
    B -->|No/Limited| I{Fever + Eosinophilia + Organ?}
    I -->|Yes| J[DRESS/DIHS]
    I -->|No| K{Acute Pustules + Neutrophilia?}
    K -->|Yes| L[AGEP]
    K -->|No| M[Other: FDE, Fixed Drug Eruption]
```
...
```mermaid
flowchart TD
    A[Suspected SJS/TEN] --> B[STOP ALL NON-ESSENTIAL DRUGS]
    B --> C[ADMIT ICU / BURNS UNIT]
    C --> D[MDT: Derm, ICU, Plastics, Ophthal, Nutrition, ID]
    D --> E[Supportive Care]
    E --> F[Fluids: Urine 0.5-1ml/kg/h]
    E --> G[Wound: Non-adherent dressings]
    E --> H[Nutrition: NG/PEG High Protein]
    E --> I[Eyes: Ophthal DAILY, Lubricants q1-2h]
    E --> J[Pain: Opioids, Avoid NSAIDs]
    E --> K[VTE Prophylaxis: LMWH]
    E --> L[Infection: No Prophylactic ABx]
    F & G & H & I & J & K & L --> M{SCORTEN ≥3 & <72h?}
    M -->|Yes| N[Consider IVIG 2-3g/kg/2-3d OR Ciclosporin 3-5mg/kg]
    M -->|No| O[Supportive Only]
    N & O --> P[Daily SCORTEN Recalculation]
    P --> Q[Rehabilitation / Discharge Planning]
    Q --> R[Long-term Follow-up: Ocular, Skin, Psych]
```
