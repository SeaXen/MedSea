---
tags: [medicine, dermatology, davidson, fcps, mrcp, disease]
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 29: Dermatology
heading: Skin Infections
topic: Scabies
status: full-fcps-mrcp-note
priority: high
created: 2026-06-16
modified: 2026-06-16
exam_relevance: [FCPS, MRCP Part 1, MRCP Part 2, PACES]
---

# Scabies

Related: [[../../../Dermatology MOC]], [[../../../Davidson Chapter 29 - Dermatology Hierarchy]]

> [!info] Davidson alignment: **Chapter 29: Dermatology** (24th Edition, Ibbotson) — Skin Infections

## Definition

Scabies is a contagious skin infestation by the mite Sarcoptes scabiei var. hominis. Intense pruritus (worse at night), burrows (linear, often in web spaces, wrists, ankles, axillae, areolae, genitals). Crusted (Norwegian) scabies in immunocompromised. Transmission: skin-to-skin (prolonged), fomites (less). Treatment: permethrin 5% cream, malathion 0.5%, ivermectin (oral). Treat contacts, wash bedding/clothing.

## Clinical Features and Presentation

Scabies presents with characteristic features as defined in Davidson 24e Ch 29. The key clinical features are summarised below. The disease is recognised by its morphology, distribution pattern, and course. Severity may be assessed using disease-specific scoring systems where applicable. Always consider the full differential when assessing a patient.

## Investigation and Management Summary

### Investigations
- Clinical diagnosis based on morphology, distribution, and history
- Skin biopsy (H&E) for histological confirmation in atypical or refractory cases
- Disease-specific tests as clinically indicated (autoantibodies, DIF for autoimmune bullous, culture for infection, KOH for fungal)
- Baseline bloods (FBC, LFT, renal) and pre-biologic screening (TB IGRA, hepatitis B/C, HIV) before systemic therapy

### Management
- **First-line:** General measures (emollients, soap substitutes, trigger avoidance) and topical therapy (corticosteroids, calcineurin inhibitors, vitamin D analogues, antifungals as appropriate)
- **Second-line:** Phototherapy (NB-UVB) or conventional systemic agents (methotrexate, ciclosporin, acitretin, azathioprine) for moderate-to-severe disease
- **Third-line:** Biologics or targeted small molecules (anti-TNF, anti-IL17/23, anti-IL4/13, JAK inhibitors) for refractory disease
- **Supportive:** Patient education, psychological support, quality-of-life assessment

## FCPS/MRCP High-Yield Summary

| Category | Key Points |
|----------|------------|
| Cause | Sarcoptes scabiei var. hominis (itch mite) |
| Burrow | Linear, curved, 1-2mm, often in web spaces, wrists, axillae, areolae, genitals |
| Pruritus | Intense, worse at night |
| Distribution | Web spaces, wrists, elbows, axillae, areolae, genitals, buttocks |
| Face | Spared in adults (involved in infants) |
| Differential | Atopic dermatitis, prurigo, papular urticaria, dermatitis herpetiformis |
| Diagnosis | Clinical; skin scraping microscopy (mite, eggs, faeces) |
| Crusted (Norwegian) | Immunocompromised; thousands of mites; hyperkeratotic plaques |
| Treatment | Permethrin 5% cream (whole body, leave 8-12h, repeat 7d), malathion 0.5%, ivermectin 200µg/kg PO single dose |
| Contacts | Treat all close contacts; wash bedding/clothing at 60°C |
| Complications | Secondary bacterial infection, post-strep GN, eczematisation |

## Differential Diagnosis

| Differential | Distinguishing Feature |
|--------------|------------------------|
| Atopic dermatitis | Flexural, chronic, no burrows |

## Viva Questions

1. **Q:** Define Scabies and outline its classification.
   **A:** Scabies is a contagious skin infestation by the mite Sarcoptes scabiei var. hominis. Intense pruritus (worse at night), burrows (linear, often in web spaces, wrists, ankles, axillae, areolae, genitals

2. **Q:** What are the key clinical features of Scabies?
   **A:** See the high-yield table above.

3. **Q:** Describe the stepwise management of Scabies.
   **A:** Topical → phototherapy → conventional systemic → biologic.

4. **Q:** What scoring system is used for severity assessment?
   **A:** Disease-specific (see high-yield table).

5. **Q:** What are the key complications to monitor?
   **A:** Secondary infection, treatment toxicity, disease-specific complications.

## Confusions and Mnemonics

1. **Scabies** = Burrows in web spaces; night itch; treat contacts

## Special Situations


| Situation | Consideration |
|-----------|---------------|
| **Pregnancy** | Avoid teratogens (retinoids, MTX, MMF) |
| **Paediatric** | Dosing adjustments; growth monitoring |
| **Elderly** | Polypharmacy caution |


## One-Page Revision Card

| **Topic** | **Scabies** |
|-----------|-----------|
| **Definition** | Scabies is a contagious skin infestation by the mite Sarcoptes scabiei var. hominis. Intense pruritu |
| **Key Features** | Sarcoptes scabiei var. hominis (itch mite) |
| **Investigation** | Clinical ± biopsy + disease-specific tests |
| **Management** | Topical → phototherapy → systemic → biologic |
| **Red Flags** | Secondary infection, systemic involvement |
| **Viva Pearl** | See viva section |

## Spaced Repetition Trackers

- [ ] Day 1, 3, 7, 15, 30, 90

## Self-Test Scorecard

| Section | Score /5 |
|---------|----------|
| Understanding | /5 |
| Recall | /5 |
| MCQ Performance | /5 |
| SBA Performance | /5 |
| Viva Confidence | /5 |
| Total | /25 |

> <35 = weak, 35-44 = acceptable, 45+ = strong

## MCQs (5)
1. **Q:** Which of the following is a key feature of Scabies?
   A. Sarcoptes scabiei var. hominis (itch mite)
   B. Always self-limiting
   C. Caused by bacterial infection only
   D. Restricted to palms and soles only
   E. No investigation needed
   **Answer: A**  *Cause: Sarcoptes scabiei var. hominis (itch mite)*
2. **Q:** What is the first-line treatment for Scabies?
   A. Topical therapy + general measures
   B. Surgical excision
   C. IVIG
   D. No treatment
   E. Antibiotics
   **Answer: A**  *Topical therapy first-line*
3. **Q:** Which pre-treatment screening is essential before systemic therapy for Scabies?
   A. TB IGRA, hepatitis B/C, HIV, pregnancy test
   B. Pregnancy test only
   C. ECG only
   D. No screening
   E. Eye exam only
   **Answer: A**  *Pre-biologic screening standard*
4. **Q:** What is the most important complication of Scabies?
   A. Secondary infection and disease-specific complications
   B. Malignancy transformation
   C. Renal failure
   D. Cardiac arrest
   E. No complications
   **Answer: A**
5. **Q:** What is the key histological/serological feature of Scabies?
   A. Secondary bacterial infection, post-strep GN, eczematisation
   B. Always normal histology
   C. Non-specific findings
   D. Only positive in late disease
   E. No test exists
   **Answer: A**

## SBA Questions (5)
1. **Scenario:** Patient with Scabies; first-line investigation?
   A. Clinical diagnosis ± biopsy if atypical  B. Antibiotics  C. CT  D. None  E. Steroids only
   **Answer: A**
2. **Scenario:** Scabies failed topical therapy; next step?
   A. Phototherapy or systemic  B. More topical  C. Observation  D. Surgery  E. Antifungals
   **Answer: A**
3. **Scenario:** Complication of Scabies?
   A. Secondary infection and complications  B. Malignancy  C. Renal failure  D. Cardiac arrest  E. None
   **Answer: A**
4. **Scenario:** Pre-treatment screening before systemic therapy for Scabies?
   A. TB, hep B/C, HIV  B. Pregnancy  C. ECG  D. None  E. Eye exam
   **Answer: A**
5. **Scenario:** Most characteristic histological finding in Scabies?
   A. Secondary bacterial infection, post-strep GN, eczematisation
   B. Non-specific  C. Always normal  D. Only late  E. No test
   **Answer: A**

## Flashcards

- **Q:** Key fact about Scabies?
  **A:** Sarcoptes scabiei var. hominis (itch mite)
- **Q:** Another key point?
  **A:** Linear, curved, 1-2mm, often in web spaces, wrists, axillae, areolae, genitals
- **Q:** Most important complication?
  **A:** Intense, worse at night
- **Q:** First-line treatment?
  **A:** Web spaces, wrists, elbows, axillae, areolae, genitals, buttocks
- **Q:** Key histological/serological feature?
  **A:** Spared in adults (involved in infants)

## Answer Key with Explanations

### MCQs
1. **A** — Cause: Sarcoptes scabiei var. hominis (itch mite)
2. **A** — Topical first-line
3. **A** — Pre-biologic screening
4. **A** — Secondary infection most common
5. **A** — Complications: Secondary bacterial infection, post-strep GN, eczematisation

### SBAs
1. **A** — Clinical + biopsy if atypical
2. **A** — Phototherapy/systemic for moderate-severe
3. **A** — Secondary infection most common
4. **A** — Pre-biologic screening
5. **A** — Secondary bacterial infection, post-strep GN, eczematisation

## Local Navigation

**Parent Heading Hub:** [[../../Skin Infections Hub]]
**Chapter Hierarchy:** [[../../../Davidson Chapter 29 - Dermatology Hierarchy]]
**Chapter MOC:** [[../../../Dermatology MOC]]

---

> [!tip] **Exam focus:** Know definition, key clinical features, first-line management, and the most important complication. Always compare with the main differential.

## PasTest Scenario SBAs (Clinical Vignettes)

> **Auto-generated PasTest/Mediscope-style scenario SBAs** grounded in the authored source. Each scenario tests a real clinical fact (triad, specific sign, contraindication, trial, first-line Rx) extracted from the topic. *Source: Ch 26: Dermatology — Scabies full*

**Q1.** What is the most appropriate first-line therapy for Scabies full?

  - **A.** First-line:
  - **B.** An advanced/surgical therapy reserved for refractory disease
  - **C.** Symptomatic treatment only, no disease-modifying therapy
  - **D.** Empiric broad-spectrum therapy without specific indication

  > **Answer: A** — First-line:
  >
  > *Source:* **First-line:** General measures (emollients, soap substitutes, trigger avoidance) and topical therapy (corticosteroids, calcineurin inhibitors, vitamin D analogues, antifungals as appropriate)

