---
tags: [medicine, dermatology, davidson, fcps, mrcp, disease]
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 29: Dermatology
heading: Skin Tumours
topic: Atypical fibroxanthoma
status: full-fcps-mrcp-note
priority: high
created: 2026-06-16
modified: 2026-06-16
exam_relevance: [FCPS, MRCP Part 1, MRCP Part 2, PACES]
---

# Atypical fibroxanthoma

Related: [[../../../Dermatology MOC]], [[../../../Davidson Chapter 29 - Dermatology Hierarchy]]

> [!info] Davidson alignment: **Chapter 29: Dermatology** (24th Edition, Ibbotson) — Skin Tumours

## Definition

Atypical fibroxanthoma (AFX) is a low-grade cutaneous sarcoma of sun-damaged skin in elderly, often considered a superficial variant of pleomorphic dermal sarcoma. Presents as rapidly growing red/pink nodule on sun-exposed sites (head, neck, ears). Histology: pleomorphic spindle and epithelioid cells, atypical mitoses, bizarre giant cells. A diagnosis of exclusion (no specific markers; rule out melanoma, SCC, BCC, sarcoma by IHC). Treatment: surgical excision (Mohs preferred).

## Clinical Features and Presentation

Atypical fibroxanthoma presents with characteristic features as defined in Davidson 24e Ch 29. The key clinical features are summarised below. The disease is recognised by its morphology, distribution pattern, and course. Severity may be assessed using disease-specific scoring systems where applicable. Always consider the full differential when assessing a patient.

## Investigation and Management Summary

### Investigations
- Clinical diagnosis based on morphology, distribution, and history
- Skin biopsy (H&E) for histological confirmation in atypical or refractory cases
- Disease-specific tests as clinically indicated (autoantibodies, DIF for autoimmune bullous, culture for infection, KOH for fungal)
- Baseline bloods (FBC, LFT, renal) and pre-biologic screening (TB IGRA, hepatitis B/C, HIV) before systemic therapy

### Management
- **First-line:** General measures (emollients, soap substitutes, trigger avoidance) and topical therapy (corticosteroids, calcineurin inhibitors, vitamin D analogues, antifungals as appropriate)
- **Second-line:** Phototherapy (NB-UVB) or conventional systemic agents (methotrexate, ciclosporin, acitretin, azathioprine) for moderate-to-severe disease
- **Third-line:** Biologics or targeted small molecules (anti-TNF, anti-IL17/23, anti-IL4/13, JAK inhibitors) for refractory disease
- **Supportive:** Patient education, psychological support, quality-of-life assessment

## FCPS/MRCP High-Yield Summary

| Category | Key Points |
|----------|------------|
| Type | Low-grade cutaneous sarcoma (diagnosis of exclusion) |
| Site | Sun-exposed: head, neck, ears, bald scalp |
| Age | Elderly (80% in >70) |
| Appearance | Rapidly growing pink/red nodule, often ulcerated |
| Histology | Pleomorphic spindle + epithelioid cells, atypical mitoses, bizarre giant cells |
| IHC | CD10+, vimentin+, CD68+, CD99+; S100- (excludes melanoma); cytokeratin- (excludes SCC) |
| Differential | Melanoma (S100+, HMB45+), SCC (CK+), BCC (BerEP4+), angiosarcoma, sarcoma |
| Treatment | Surgical excision; Mohs preferred; wide local excision alternative |
| Prognosis | Good if completely excised; rare metastasis; local recurrence ~5-10% |
| Pleomorphic dermal sarcoma | Deep/infiltrative variant of AFX |

## Differential Diagnosis

| Differential | Distinguishing Feature |
|--------------|------------------------|
| Melanoma | S100+, HMB45+ |

## Viva Questions

1. **Q:** Define Atypical fibroxanthoma and outline its classification.
   **A:** Atypical fibroxanthoma (AFX) is a low-grade cutaneous sarcoma of sun-damaged skin in elderly, often considered a superficial variant of pleomorphic dermal sarcoma. Presents as rapidly growing red/pink

2. **Q:** What are the key clinical features of Atypical fibroxanthoma?
   **A:** See the high-yield table above.

3. **Q:** Describe the stepwise management of Atypical fibroxanthoma.
   **A:** Topical → phototherapy → conventional systemic → biologic.

4. **Q:** What scoring system is used for severity assessment?
   **A:** Disease-specific (see high-yield table).

5. **Q:** What are the key complications to monitor?
   **A:** Secondary infection, treatment toxicity, disease-specific complications.

## Confusions and Mnemonics

1. **AFX** = Diagnosis of exclusion; pleomorphic; CD10+ CD68+; sun-damaged

## Special Situations


| Situation | Consideration |
|-----------|---------------|
| **Pregnancy** | Avoid teratogens (retinoids, MTX, MMF) |
| **Paediatric** | Dosing adjustments; growth monitoring |
| **Elderly** | Polypharmacy caution |


## One-Page Revision Card

| **Topic** | **Atypical fibroxanthoma** |
|-----------|-----------|
| **Definition** | Atypical fibroxanthoma (AFX) is a low-grade cutaneous sarcoma of sun-damaged skin in elderly, often  |
| **Key Features** | Low-grade cutaneous sarcoma (diagnosis of exclusion) |
| **Investigation** | Clinical ± biopsy + disease-specific tests |
| **Management** | Topical → phototherapy → systemic → biologic |
| **Red Flags** | Secondary infection, systemic involvement |
| **Viva Pearl** | See viva section |

## Spaced Repetition Trackers

- [ ] Day 1, 3, 7, 15, 30, 90

## Self-Test Scorecard

| Section | Score /5 |
|---------|----------|
| Understanding | /5 |
| Recall | /5 |
| MCQ Performance | /5 |
| SBA Performance | /5 |
| Viva Confidence | /5 |
| Total | /25 |

> <35 = weak, 35-44 = acceptable, 45+ = strong

## MCQs (5)
1. **Q:** Which of the following is a key feature of Atypical fibroxanthoma?
   A. Low-grade cutaneous sarcoma (diagnosis of exclusion)
   B. Always self-limiting
   C. Caused by bacterial infection only
   D. Restricted to palms and soles only
   E. No investigation needed
   **Answer: A**  *Type: Low-grade cutaneous sarcoma (diagnosis of exclusion)*
2. **Q:** What is the first-line treatment for Atypical fibroxanthoma?
   A. Topical therapy + general measures
   B. Surgical excision
   C. IVIG
   D. No treatment
   E. Antibiotics
   **Answer: A**  *Topical therapy first-line*
3. **Q:** Which pre-treatment screening is essential before systemic therapy for Atypical fibroxanthoma?
   A. TB IGRA, hepatitis B/C, HIV, pregnancy test
   B. Pregnancy test only
   C. ECG only
   D. No screening
   E. Eye exam only
   **Answer: A**  *Pre-biologic screening standard*
4. **Q:** What is the most important complication of Atypical fibroxanthoma?
   A. Secondary infection and disease-specific complications
   B. Malignancy transformation
   C. Renal failure
   D. Cardiac arrest
   E. No complications
   **Answer: A**
5. **Q:** What is the key histological/serological feature of Atypical fibroxanthoma?
   A. Deep/infiltrative variant of AFX
   B. Always normal histology
   C. Non-specific findings
   D. Only positive in late disease
   E. No test exists
   **Answer: A**

## SBA Questions (5)
1. **Scenario:** Patient with Atypical fibroxanthoma; first-line investigation?
   A. Clinical diagnosis ± biopsy if atypical  B. Antibiotics  C. CT  D. None  E. Steroids only
   **Answer: A**
2. **Scenario:** Atypical fibroxanthoma failed topical therapy; next step?
   A. Phototherapy or systemic  B. More topical  C. Observation  D. Surgery  E. Antifungals
   **Answer: A**
3. **Scenario:** Complication of Atypical fibroxanthoma?
   A. Secondary infection and complications  B. Malignancy  C. Renal failure  D. Cardiac arrest  E. None
   **Answer: A**
4. **Scenario:** Pre-treatment screening before systemic therapy for Atypical fibroxanthoma?
   A. TB, hep B/C, HIV  B. Pregnancy  C. ECG  D. None  E. Eye exam
   **Answer: A**
5. **Scenario:** Most characteristic histological finding in Atypical fibroxanthoma?
   A. Deep/infiltrative variant of AFX
   B. Non-specific  C. Always normal  D. Only late  E. No test
   **Answer: A**

## Flashcards

- **Q:** Key fact about Atypical fibroxanthoma?
  **A:** Low-grade cutaneous sarcoma (diagnosis of exclusion)
- **Q:** Another key point?
  **A:** Sun-exposed: head, neck, ears, bald scalp
- **Q:** Most important complication?
  **A:** Elderly (80% in >70)
- **Q:** First-line treatment?
  **A:** Rapidly growing pink/red nodule, often ulcerated
- **Q:** Key histological/serological feature?
  **A:** Pleomorphic spindle + epithelioid cells, atypical mitoses, bizarre giant cells

## Answer Key with Explanations

### MCQs
1. **A** — Type: Low-grade cutaneous sarcoma (diagnosis of exclusion)
2. **A** — Topical first-line
3. **A** — Pre-biologic screening
4. **A** — Secondary infection most common
5. **A** — Pleomorphic dermal sarcoma: Deep/infiltrative variant of AFX

### SBAs
1. **A** — Clinical + biopsy if atypical
2. **A** — Phototherapy/systemic for moderate-severe
3. **A** — Secondary infection most common
4. **A** — Pre-biologic screening
5. **A** — Deep/infiltrative variant of AFX

## Local Navigation

**Parent Heading Hub:** [[../../Skin Tumours Hub]]
**Chapter Hierarchy:** [[../../../Davidson Chapter 29 - Dermatology Hierarchy]]
**Chapter MOC:** [[../../../Dermatology MOC]]

---

> [!tip] **Exam focus:** Know definition, key clinical features, first-line management, and the most important complication. Always compare with the main differential.

## PasTest Scenario SBAs (Clinical Vignettes)

> **Auto-generated PasTest/Mediscope-style scenario SBAs** grounded in the authored source. Each scenario tests a real clinical fact (triad, specific sign, contraindication, trial, first-line Rx) extracted from the topic. *Source: Ch 26: Dermatology — Atypical fibroxanthoma full*

**Q1.** What is the most appropriate first-line therapy for Atypical fibroxanthoma full?

  - **A.** First-line:
  - **B.** An advanced/surgical therapy reserved for refractory disease
  - **C.** Symptomatic treatment only, no disease-modifying therapy
  - **D.** Empiric broad-spectrum therapy without specific indication

  > **Answer: A** — First-line:
  >
  > *Source:* **First-line:** General measures (emollients, soap substitutes, trigger avoidance) and topical therapy (corticosteroids, calcineurin inhibitors, vitamin D analogues, antifungals as appropriate)

