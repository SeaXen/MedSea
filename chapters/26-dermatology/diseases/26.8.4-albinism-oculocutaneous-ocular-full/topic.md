---
tags: [medicine, dermatology, davidson, fcps, mrcp, disease]
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 29: Dermatology
heading: Disorders of Pigmentation
topic: Albinism (oculocutaneous, ocular)
status: full-fcps-mrcp-note
priority: high
created: 2026-06-16
modified: 2026-06-16
exam_relevance: [FCPS, MRCP Part 1, MRCP Part 2, PACES]
---

# Albinism (oculocutaneous, ocular)

Related: [[../../../Dermatology MOC]], [[../../../Davidson Chapter 29 - Dermatology Hierarchy]]

> [!info] Davidson alignment: **Chapter 29: Dermatology** (24th Edition, Ibbotson) — Disorders of Pigmentation

## Definition

Albinism encompasses a group of autosomal recessive disorders with reduced or absent melanin synthesis. Oculocutaneous albinism (OCA): generalised hypopigmentation of skin, hair, eyes; 7 types (OCA1-7). Ocular albinism (OA1): X-linked, eye only. Features: hypopigmented skin/hair, nystagmus, photophobia, foveal hypoplasia, reduced visual acuity. Major concern: sun protection (skin cancer risk) and visual support.

## Clinical Features and Presentation

Albinism (oculocutaneous, ocular) presents with characteristic features as defined in Davidson 24e Ch 29. The key clinical features are summarised below. The disease is recognised by its morphology, distribution pattern, and course. Severity may be assessed using disease-specific scoring systems where applicable. Always consider the full differential when assessing a patient.

## Investigation and Management Summary

### Investigations
- Clinical diagnosis based on morphology, distribution, and history
- Skin biopsy (H&E) for histological confirmation in atypical or refractory cases
- Disease-specific tests as clinically indicated (autoantibodies, DIF for autoimmune bullous, culture for infection, KOH for fungal)
- Baseline bloods (FBC, LFT, renal) and pre-biologic screening (TB IGRA, hepatitis B/C, HIV) before systemic therapy

### Management
- **First-line:** General measures (emollients, soap substitutes, trigger avoidance) and topical therapy (corticosteroids, calcineurin inhibitors, vitamin D analogues, antifungals as appropriate)
- **Second-line:** Phototherapy (NB-UVB) or conventional systemic agents (methotrexate, ciclosporin, acitretin, azathioprine) for moderate-to-severe disease
- **Third-line:** Biologics or targeted small molecules (anti-TNF, anti-IL17/23, anti-IL4/13, JAK inhibitors) for refractory disease
- **Supportive:** Patient education, psychological support, quality-of-life assessment

## FCPS/MRCP High-Yield Summary

| Category | Key Points |
|----------|------------|
| OCA types | OCA1 (tyrosinase negative), OCA2 (P gene), OCA3 (TYRP1), OCA4 (SLC45A2), OCA5, OCA6, OCA7 |
| Inheritance | Autosomal recessive (OA1: X-linked) |
| Skin | Hypopigmented, milky white, pink undertones; severe sunburn |
| Hair | White to yellow to brown (depending on type) |
| Eyes | Nystagmus, photophobia, foveal hypoplasia, iris transillumination, reduced VA |
| Skin cancer | 1000x risk; SCC, BCC, melanoma from young age |
| Hermansky-Pudlak | OCA + bleeding diathesis + pulmonary fibrosis |
| Chediak-Higashi | OCA + immunodeficiency + neurological |
| Treatment | Strict photoprotection, skin exams, visual support, genetic counselling |
| Differential | Vitiligo, piebaldism, Waardenburg |

## Differential Diagnosis

| Differential | Distinguishing Feature |
|--------------|------------------------|
| Vitiligo | Acquired, depigmented patches, normal vision |

## Viva Questions

1. **Q:** Define Albinism (oculocutaneous, ocular) and outline its classification.
   **A:** Albinism encompasses a group of autosomal recessive disorders with reduced or absent melanin synthesis. Oculocutaneous albinism (OCA): generalised hypopigmentation of skin, hair, eyes; 7 types (OCA1-7

2. **Q:** What are the key clinical features of Albinism (oculocutaneous, ocular)?
   **A:** See the high-yield table above.

3. **Q:** Describe the stepwise management of Albinism (oculocutaneous, ocular).
   **A:** Topical → phototherapy → conventional systemic → biologic.

4. **Q:** What scoring system is used for severity assessment?
   **A:** Disease-specific (see high-yield table).

5. **Q:** What are the key complications to monitor?
   **A:** Secondary infection, treatment toxicity, disease-specific complications.

## Confusions and Mnemonics

1. OCA = nystagmus + foveal hypoplasia + skin cancer risk

## Special Situations


| Situation | Consideration |
|-----------|---------------|
| **Pregnancy** | Avoid teratogens (retinoids, MTX, MMF) |
| **Paediatric** | Dosing adjustments; growth monitoring |
| **Elderly** | Polypharmacy caution |


## One-Page Revision Card

| **Topic** | **Albinism (oculocutaneous, ocular)** |
|-----------|-----------|
| **Definition** | Albinism encompasses a group of autosomal recessive disorders with reduced or absent melanin synthes |
| **Key Features** | OCA1 (tyrosinase negative), OCA2 (P gene), OCA3 (TYRP1), OCA4 (SLC45A2), OCA5, OCA6, OCA7 |
| **Investigation** | Clinical ± biopsy + disease-specific tests |
| **Management** | Topical → phototherapy → systemic → biologic |
| **Red Flags** | Secondary infection, systemic involvement |
| **Viva Pearl** | See viva section |

## Spaced Repetition Trackers

- [ ] Day 1, 3, 7, 15, 30, 90

## Self-Test Scorecard

| Section | Score /5 |
|---------|----------|
| Understanding | /5 |
| Recall | /5 |
| MCQ Performance | /5 |
| SBA Performance | /5 |
| Viva Confidence | /5 |
| Total | /25 |

> <35 = weak, 35-44 = acceptable, 45+ = strong

## MCQs (5)
1. **Q:** Which of the following is a key feature of Albinism (oculocutaneous, ocular)?
   A. OCA1 (tyrosinase negative), OCA2 (P gene), OCA3 (TYRP1), OCA4 (SLC45A2), OCA5, OCA6, OCA7
   B. Always self-limiting
   C. Caused by bacterial infection only
   D. Restricted to palms and soles only
   E. No investigation needed
   **Answer: A**  *OCA types: OCA1 (tyrosinase negative), OCA2 (P gene), OCA3 (TYRP1), OCA4 (SLC45A2), OCA5, OCA6, OCA7*
2. **Q:** What is the first-line treatment for Albinism (oculocutaneous, ocular)?
   A. Topical therapy + general measures
   B. Surgical excision
   C. IVIG
   D. No treatment
   E. Antibiotics
   **Answer: A**  *Topical therapy first-line*
3. **Q:** Which pre-treatment screening is essential before systemic therapy for Albinism (oculocutaneous, ocular)?
   A. TB IGRA, hepatitis B/C, HIV, pregnancy test
   B. Pregnancy test only
   C. ECG only
   D. No screening
   E. Eye exam only
   **Answer: A**  *Pre-biologic screening standard*
4. **Q:** What is the most important complication of Albinism (oculocutaneous, ocular)?
   A. Secondary infection and disease-specific complications
   B. Malignancy transformation
   C. Renal failure
   D. Cardiac arrest
   E. No complications
   **Answer: A**
5. **Q:** What is the key histological/serological feature of Albinism (oculocutaneous, ocular)?
   A. Vitiligo, piebaldism, Waardenburg
   B. Always normal histology
   C. Non-specific findings
   D. Only positive in late disease
   E. No test exists
   **Answer: A**

## SBA Questions (5)
1. **Scenario:** Patient with Albinism (oculocutaneous, ocular); first-line investigation?
   A. Clinical diagnosis ± biopsy if atypical  B. Antibiotics  C. CT  D. None  E. Steroids only
   **Answer: A**
2. **Scenario:** Albinism (oculocutaneous, ocular) failed topical therapy; next step?
   A. Phototherapy or systemic  B. More topical  C. Observation  D. Surgery  E. Antifungals
   **Answer: A**
3. **Scenario:** Complication of Albinism (oculocutaneous, ocular)?
   A. Secondary infection and complications  B. Malignancy  C. Renal failure  D. Cardiac arrest  E. None
   **Answer: A**
4. **Scenario:** Pre-treatment screening before systemic therapy for Albinism (oculocutaneous, ocular)?
   A. TB, hep B/C, HIV  B. Pregnancy  C. ECG  D. None  E. Eye exam
   **Answer: A**
5. **Scenario:** Most characteristic histological finding in Albinism (oculocutaneous, ocular)?
   A. Vitiligo, piebaldism, Waardenburg
   B. Non-specific  C. Always normal  D. Only late  E. No test
   **Answer: A**

## Flashcards

- **Q:** Key fact about Albinism (oculocutaneous, ocular)?
  **A:** OCA1 (tyrosinase negative), OCA2 (P gene), OCA3 (TYRP1), OCA4 (SLC45A2), OCA5, OCA6, OCA7
- **Q:** Another key point?
  **A:** Autosomal recessive (OA1: X-linked)
- **Q:** Most important complication?
  **A:** Hypopigmented, milky white, pink undertones; severe sunburn
- **Q:** First-line treatment?
  **A:** White to yellow to brown (depending on type)
- **Q:** Key histological/serological feature?
  **A:** Nystagmus, photophobia, foveal hypoplasia, iris transillumination, reduced VA

## Answer Key with Explanations

### MCQs
1. **A** — OCA types: OCA1 (tyrosinase negative), OCA2 (P gene), OCA3 (TYRP1), OCA4 (SLC45A2), OCA5, OCA6, OCA7
2. **A** — Topical first-line
3. **A** — Pre-biologic screening
4. **A** — Secondary infection most common
5. **A** — Differential: Vitiligo, piebaldism, Waardenburg

### SBAs
1. **A** — Clinical + biopsy if atypical
2. **A** — Phototherapy/systemic for moderate-severe
3. **A** — Secondary infection most common
4. **A** — Pre-biologic screening
5. **A** — Vitiligo, piebaldism, Waardenburg

## Local Navigation

**Parent Heading Hub:** [[../../Disorders of Pigmentation Hub]]
**Chapter Hierarchy:** [[../../../Davidson Chapter 29 - Dermatology Hierarchy]]
**Chapter MOC:** [[../../../Dermatology MOC]]

---

> [!tip] **Exam focus:** Know definition, key clinical features, first-line management, and the most important complication. Always compare with the main differential.
