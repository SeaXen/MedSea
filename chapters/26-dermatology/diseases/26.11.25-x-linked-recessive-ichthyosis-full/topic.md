---
tags: [medicine, dermatology, davidson, fcps, mrcp, disease]
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 29: Dermatology
heading: Genodermatoses
topic: X-linked recessive ichthyosis
status: full-fcps-mrcp-note
priority: high
created: 2026-06-16
modified: 2026-06-16
exam_relevance: [FCPS, MRCP Part 1, MRCP Part 2, PACES]
---

# X-linked recessive ichthyosis

Related: [[../../../Dermatology MOC]], [[../../../Davidson Chapter 29 - Dermatology Hierarchy]]

> [!info] Davidson alignment: **Chapter 29: Dermatology** (24th Edition, Ibbotson) — Genodermatoses

## Definition

X-linked recessive ichthyosis (XLRI) is a disorder due to steroid sulfatase (STS) deficiency, affecting 1 in 2000-6000 males. Onset at birth (or soon after), with large dark brown adherent scales (extensor surfaces, trunk, neck, face, pre-auricular), often involving palms/soles, neck (often dirty neck appearance). Associated: cryptorchidism, corneal opacities, prolonged labour (maternal STS deficiency), attention deficits. Treatment: emollients, urea, salicylic acid, topical retinoids, vitamin D analogues.

## Clinical Features and Presentation

X-linked recessive ichthyosis presents with characteristic features as defined in Davidson 24e Ch 29. The key clinical features are summarised below. The disease is recognised by its morphology, distribution pattern, and course. Severity may be assessed using disease-specific scoring systems where applicable. Always consider the full differential when assessing a patient.

## Investigation and Management Summary

### Investigations
- Clinical diagnosis based on morphology, distribution, and history
- Skin biopsy (H&E) for histological confirmation in atypical or refractory cases
- Disease-specific tests as clinically indicated (autoantibodies, DIF for autoimmune bullous, culture for infection, KOH for fungal)
- Baseline bloods (FBC, LFT, renal) and pre-biologic screening (TB IGRA, hepatitis B/C, HIV) before systemic therapy

### Management
- **First-line:** General measures (emollients, soap substitutes, trigger avoidance) and topical therapy (corticosteroids, calcineurin inhibitors, vitamin D analogues, antifungals as appropriate)
- **Second-line:** Phototherapy (NB-UVB) or conventional systemic agents (methotrexate, ciclosporin, acitretin, azathioprine) for moderate-to-severe disease
- **Third-line:** Biologics or targeted small molecules (anti-TNF, anti-IL17/23, anti-IL4/13, JAK inhibitors) for refractory disease
- **Supportive:** Patient education, psychological support, quality-of-life assessment

## FCPS/MRCP High-Yield Summary

| Category | Key Points |
|----------|------------|
| Inheritance | X-linked recessive (males) |
| Gene | STS (steroid sulfatase) |
| Prevalence | 1 in 2000-6000 males |
| Scale | Large dark brown adherent |
| Distribution | Extensor, trunk, neck, face, pre-auricular, palms/soles |
| Onset | At birth (collodion baby sometimes) |
| Associations | Cryptorchidism (20%), corneal opacities (dot/foil), prolonged labour, attention deficits |
| Diagnosis | STS activity (low/absent in fibroblasts, leukocytes); elevated cholesterol sulfate in serum/plasma |
| Treatment | Emollients, urea, salicylic acid, topical retinoids, vitamin D analogues |
| Pregnancy | Maternal STS deficiency: delayed/prolonged labour |
| Differential | Ichthyosis vulgaris, other ichthyoses |

## Differential Diagnosis

| Differential | Distinguishing Feature |
|--------------|------------------------|
| Ichthyosis vulgaris | FLG, AD, fine scale, palms/soles spared |

## Viva Questions

1. **Q:** Define X-linked recessive ichthyosis and outline its classification.
   **A:** X-linked recessive ichthyosis (XLRI) is a disorder due to steroid sulfatase (STS) deficiency, affecting 1 in 2000-6000 males. Onset at birth (or soon after), with large dark brown adherent scales (ext

2. **Q:** What are the key clinical features of X-linked recessive ichthyosis?
   **A:** See the high-yield table above.

3. **Q:** Describe the stepwise management of X-linked recessive ichthyosis.
   **A:** Topical → phototherapy → conventional systemic → biologic.

4. **Q:** What scoring system is used for severity assessment?
   **A:** Disease-specific (see high-yield table).

5. **Q:** What are the key complications to monitor?
   **A:** Secondary infection, treatment toxicity, disease-specific complications.

## Confusions and Mnemonics

- See clinical features

## Special Situations


| Situation | Consideration |
|-----------|---------------|
| **Pregnancy** | Avoid teratogens (retinoids, MTX, MMF) |


## One-Page Revision Card

| **Topic** | **X-linked recessive ichthyosis** |
|-----------|-----------|
| **Definition** | X-linked recessive ichthyosis (XLRI) is a disorder due to steroid sulfatase (STS) deficiency, affect |
| **Key Features** | X-linked recessive (males) |
| **Investigation** | Clinical ± biopsy + disease-specific tests |
| **Management** | Topical → phototherapy → systemic → biologic |
| **Red Flags** | Secondary infection, systemic involvement |
| **Viva Pearl** | See viva section |

## Spaced Repetition Trackers

- [ ] Day 1, 3, 7, 15, 30, 90

## Self-Test Scorecard

| Section | Score /5 |
|---------|----------|
| Understanding | /5 |
| Recall | /5 |
| MCQ Performance | /5 |
| SBA Performance | /5 |
| Viva Confidence | /5 |
| Total | /25 |

> <35 = weak, 35-44 = acceptable, 45+ = strong

## MCQs (5)
1. **Q:** Which of the following is a key feature of X-linked recessive ichthyosis?
   A. X-linked recessive (males)
   B. Always self-limiting
   C. Caused by bacterial infection only
   D. Restricted to palms and soles only
   E. No investigation needed
   **Answer: A**
2. **Q:** What is the first-line treatment for X-linked recessive ichthyosis?
   A. Topical therapy + general measures
   B. Surgical excision
   C. IVIG
   D. No treatment
   E. Antibiotics
   **Answer: A**
3. **Q:** Which pre-treatment screening is essential before systemic therapy?
   A. TB IGRA, hepatitis B/C, HIV, pregnancy test
   B. Pregnancy test only
   C. ECG only
   D. No screening
   E. Eye exam only
   **Answer: A**
4. **Q:** What is the most important complication of X-linked recessive ichthyosis?
   A. Secondary infection and disease-specific complications
   B. Malignancy transformation
   C. Renal failure
   D. Cardiac arrest
   E. No complications
   **Answer: A**
5. **Q:** What is the key histological/serological feature?
   A. Ichthyosis vulgaris, other ichthyoses
   B. Always normal
   C. Non-specific
   D. Only late
   E. No test
   **Answer: A**

## SBA Questions (5)
1. **Scenario:** Patient with X-linked recessive ichthyosis; first-line investigation?
   A. Clinical diagnosis ± biopsy if atypical  B. Antibiotics  C. CT  D. None  E. Steroids only
   **Answer: A**
2. **Scenario:** Failed topical therapy; next step?
   A. Phototherapy or systemic  B. More topical  C. Observation  D. Surgery  E. Antifungals
   **Answer: A**
3. **Scenario:** Complication of X-linked recessive ichthyosis?
   A. Secondary infection and complications  B. Malignancy  C. Renal failure  D. Cardiac arrest  E. None
   **Answer: A**
4. **Scenario:** Pre-treatment screening before systemic therapy?
   A. TB, hep B/C, HIV  B. Pregnancy  C. ECG  D. None  E. Eye exam
   **Answer: A**
5. **Scenario:** Most characteristic histological finding in X-linked recessive ichthyosis?
   A. Ichthyosis vulgaris, other ichthyoses
   B. Non-specific  C. Always normal  D. Only late  E. No test
   **Answer: A**

## Flashcards

- **Q:** Key fact about X-linked recessive ichthyosis?
  **A:** X-linked recessive (males)
- **Q:** Another key point?
  **A:** STS (steroid sulfatase)
- **Q:** Most important complication?
  **A:** 1 in 2000-6000 males
- **Q:** First-line treatment?
  **A:** Large dark brown adherent
- **Q:** Key histological/serological feature?
  **A:** Extensor, trunk, neck, face, pre-auricular, palms/soles

## Answer Key with Explanations

### MCQs
1. **A** — Inheritance: X-linked recessive (males)
2. **A** — Topical first-line
3. **A** — Pre-biologic screening
4. **A** — Secondary infection most common
5. **A** — Differential: Ichthyosis vulgaris, other ichthyoses

### SBAs
1. **A** — Clinical + biopsy if atypical
2. **A** — Phototherapy/systemic for moderate-severe
3. **A** — Secondary infection most common
4. **A** — Pre-biologic screening
5. **A** — Ichthyosis vulgaris, other ichthyoses

## Local Navigation

**Parent Heading Hub:** [[../../Genodermatoses Hub]]
**Chapter Hierarchy:** [[../../../Davidson Chapter 29 - Dermatology Hierarchy]]
**Chapter MOC:** [[../../../Dermatology MOC]]

---

> [!tip] **Exam focus:** Know definition, key clinical features, first-line management, and the most important complication. Always compare with the main differential.

## PasTest Scenario SBAs (Clinical Vignettes)

> **Auto-generated PasTest/Mediscope-style scenario SBAs** grounded in the authored source. Each scenario tests a real clinical fact (triad, specific sign, contraindication, trial, first-line Rx) extracted from the topic. *Source: Ch 26: Dermatology — X-linked recessive ichthyosis full*

**Q1.** What is the most appropriate first-line therapy for X-linked recessive ichthyosis full?

  - **A.** First-line:
  - **B.** An advanced/surgical therapy reserved for refractory disease
  - **C.** Symptomatic treatment only, no disease-modifying therapy
  - **D.** Empiric broad-spectrum therapy without specific indication

  > **Answer: A** — First-line:
  >
  > *Source:* **First-line:** General measures (emollients, soap substitutes, trigger avoidance) and topical therapy (corticosteroids, calcineurin inhibitors, vitamin D analogues, antifungals as appropriate)

