...
```mermaid
flowchart TD
    A[Pruritic Eczema] --> B{UK Criteria Met?}
    B -->|Yes| C[Atopic Dermatitis]
    B -->|No| D[Differentials: Contact, Seb Derm, Psoriasis, CTCL]
    C --> E[Assess Severity: EASI/SCORAD/POEM/DLQI]
    E --> F{Severity}
    F -->|Mild| G[Emollients + Mild TCS face/flexures]
    F -->|Moderate| H[Potent TCS body + TCI face/flexures]
    F -->|Severe| I[TCI + NB-UVB or Systemic]
    H & I --> J{Controlled?}
    J -->|Yes| K[Maintenance: Emollients + Weekend TCS/TCI]
    J -->|No| L[Escalate: Systemic → Biologic]
    L --> M{Ciclosporin vs MTX vs Biologic}
    M -->|Rapid control needed| N[Ciclosporin 3mg/kg]
    M -->|Steroid-sparing| O[MTX 15mg/wk + Folate]
    M -->|Refractory| P[Dupilumab → JAKi]
```
...
> **Auto-generated PasTest/Mediscope-style scenario SBAs** grounded in the authored source. Each scenario tests a real clinical fact (triad, specific sign, contraindication, trial, first-line Rx) extracted from the topic. *Source: Ch 26: Dermatology — Atopic Dermatitis*

**Q1.** Which of the following features is most specific or characteristic of Atopic Dermatitis?
...
**Q2.** What is the most appropriate first-line therapy for Atopic Dermatitis?
  ...
  > **Answer: A** — Pimecrolimus
  >
  > *Source:* **Pimecrolimus**   1% cream   Mild-moderate, face/flexures   Less burning   Less potent than tacrolimus  
