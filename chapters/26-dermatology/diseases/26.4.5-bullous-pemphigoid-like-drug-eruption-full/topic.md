---
tags: [medicine, dermatology, davidson, fcps, mrcp, disease]
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 29: Dermatology
heading: Vesiculobullous Disorders
topic: Bullous pemphigoid-like drug eruption
status: full-fcps-mrcp-note
priority: high
created: 2026-06-16
modified: 2026-06-16
exam_relevance: [FCPS, MRCP Part 1, MRCP Part 2, PACES]
---

# Bullous pemphigoid-like drug eruption

Related: [[../../../Dermatology MOC]], [[../../../Davidson Chapter 29 - Dermatology Hierarchy]]

> [!info] Davidson alignment: **Chapter 29: Dermatology** (24th Edition, Ibbotson) — Vesiculobullous Disorders

## Definition

Bullous pemphigoid-like drug eruption is a drug-induced BP-like presentation. Common drugs: DPP-4 inhibitors (gliptins), diuretics, antibiotics (penicillins, sulfonamides), NSAIDs, vaccines (rare). Latency weeks to months. Resolves with drug withdrawal, but may persist.

## Clinical Features and Presentation

Bullous pemphigoid-like drug eruption presents with characteristic features as defined in Davidson 24e Ch 29. The key clinical features are summarised below. The disease is recognised by its morphology, distribution pattern, and course. Severity may be assessed using disease-specific scoring systems where applicable. Always consider the full differential when assessing a patient.

## Investigation and Management Summary

### Investigations
- Clinical diagnosis based on morphology, distribution, and history
- Skin biopsy (H&E) for histological confirmation in atypical or refractory cases
- Disease-specific tests as clinically indicated (autoantibodies, DIF for autoimmune bullous, culture for infection, KOH for fungal)
- Baseline bloods (FBC, LFT, renal) and pre-biologic screening (TB IGRA, hepatitis B/C, HIV) before systemic therapy

### Management
- **First-line:** General measures (emollients, soap substitutes, trigger avoidance) and topical therapy (corticosteroids, calcineurin inhibitors, vitamin D analogues, antifungals as appropriate)
- **Second-line:** Phototherapy (NB-UVB) or conventional systemic agents (methotrexate, ciclosporin, acitretin, azathioprine) for moderate-to-severe disease
- **Third-line:** Biologics or targeted small molecules (anti-TNF, anti-IL17/23, anti-IL4/13, JAK inhibitors) for refractory disease
- **Supportive:** Patient education, psychological support, quality-of-life assessment

## FCPS/MRCP High-Yield Summary

| Category | Key Points |
|----------|------------|
| Common drugs | DPP-4 inhibitors, diuretics, penicillins, sulfonamides, NSAIDs |
| Latency | Weeks to months |
| Mechanism | Drug-induced autoantibodies against BP180/BP230 |
| Treatment | STOP drug; topical clobetasol; systemic steroids if severe |
| DIF | Linear IgG/C3 at BMZ (same as classic BP) |
| DPP-4 inhibitors | Strongest association (2-4x risk); older patients with diabetes |
| Resolution | Weeks to months after drug withdrawal; may persist |
| Differential | Classic BP (idiopathic, elderly, no drug trigger) |
| Vaccines | Rare reports (COVID-19, influenza) |
| Monitoring | BP180 NC16A titres to assess response |

## Differential Diagnosis

| Differential | Distinguishing Feature |
|--------------|------------------------|
| Classic BP | Idiopathic, elderly, no drug trigger |

## Viva Questions

1. **Q:** Define Bullous pemphigoid-like drug eruption and outline its classification.
   **A:** Bullous pemphigoid-like drug eruption is a drug-induced BP-like presentation. Common drugs: DPP-4 inhibitors (gliptins), diuretics, antibiotics (penicillins, sulfonamides), NSAIDs, vaccines (rare). La

2. **Q:** What are the key clinical features of Bullous pemphigoid-like drug eruption?
   **A:** See the high-yield table above.

3. **Q:** Describe the stepwise management of Bullous pemphigoid-like drug eruption.
   **A:** Topical → phototherapy → conventional systemic → biologic.

4. **Q:** What scoring system is used for severity assessment?
   **A:** Disease-specific (see high-yield table).

5. **Q:** What are the key complications to monitor?
   **A:** Secondary infection, treatment toxicity, disease-specific complications.

## Confusions and Mnemonics

1. **BP-like drug eruption** = DPP-4 inhibitors + diuretics + penicillins

## Special Situations


| Situation | Consideration |
|-----------|---------------|
| **Pregnancy** | Avoid teratogens (retinoids, MTX, MMF) |
| **Paediatric** | Dosing adjustments; growth monitoring |
| **Elderly** | Polypharmacy caution |


## One-Page Revision Card

| **Topic** | **Bullous pemphigoid-like drug eruption** |
|-----------|-----------|
| **Definition** | Bullous pemphigoid-like drug eruption is a drug-induced BP-like presentation. Common drugs: DPP-4 in |
| **Key Features** | DPP-4 inhibitors, diuretics, penicillins, sulfonamides, NSAIDs |
| **Investigation** | Clinical ± biopsy + disease-specific tests |
| **Management** | Topical → phototherapy → systemic → biologic |
| **Red Flags** | Secondary infection, systemic involvement |
| **Viva Pearl** | See viva section |

## Spaced Repetition Trackers

- [ ] Day 1, 3, 7, 15, 30, 90

## Self-Test Scorecard

| Section | Score /5 |
|---------|----------|
| Understanding | /5 |
| Recall | /5 |
| MCQ Performance | /5 |
| SBA Performance | /5 |
| Viva Confidence | /5 |
| Total | /25 |

> <35 = weak, 35-44 = acceptable, 45+ = strong

## MCQs (5)
1. **Q:** Which of the following is a key feature of Bullous pemphigoid-like drug eruption?
   A. DPP-4 inhibitors, diuretics, penicillins, sulfonamides, NSAIDs
   B. Always self-limiting
   C. Caused by bacterial infection only
   D. Restricted to palms and soles only
   E. No investigation needed
   **Answer: A**  *Common drugs: DPP-4 inhibitors, diuretics, penicillins, sulfonamides, NSAIDs*
2. **Q:** What is the first-line treatment for Bullous pemphigoid-like drug eruption?
   A. Topical therapy + general measures
   B. Surgical excision
   C. IVIG
   D. No treatment
   E. Antibiotics
   **Answer: A**  *Topical therapy first-line*
3. **Q:** Which pre-treatment screening is essential before systemic therapy for Bullous pemphigoid-like drug eruption?
   A. TB IGRA, hepatitis B/C, HIV, pregnancy test
   B. Pregnancy test only
   C. ECG only
   D. No screening
   E. Eye exam only
   **Answer: A**  *Pre-biologic screening standard*
4. **Q:** What is the most important complication of Bullous pemphigoid-like drug eruption?
   A. Secondary infection and disease-specific complications
   B. Malignancy transformation
   C. Renal failure
   D. Cardiac arrest
   E. No complications
   **Answer: A**
5. **Q:** What is the key histological/serological feature of Bullous pemphigoid-like drug eruption?
   A. BP180 NC16A titres to assess response
   B. Always normal histology
   C. Non-specific findings
   D. Only positive in late disease
   E. No test exists
   **Answer: A**

## SBA Questions (5)
1. **Scenario:** Patient with Bullous pemphigoid-like drug eruption; first-line investigation?
   A. Clinical diagnosis ± biopsy if atypical  B. Antibiotics  C. CT  D. None  E. Steroids only
   **Answer: A**
2. **Scenario:** Bullous pemphigoid-like drug eruption failed topical therapy; next step?
   A. Phototherapy or systemic  B. More topical  C. Observation  D. Surgery  E. Antifungals
   **Answer: A**
3. **Scenario:** Complication of Bullous pemphigoid-like drug eruption?
   A. Secondary infection and complications  B. Malignancy  C. Renal failure  D. Cardiac arrest  E. None
   **Answer: A**
4. **Scenario:** Pre-treatment screening before systemic therapy for Bullous pemphigoid-like drug eruption?
   A. TB, hep B/C, HIV  B. Pregnancy  C. ECG  D. None  E. Eye exam
   **Answer: A**
5. **Scenario:** Most characteristic histological finding in Bullous pemphigoid-like drug eruption?
   A. BP180 NC16A titres to assess response
   B. Non-specific  C. Always normal  D. Only late  E. No test
   **Answer: A**

## Flashcards

- **Q:** Key fact about Bullous pemphigoid-like drug eruption?
  **A:** DPP-4 inhibitors, diuretics, penicillins, sulfonamides, NSAIDs
- **Q:** Another key point?
  **A:** Weeks to months
- **Q:** Most important complication?
  **A:** Drug-induced autoantibodies against BP180/BP230
- **Q:** First-line treatment?
  **A:** STOP drug; topical clobetasol; systemic steroids if severe
- **Q:** Key histological/serological feature?
  **A:** Linear IgG/C3 at BMZ (same as classic BP)

## Answer Key with Explanations

### MCQs
1. **A** — Common drugs: DPP-4 inhibitors, diuretics, penicillins, sulfonamides, NSAIDs
2. **A** — Topical first-line
3. **A** — Pre-biologic screening
4. **A** — Secondary infection most common
5. **A** — Monitoring: BP180 NC16A titres to assess response

### SBAs
1. **A** — Clinical + biopsy if atypical
2. **A** — Phototherapy/systemic for moderate-severe
3. **A** — Secondary infection most common
4. **A** — Pre-biologic screening
5. **A** — BP180 NC16A titres to assess response

## Local Navigation

**Parent Heading Hub:** [[../../Vesiculobullous Disorders Hub]]
**Chapter Hierarchy:** [[../../../Davidson Chapter 29 - Dermatology Hierarchy]]
**Chapter MOC:** [[../../../Dermatology MOC]]

---

> [!tip] **Exam focus:** Know definition, key clinical features, first-line management, and the most important complication. Always compare with the main differential.
