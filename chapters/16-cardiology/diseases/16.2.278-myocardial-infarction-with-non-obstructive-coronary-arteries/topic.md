...
source: "/mnt/tb/Medicine/Cardiology/Cardiomyopathies and Myocardial Disease/Dilated cardiomyopathy.md"
...
```mermaid
flowchart TD
A[HF symptoms / cardiomegaly / arrhythmia] --> B[Assess stability and congestion-perfusion]
B --> C[ECG + CXR + labs]
C --> D[Echo shows dilated poorly contracting ventricle]
D --> E[Search for cause: ischemia, alcohol, myocarditis, family, toxins, tachycardia]
E --> F[Start HFrEF-based therapy and treat reversible cause]
F --> G[Assess arrhythmia, thrombus, device need, advanced HF risk]
```
