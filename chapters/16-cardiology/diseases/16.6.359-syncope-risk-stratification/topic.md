...
```mermaid
flowchart TD
A[Syncope presentation] --> B{Red flags?}
B -->|Yes| C[ADMIT: Telemetry + Workup]
B -->|No| D{OESIL = 0?}
D -->|Yes| E[DISCHARGE: Follow-up + Safety netting]
D -->|No| F[ED OBSERVATION UNIT: 6-12h monitoring]
F --> G{Events / Abnormal findings?}
G -->|Yes| C
G -->|No| H[DISCHARGE: Outpatient workup]
C --> C1[Telemetry 24-48h]
C --> C2[Echo if not done]
C --> C3[Holter 24-48h]
C --> C4[Consider ILR if unexplained]
C --> C5[EPS if structural HD + syncope]
```
