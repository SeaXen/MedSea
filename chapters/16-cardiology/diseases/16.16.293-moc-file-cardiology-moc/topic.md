...
source: "/mnt/tb/Medicine/Cardiology/Heart Failure.md"
...
```mermaid
flowchart TD
  A[Suspected Heart Failure] --> B{Unstable?}
  B -->|Yes| C[Oxygen if hypoxemic, monitor, IV access, ECG, treat arrhythmia/ACS, urgent senior help]
  B -->|No| D[History + exam for congestion/perfusion]
  D --> E[ECG + CXR + BNP/NT-proBNP + bloods]
  E --> F[Echo to define EF, valves, structure]
  F --> G{Profile}
  G --> H[Warm & Wet -> diuretics +/- vasodilator]
  G --> I[Cold & Wet -> consider inotrope/ICU/advanced support]
  G --> J[Chronic HFrEF -> foundational therapy]
  G --> K[HFpEF -> treat congestion and comorbidity]
```
...
```mermaid
flowchart TD
  A[Breathless patient] --> B{HF likely?}
  B -->|Congestion/JVP/orthopnea| C[ECG + CXR + BNP + labs]
  B -->|Alternative diagnosis likely| D[Evaluate COPD/Pneumonia/PE/Renal causes]
  C --> E[Echo]
  E --> F{Phenotype}
  F -->|HFrEF| G[Foundational HFrEF therapy]
  F -->|HFpEF| H[Diuretics + treat HTN/AF/DM/CKD/obesity]
  C --> I{Acute pulmonary edema?}
  I -->|Yes| J[IV loop diuretic + nitrates if BP allows + NIV]
  C --> K{Cold & wet / shock?}
  K -->|Yes| L[ICU, urgent echo, inotrope/vasopressor, advanced support]
```
