...
davidson_topic_id: "16.16.309"
topic: "Pulseless electrical activity from cardiac causes"
...
source: "/mnt/tb/Medicine/Cardiology/Syncope, Shock, and Acute Hemodynamic Emergencies/Pulseless electrical activity from cardiac causes.md"
...
```mermaid
flowchart TD
A[PEA: Organised ECG, no pulse] --> B[CPR + Adrenaline 1mg IV q3-5min]
B --> C[POCUS during pulse check <10s]
C --> D{Tamponade?}
D -->|Yes| E[Pericardiocentesis NOW]
C --> F{Tension PTX?}
F -->|Yes| G[Needle decompression NOW]
C --> H{Massive PE? RV dilation + McConnell}
H -->|Yes| I[Thrombolysis NOW]
C --> J{RV infarct? Inferior MI + RV dil}
J -->|Yes| K[Fluids + avoid nitrates + PCI]
C --> L{Acute LV failure? Global hypokinesis}
L -->|Yes| M[Inotropes + mechanical support]
C --> N{Other H's & T's}
N --> O[Correct: K+, acidosis, hypoxia, hypovolaemia, toxins]
```
