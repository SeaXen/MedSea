...
davidson_topic_id: "16.1.61"
topic: "Obstructive shock from cardiac causes"
...
source: "/mnt/tb/Medicine/Cardiology/Syncope, Shock, and Acute Hemodynamic Emergencies/Obstructive shock from cardiac causes.md"
...
```mermaid
flowchart TD
A[Undifferentiated shock: obstructive suspected] --> B{Pocus/Clinical}
B -->|JVD + muffled HS + pulsus paradoxus| C[Tamponade → Pericardiocentesis]
B -->|Tracheal deviation + absent BS| D[Tension PTX → Needle decompression]
B -->|RV dilation + McConnell + DVT| E[Massive PE → Thrombolysis]
B -->|Tearing pain + pulse deficit + AR| F[Aortic dissection → BP control + Surgery]
B -->|Unclear| G[Bedside Echo + ECG + CXR]
G --> G1[IVC plethora + RA/RV collapse → Tamponade]
G --> G2[RV dilation + septal shift → PE]
G --> G3[Pneumothorax on lung ultrasound]
G --> G4[Aortic flap on TOE]
```
