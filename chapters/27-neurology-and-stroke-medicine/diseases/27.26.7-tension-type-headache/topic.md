---
tags: [medicine, neurology, davidson, headache, tension-type-headache, fcps, mrcp]
chapter: Neurology
...
```mermaid
flowchart TD
  A[Patient with headache] --> B{Red flags or abnormal neurological exam?}
  B -->|Yes| C[Evaluate for secondary headache: CT/MRI, LP if indicated, ESR/CRP, BP, fundoscopy, targeted tests]
  B -->|No| D{Typical TTH pattern? bilateral pressing mild-moderate no nausea no activity worsening}
  D -->|No| E[Consider migraine, cluster/TAC, medication-overuse, neuralgia, cervicogenic, meningitis, SAH, raised ICP]
  D -->|Yes| F[Classify: infrequent episodic / frequent episodic / chronic]
  F --> G[Assess stress, sleep, posture, anxiety/depression, analgesic frequency]
  G --> H[Simple acute therapy + lifestyle measures]
  H --> I{Frequent or chronic disabling pattern?}
  I -->|Yes| J[Consider preventive therapy + headache diary + treat comorbidity]
  I -->|No| K[Review and avoid medication overuse]
```
...
```mermaid
mindmap
  root((Tension-type headache))
    Definition
      Bilateral
      Pressing tight pain
      Mild moderate
      No nausea vomiting
      No activity worsening
    Classification
      Infrequent episodic
      Frequent episodic
      Chronic
    Mechanisms
      Pericranial nociception
      Central sensitization
      Stress sleep mood posture
    Clinical clues
      Band-like pressure
      Neck shoulder tightness
      Normal neuro exam
...
```mermaid
flowchart TD
  A[Bilateral recurrent headache] --> B{Red flags?}
  B -->|Yes| C[Evaluate secondary causes]
  B -->|No| D{Pressing tight mild-moderate, no nausea, no activity worsening?}
  D -->|No| E[Think migraine, cluster/TAC, cervicogenic, MOH]
  D -->|Yes| F[Diagnose likely TTH]
  F --> G[Classify episodic vs chronic]
  G --> H[Check stress, sleep, posture, mood, analgesic use]
  H --> I[Acute simple analgesia + non-drug care]
  I --> J{Frequent/chronic?}
  J -->|Yes| K[Consider amitriptyline + diary + comorbidity treatment]
  J -->|No| L[Review and avoid medication overuse]
```
