---
tags: [medicine, stroke-medicine, davidson, full-note, special-stroke-scenarios, pregnancy]
...
topic: Pregnancy-related stroke
...
```mermaid
flowchart TD
  A[Pregnant/postpartum patient with focal deficit or severe neuro symptoms] --> B[ABC + glucose + BP + obstetric context]
  B --> C[Urgent brain imaging]
  C --> D{Stroke type?}
  D -->|Ischemic| E[Assess reperfusion eligibility and mechanism]
  D -->|Hemorrhagic| F[Control BP, neurocritical care, cause-directed workup]
  D -->|Venous/CVST| G[CTV/MRV and anticoagulation pathway]
  E --> H[Maternal-fetal safety review + stroke-unit care]
  F --> H
  G --> H
```
...
```mermaid
flowchart LR
  A[Pregnant/postpartum neuro emergency] --> B[Stabilize mother]
  B --> C[Urgent brain imaging]
  C --> D{Stroke type?}
  D -->|Ischemic| E[Reperfusion/mechanism pathway]
  D -->|Hemorrhagic| F[BP control + neurocritical care]
  D -->|Venous| G[CTV/MRV + anticoagulation]
```
