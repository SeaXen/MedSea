---
tags: [medicine, stroke-medicine, davidson, full-note, reperfusion, thrombolysis, tenecteplase, alteplase]
chapter: Stroke Medicine
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 29: Stroke medicine
heading: Reperfusion Therapy
topic_group: Intravenous thrombolysis
topic: Tenecteplase vs alteplase practical points
...
```mermaid
flowchart TD
  A[Eligible acute ischemic stroke] --> B[Exclude hemorrhage + check safety criteria]
  B --> C[Decide thrombolysis candidate]
  C --> D{Local protocol: alteplase or tenecteplase?}
  D -->|Alteplase| E[Bolus + infusion protocol]
  D -->|Tenecteplase| F[Single-bolus protocol]
  E --> G[Post-thrombolysis monitoring]
  F --> G
  G --> H[Proceed with thrombectomy pathway if LVO]
```
...
```mermaid
mindmap
  root((Tenecteplase vs alteplase))
    Same
      Acute ischemic stroke use
      Need CT bleed exclusion
      Same major contraindication logic
      Same major bleed risk
    Tenecteplase
      Single bolus
      Workflow simplicity
      Useful in transfer/LVO pathways
    Alteplase
      Bolus plus infusion
...
```mermaid
flowchart LR
  A[Eligible thrombolysis patient] --> B[Safety screening]
  B --> C{Protocol drug?}
  C -->|Tenecteplase| D[Single bolus]
  C -->|Alteplase| E[Bolus + infusion]
  D --> F[Monitor + thrombectomy path if LVO]
  E --> F
```
