---
tags: [medicine, stroke-medicine, davidson, full-note, stroke-unit-care, blood-pressure, ischaemic-stroke]
chapter: Stroke Medicine
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 29: Stroke medicine
heading: Stroke Unit Care and Complications
topic_group: Physiological optimization
topic: Blood pressure management in acute ischaemic stroke
...
```mermaid
flowchart TD
  A[Acute ischaemic stroke] --> B[Confirm repeated BP properly]
  B --> C{Reperfusion therapy planned?}
  C -->|Yes: IV thrombolysis| D[Lower BP to <185/110 before lysis]
  D --> E[Maintain <180/105 after lysis with close monitoring]
  C -->|Yes: thrombectomy pathway| F[Follow local thrombectomy BP strategy and avoid extremes]
  C -->|No reperfusion| G{BP extremely high or another emergency present?}
  G -->|No| H[Permissive hypertension + treat triggers such as pain, fever, retention]
  G -->|Yes| I[Cautious BP reduction with IV agents]
  E --> J[Watch for neuro worsening or hemorrhagic transformation]
  F --> J
  H --> J
  I --> J
```
...
```mermaid
flowchart LR
  A[High BP in AIS] --> B{Thrombolysis planned?}
  B -->|Yes| C[Lower to <185/110]
  C --> D[Give thrombolysis]
  D --> E[Maintain <180/105]
  B -->|No| F{Other emergency or >220/120?}
  F -->|No| G[Observe + permissive HTN]
  F -->|Yes| H[Cautious IV BP reduction]
```
