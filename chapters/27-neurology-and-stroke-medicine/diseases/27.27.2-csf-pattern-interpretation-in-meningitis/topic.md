---
tags: [medicine, neurology, davidson, meningitis, csf, lumbar-puncture, interpretation, fcps, mrcp]
chapter: Neurology
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 28: Neurology
heading: Meningitis
topic_group: Workup and management
topic: CSF pattern interpretation in meningitis
exam: [FCPS, MRCP]
...
```mermaid
flowchart TD
  A[Suspected meningitis] --> B[Assess LP safety]
  B --> C{Signs needing imaging / no immediate LP?}
  C -->|Yes| D[Blood cultures + empiric therapy first]
  C -->|No| E[Perform LP promptly]
  E --> F[Measure opening pressure]
  F --> G[Send cells, protein, glucose, Gram stain/culture, special tests]
  G --> H{CSF pattern}
  H -->|Neutrophils + low glucose + high protein| I[Bacterial likely]
  H -->|Lymphocytes + normal glucose/mild protein rise| J[Viral likely]
  H -->|Lymphocytes + low glucose + very high protein| K[TB/fungal likely]
  H -->|Atypical/mixed| L[Partially treated or alternative diagnosis]
```
...
```mermaid
flowchart LR
  A[Suspected meningitis] --> B[Is LP safe?]
  B -->|No| C[Treat first + image]
  B -->|Yes| D[LP with opening pressure]
  D --> E[Cells + protein + glucose + microbiology]
  E --> F{Pattern}
  F -->|Neutrophils low glucose| G[Bacterial]
  F -->|Lymphocytes normal glucose| H[Viral]
  F -->|Lymphocytes low glucose high protein| I[TB/Fungal]
```
...
> **Auto-generated PasTest/Mediscope-style scenario SBAs** grounded in the authored source. Each scenario tests a real clinical fact (triad, specific sign, contraindication, trial, first-line Rx) extracted from the topic. *Source: Ch 27: Neurology & Stroke — CSF pattern interpretation in meningitis*

**Q1.** What is the most appropriate first-line therapy for CSF pattern interpretation in meningitis?
  ...
  > **Answer: A** — TB meningitis
  >
  > *Source:* If CSF suggests **TB meningitis**, start anti-TB therapy plus steroids per protocol.
