...
```mermaid
flowchart TD
  A[Patient with recurrent headache] --> B{Red flags / SNOOP?}
  B -->|Yes| C[Evaluate for secondary headache: CT/MRI, LP, ESR/CRP, eye exam, BP, targeted tests]
  B -->|No| D{Typical migraine pattern?}
  D -->|Yes| E[Classify: with aura / without aura / chronic / menstrual / complicated]
  D -->|No| F[Consider tension-type, cluster/TAC, sinus, cervicogenic, neuralgia, MOH]
  E --> G[Assess disability, attack frequency, medication overuse, comorbidities, pregnancy]
  G --> H[Acute treatment plan]
  H --> I{>=4 disabling days per month, frequent attacks, prolonged attacks, or poor acute control?}
  I -->|Yes| J[Start preventive therapy + trigger modification + headache diary]
  I -->|No| K[Continue optimized acute plan and review]
```
...
```mermaid
flowchart TD
  A[Typical recurrent headache] --> B{Migraine features present?}
  B -->|No| C[Reconsider differential diagnosis]
  B -->|Yes| D{Any red flag or abnormal neuro exam?}
  D -->|Yes| E[Evaluate for secondary headache]
  D -->|No| F[Classify migraine subtype]
  F --> G[Acute treatment]
  G --> H[Check disability, monthly frequency, medication overuse]
  H --> I{Needs prevention?}
  I -->|Yes| J[Start preventive therapy + diary + trigger/lifestyle work]
  I -->|No| K[Review response and avoid overuse]
```
...
> **Auto-generated PasTest/Mediscope-style scenario SBAs** grounded in the authored source. Each scenario tests a real clinical fact (triad, specific sign, contraindication, trial, first-line Rx) extracted from the topic. *Source: Ch 27: Neurology & Stroke — Migraine with and without aura*

**Q1.** Which of the following features is most specific or characteristic of Migraine with and without aura?
  ...
  > **Answer: A** — A. Sumatriptan
  >
  > *Source:* **A. Sumatriptan** — triptans are migraine-specific abortive therapy

**Q2.** What is the most appropriate first-line therapy for Migraine with and without aura?
  ...
  > **Answer: A** — headache diary
  >
  > *Source:* use a **headache diary**
