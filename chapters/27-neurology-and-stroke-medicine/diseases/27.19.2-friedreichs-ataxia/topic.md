---
tags: [medicine, neurology, genetic, ataxia, fcps, mrcp]
chapter: Neurology
topic: Friedreichs Ataxia
exam: [FCPS, MRCP Part 1, MRCP Part 2, PACES]
references:
  clinical: ['Davidson 24th Ed Ch25', 'Adams & Victor Principles of Neurology', 'GeneReviews']
related: ['Ataxia-Telangiectasia', 'Spinocerebellar Ataxias', 'Mitochondrial Disorders', 'Hereditary Spastic Paraplegia']
...
```mermaid
flowchart TD
    A[FXN Mutation] --> B[↓ Frataxin]
    B --> C[Mitochondrial Iron Accumulation]
    C --> D[Fe-S Cluster Defect]
    D --> E[OXPHOS Dysfunction + ↑ ROS]
    E --> F[Cell Death in Iron-Rich Tissues]
    F --> G[Spinocerebellar, Dorsal Columns, Heart, Pancreas]
```
...
```mermaid
flowchart TD
    A[Progressive ataxia + areflexia + Babinski] --> B[Genetic Testing: FXN GAA Repeats]
    B -->|Homozygous expansion| C[FRDA Confirmed]
    B -->|Negative| D[Consider AVED, AT, SCAs, mitochondrial]
    C --> E[Cardiac Workup: ECG, Echo, Holter]
    E --> F[Glucose/HbA1c]
    F --> G[MRI Spine + Nerve Conduction]
```
