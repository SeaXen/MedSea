---
tags: [medicine, neurology, davidson, headache, meningitis, encephalitis, secondary-headache, csf, fcps, mrcp]
chapter: Neurology
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 28: Neurology
heading: Headache Syndromes
topic_group: Secondary headache red flags
topic: Meningitis and encephalitis clues
exam: [FCPS, MRCP]
...
```mermaid
flowchart TD
A[Headache + fever or altered mental state] --> B[ABC, vitals, glucose, sepsis check]
B --> C[Blood cultures + urgent senior review]
C --> D{Signs of raised ICP/focal deficit/reduced GCS?}
D -->|Yes| E[Consider imaging before LP if indicated]
D -->|No| F[Prompt LP if safe]
E --> G[Start empiric treatment without dangerous delay]
F --> G
G --> H[Interpret CSF and continue targeted therapy]
```
...
```mermaid
flowchart LR
A[Headache + fever] --> B[Assess meningism and mental state]
B --> C{Confusion/seizure/focal signs?}
C -->|Yes| D[Think encephalitis/meningoencephalitis]
C -->|No| E[Think meningitis but keep broad differential]
D --> F[Urgent treatment + LP/imaging logic]
E --> F
```
