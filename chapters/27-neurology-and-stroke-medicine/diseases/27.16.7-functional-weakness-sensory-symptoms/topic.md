---
tags: [medicine, neurology, functional, conversion, fcps, mrcp]
chapter: Neurology
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 25: Neurology
topic: Functional Weakness & Sensory Symptoms
exam: [FCPS, MRCP Part 1, MRCP Part 2, PACES]
references:
  clinical: ["Davidson 24th Ed Ch25", "Adams & Victor's Principles of Neurology", "Stone et al. 2010 JNNP", "PasTest", "MRCP Notes", "neurosymptoms.org"]
related: ['Functional Visual Symptoms', 'Functional Movement Disorders', 'Dissociative Attacks']
...
```mermaid
flowchart TD
    A[Vulnerability] --> B[Acute Trigger]
    B --> C[Abnormal Predictions / Attention]
    C --> D[Altered Sense of Agency]
    D --> E[Functional Symptoms]
    E --> F[Reinforcement by Attention]
```
...
```mermaid
flowchart TD
    A[Sudden weakness / sensory loss] --> B[History for FND clues]
    B --> C[Examination: positive signs]
    C --> D[Hoover's / Hip abductor positive]
    D --> E[Screening MRI, NCS, EEG]
    E --> F{Investigations normal?}
    F -->|Yes| G[Diagnose FND positively]
    F -->|Organic lesion| H[Co-existing FND possible]
    G --> I[Explain + Refer PT/CBT]
```
