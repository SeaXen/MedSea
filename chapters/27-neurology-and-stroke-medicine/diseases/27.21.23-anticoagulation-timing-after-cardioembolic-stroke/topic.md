---
tags: [medicine, stroke-medicine, davidson, full-note, secondary-prevention, anticoagulation, cardioembolic-stroke]
chapter: Stroke Medicine
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 29: Stroke medicine
heading: Secondary Prevention
topic_group: Antithrombotic strategies
topic: Anticoagulation timing after cardioembolic stroke
...
```mermaid
flowchart TD
  A[Acute ischaemic stroke] --> B[Determine mechanism]
  B --> C{Cardioembolic source likely/confirmed?}
  C -->|No| D[Use non-cardioembolic prevention pathway]
  C -->|Yes| E[Assess infarct size/severity + CT/MRI bleed risk]
  E --> F{TIA/small stroke?}
  F -->|Yes| G[Earlier anticoagulation may be appropriate]
  F -->|No| H{Moderate/large stroke or hemorrhagic risk?}
  H -->|Yes| I[Delay and re-image before starting]
  H -->|No| J[Intermediate timing]
  G --> K[Long-term anticoagulation]
  I --> K
  J --> K
```
...
```mermaid
mindmap
  root((Anticoagulation timing after cardioembolic stroke))
    Why needed
      Prevent recurrent embolism
      AF common cause
    Why delayed sometimes
    ...
    Long term
      DOAC if suitable
      Warfarin in valve cases
```

## Flowchart
```mermaid
flowchart LR
  A[Cardioembolic stroke] --> B[Assess infarct size + bleed risk]
  B --> C{TIA/small infarct?}
  C -->|Yes| D[Earlier anticoagulation]
  C -->|No| E{Large infarct/HT risk?}
  E -->|Yes| F[Delay + repeat imaging]
  E -->|No| G[Intermediate timing]
  D --> H[Long-term anticoagulation]
  F --> H
  G --> H
```
