---
tags: [medicine, stroke-medicine, davidson, full-note, recovery-rehabilitation-prognosis, aphasia, communication]
...
```mermaid
flowchart TD
  A[Stroke patient with communication difficulty] --> B[Check hearing, consciousness, motor speech, and attention]
  B --> C[Assess fluency, comprehension, repetition, naming]
  C --> D{Aphasia pattern?}
  D -->|Yes| E[Speech-language therapy + communication support]
  D -->|No/unclear| F[Consider dysarthria, delirium, neglect, hearing loss]
  E --> G[Reassess reading, writing, mood, and caregiver needs]
```
...
```mermaid
flowchart LR
  A[Communication difficulty after stroke] --> B[Check attention/hearing/articulation]
  B --> C[Test fluency, comprehension, repetition, naming]
  C --> D[Classify aphasia pattern]
  D --> E[Speech therapy + communication support]
```
