---
tags: [medicine, stroke-medicine, davidson, full-note, secondary-prevention, hypertension]
chapter: Stroke Medicine
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 29: Stroke medicine
heading: Secondary Prevention
topic_group: Risk-factor modification
topic: Hypertension management for secondary stroke prevention
...
```mermaid
flowchart TD
  A[Stroke/TIA survivor] --> B[Reassess when acute phase stabilized]
  B --> C[Confirm persistent hypertension with repeated readings]
  C --> D[Check comorbidities, adherence, lifestyle, kidney function]
  D --> E[Start/optimize lifestyle + antihypertensive therapy]
  E --> F[Monitor BP, side effects, orthostasis, renal function]
  F --> G[Escalate combination therapy if target unmet]
```
...
```mermaid
flowchart LR
  A[Persistent post-stroke hypertension] --> B[Lifestyle optimization]
  B --> C[Start/adjust antihypertensive therapy]
  C --> D[Check renal function/electrolytes]
  D --> E[Monitor adherence + home BP]
  E --> F[Escalate to combination therapy if needed]
```
