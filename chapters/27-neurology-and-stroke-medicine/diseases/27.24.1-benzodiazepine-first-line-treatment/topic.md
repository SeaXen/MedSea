---
tags: [medicine, neurology, davidson, epilepsy, status-epilepticus, benzodiazepine, emergency, fcps, mrcp]
chapter: Neurology
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 28: Neurology
heading: Epilepsy
topic_group: Status epilepticus
topic: Benzodiazepine first-line treatment
exam: [FCPS, MRCP]
...
```mermaid
flowchart TD
  A[Convulsive seizure >5 min or repeated without recovery] --> B[ABC, oxygen, glucose, IV access if possible]
  B --> C{IV access available?}
  C -->|Yes| D[IV benzodiazepine]
  C -->|No| E[Buccal/intranasal/IM benzodiazepine]
  D --> F{Seizure stops?}
  E --> F
  F -->|Yes| G[Monitor airway and search cause]
  F -->|No| H[Repeat as protocol and prepare second-line ASM]
  H --> I[Escalate to levetiracetam/valproate/phenytoin pathway]
```
...
```mermaid
flowchart LR
  A[Status epilepticus recognized] --> B[Give benzodiazepine promptly]
  B --> C[Monitor airway and oxygenation]
  C --> D{Seizure stopped?}
  D -->|Yes| E[Cause search and observation]
  D -->|No| F[Second-line escalation]
```
