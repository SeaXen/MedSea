---
tags: [medicine, stroke-medicine, davidson, full-note, stroke-unit-care, blood-pressure, intracerebral-haemorrhage]
...
topic: Blood pressure management in intracerebral haemorrhage
...
```mermaid
flowchart TD
  A[Confirmed/suspected ICH] --> B[Repeat BP + urgent CT head]
  B --> C[Assess GCS, hematoma size, ICP clues, comorbidities]
  C --> D[Start monitored IV BP control if markedly elevated]
  D --> E[Aim for smooth controlled SBP reduction]
  E --> F[Serial neuro observations + repeat BP]
  F --> G{Deterioration or mass effect?}
  G -->|Yes| H[Urgent neurosurgical review / ICU care]
  G -->|No| I[Continue protocol-based stroke-unit management]
```
...
```mermaid
flowchart LR
  A[ICH + severe hypertension] --> B[Confirm on CT + repeat BP]
  B --> C[Start monitored IV BP control]
  C --> D[Reduce SBP smoothly toward safe range]
  D --> E[Serial neuro checks]
  E --> F{Worsening / ICP signs?}
  F -->|Yes| G[Urgent ICU / neurosurgery review]
  F -->|No| H[Continue stroke-unit care]
```
