---
tags: [medicine, stroke-medicine, davidson, full-note, posterior-circulation, basilar-artery-occlusion]
chapter: Stroke Medicine
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 29: Stroke medicine
heading: Special Stroke Scenarios
topic_group: Posterior circulation and brainstem issues
...
```mermaid
flowchart TD
  A[Acute posterior circulation symptoms / depressed consciousness] --> B[ABC + glucose + neuro exam]
  B --> C[Suspect brainstem stroke?]
  C -->|Yes| D[Urgent non-contrast CT + CTA or MRA]
  C -->|No| E[Consider other emergency differentials]
  D --> F{Basilar occlusion confirmed / strongly suspected?}
  F -->|Yes| G[Activate reperfusion pathway and thrombectomy evaluation]
  G --> H[ICU/stroke-unit care + airway monitoring]
  H --> I[Monitor for edema, aspiration, respiratory failure, locked-in state]
  F -->|No| J[Alternative posterior circulation workup]
```
...
```mermaid
flowchart LR
  A[Posterior circulation syndrome] --> B[NCCT]
  B --> C[CTA/MRA]
  C --> D{Basilar occlusion?}
  D -->|Yes| E[Reperfusion pathway]
  E --> F[Thrombectomy consideration]
  E --> G[IV alteplase if eligible]
  F --> H[ICU/stroke-unit monitoring]
  G --> H
```
...
> **Auto-generated PasTest/Mediscope-style scenario SBAs** grounded in the authored source. Each scenario tests a real clinical fact (triad, specific sign, contraindication, trial, first-line Rx) extracted from the topic. *Source: Ch 27: Neurology & Stroke — Basilar artery occlusion*

**Q1.** What is the most appropriate first-line therapy for Basilar artery occlusion?

  - **A.** Use antiplatelet, anticoagulation, statin, or vascular risk-factor control according to the true mechanism
  - **B.** An advanced/surgical therapy reserved for refractory disease
  - **C.** Symptomatic treatment only, no disease-modifying therapy
  - **D.** Empiric broad-spectrum therapy without specific indication

  > **Answer: A** — Use antiplatelet, anticoagulation, statin, or vascular risk-factor control according to the true mechanism
  >
  > *Source:* Use antiplatelet, anticoagulation, statin, or vascular risk-factor control according to the true mechanism.
