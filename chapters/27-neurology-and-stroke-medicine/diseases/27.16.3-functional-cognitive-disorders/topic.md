---
tags: [medicine, neurology, functional, cognitive, fcps, mrcp]
chapter: Neurology
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 25: Neurology
topic: Functional Cognitive Disorders
exam: [FCPS, MRCP Part 1, MRCP Part 2, PACES]
references:
  anatomy: []
  physiology: []
  clinical: ['Davidson 24th Ed Ch25', 'Neurology: A Clinician''s Approach', 'Stone et al BMJ 2020', 'Schmidtke & Metternich 2009', 'Ball et al JNNP 2020', 'PasTest', 'MRCP Part 1/2 Notes', 'Personal notes']
related: ['Functional Neurological Disorders', 'Functional Movement Disorders', 'Dementia Workup', 'Mild Cognitive Impairment', 'Depression Pseudodementia']
...
```mermaid
flowchart TD
    A[Predisposing factors: anxiety, trauma, illness beliefs] --> B[Trigger: stress, illness, pandemic]
    B --> C[Heightened body/cognitive attention]
    C --> D[Impaired top-down attentional control]
    D --> E[Predictive processing mismatch]
    E --> F[Functional cognitive symptoms]
    F --> G[Behavioural reinforcement & fear]
    G --> H[Symptom persistence - Bayesian inference model]
```
...
```mermaid
flowchart TD
    A[Cognitive complaint] --> B[Detailed history + cognitive exam]
    B --> C{Positive functional signs?}
    C -->|Yes| D[Functional Cognitive Disorder]
    C -->|No/Unclear| E[Standardised cognitive testing]
    D --> F[Investigate comorbid anxiety/depression/sleep]
    E --> G{Normal?}
    G -->|Yes| H[Investigate mood, sleep, pain, drugs]
    G -->|No MCI/dementia| I[MCI workup: MRI, biomarkers]
    H --> J{Symptomatic after treatment?}
    J -->|No| K[Reassure + monitor]
    J -->|Yes| L[Repeat testing in 6-12 mo]
```
...
```mermaid
flowchart TD
    A[FCD Diagnosis] --> B[Positive explanation & education]
    B --> C[Address comorbidities]
    C --> D[Mood, anxiety, sleep, pain]
    A --> E[Therapy]
    E --> F[CBT or ACT]
    E --> G[Group therapy]
    E --> H[Occupational therapy]
    A --> I[Reassess at 6-12 months]
    I --> J{Improved?}
    J -->|Yes| K[Step down + monitor]
    J -->|No| L[Repeat testing, MRI, consider biomarker]
    L --> M{New diagnosis?}
    M -->|Yes| N[Treat accordingly]
    M -->|No| O[Continue therapy + MDT]
```
...
```mermaid
mindmap
  root((Functional Cognitive Disorder))
    Definition
...
> **Auto-generated PasTest/Mediscope-style scenario SBAs** grounded in the authored source. Each scenario tests a real clinical fact (triad, specific sign, contraindication, trial, first-line Rx) extracted from the topic. *Source: Ch 27: Neurology & Stroke — Functional Cognitive Disorders*

**Q1.** Which of the following features is most specific or characteristic of Functional Cognitive Disorders?
  ...
  > **Answer: A** — Functional impact:
  >
  > *Source:* ting, word-finding, "brain fog"
- **Mood:** Anxiety (drives catastrophising), low mood, anhedonia
- **Functional impact:** Variable — often preserved in key domains despite complaints (drives, manages

**Q2.** What is the most appropriate first-line therapy for Functional Cognitive Disorders?
  ...
  > **Answer: A** — Significant mood disorder
  >
  > *Source:* **Significant mood disorder**   Treat with SSRI/SNRI, refer psychiatry  
