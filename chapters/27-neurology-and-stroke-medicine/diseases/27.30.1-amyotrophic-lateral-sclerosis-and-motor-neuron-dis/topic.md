...
```mermaid
flowchart TD
  A[Progressive weakness] --> B{Pure motor syndrome?}
  B -->|No| C[Look for sensory, cerebellar, extrapyramidal, or structural alternative]
  B -->|Yes| D{LMN signs present? wasting/fasciculations}
  D -->|No| E[Consider pure UMN disorders, myelopathy, PLS, stroke-related syndromes]
  D -->|Yes| F{UMN signs also present? brisk reflexes/extensor plantars/spasticity}
  F -->|No| G[Consider LMN-predominant disorders: MMN, neuropathy, SMA, myopathy]
  F -->|Yes| H[Consider ALS/MND]
  H --> I[Assess bulbar and respiratory involvement]
  I --> J[EMG/NCS + MRI + blood tests to support ALS and exclude mimics]
  J --> K[Multidisciplinary management and prognostic discussion]
```
