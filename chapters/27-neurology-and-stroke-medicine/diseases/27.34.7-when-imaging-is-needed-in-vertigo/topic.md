---
tags: [medicine, neurology, davidson, vestibular, vertigo, neuroimaging, red-flags, posterior-fossa, fcps, mrcp]
chapter: Neurology
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 28: Neurology
heading: Vestibular Disorders
topic_group: Approach to dizziness and vertigo
topic: When imaging is needed in vertigo
exam: [FCPS, MRCP]
...
```mermaid
flowchart TD
A[Patient with vertigo] --> B{Classic benign peripheral pattern?}
B -->|Yes| C[Usually no immediate imaging]
B -->|No / uncertain| D[Check central red flags]
D --> E{Vertical/direction-changing nystagmus, severe ataxia, focal deficits, headache, altered state?}
E -->|Yes| F[Urgent neuroimaging]
E -->|No| G[Consider targeted imaging if atypical course persists]
F --> H{Need immediate triage?}
H -->|Yes| I[CT initially if fastest available]
H -->|No or posterior fossa detail needed| J[MRI preferred]
```
...
```mermaid
flowchart LR
A[Vertigo] --> B[Bedside localization]
B --> C[Benign peripheral pattern?]
C -->|Yes| D[Usually no imaging]
C -->|No| E[Central red flags?]
E -->|Yes| F[Urgent imaging]
F --> G[CT if fastest triage]
F --> H[MRI if posterior fossa detail needed]
```
