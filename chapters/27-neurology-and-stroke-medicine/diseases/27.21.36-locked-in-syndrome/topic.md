---
tags: [medicine, stroke-medicine, davidson, full-note, posterior-circulation, brainstem]
...
```mermaid
flowchart TD
  A[Patient appears unresponsive or minimally responsive] --> B[Assess ABCs, glucose, pupils, eye opening]
  B --> C{Vertical eye movement or blink to command present?}
  C -->|Yes| D[Suspect locked-in syndrome]
  C -->|No| E[Consider coma, sedation, severe encephalopathy, total locked-in]
  D --> F[Urgent CT + CTA/MRA posterior circulation]
  F --> G[Consider MRI brain / pontine lesion confirmation]
  G --> H[Stroke reperfusion pathway + ICU/stroke-unit care]
  H --> I[Establish communication system and supportive care]
```
...
```mermaid
flowchart LR
  A[Apparent coma / no speech / no limb movement] --> B[Test eye opening and blink to command]
  B --> C{Purposeful vertical gaze or blink?}
  C -->|Yes| D[Suspect locked-in syndrome]
  D --> E[CTA/MRA for basilar occlusion]
  E --> F[Reperfusion evaluation]
  F --> G[ICU care + communication system + rehab]
  C -->|No| H[Assess coma / total locked-in / other causes]
```
