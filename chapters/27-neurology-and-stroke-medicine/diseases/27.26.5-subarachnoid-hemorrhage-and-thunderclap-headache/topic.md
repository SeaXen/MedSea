---
tags: [medicine, neurology, davidson, headache, thunderclap-headache, subarachnoid-hemorrhage, secondary-headache, fcps, mrcp]
chapter: Neurology
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 28: Neurology
heading: Headache Syndromes
topic_group: Secondary headache red flags
topic: Subarachnoid hemorrhage and thunderclap headache
exam: [FCPS, MRCP]
...
```mermaid
flowchart TD
A[Thunderclap headache] --> B[ABC + neuro exam + BP + glucose]
B --> C[Urgent non-contrast CT head]
C --> D{SAH seen?}
D -->|Yes| E[Neurosurgical/acute specialist pathway + stabilization]
D -->|No but suspicion high| F[Further workup including LP/vascular imaging per protocol]
F --> G[Confirm or exclude SAH/mimic]
E --> H[Monitor for rebleed, hydrocephalus, vasospasm]
```
...
```mermaid
flowchart LR
A[Sudden worst headache] --> B[Think SAH]
B --> C[Urgent CT head]
C --> D{Positive?}
D -->|Yes| E[Acute specialist management]
D -->|No + suspicion high| F[LP/vascular imaging]
F --> G[Confirm or exclude SAH]
```
...
> **Auto-generated PasTest/Mediscope-style scenario SBAs** grounded in the authored source. Each scenario tests a real clinical fact (triad, specific sign, contraindication, trial, first-line Rx) extracted from the topic. *Source: Ch 27: Neurology & Stroke — Subarachnoid hemorrhage and thunderclap headache*

**Q1.** Which of the following features is most specific or characteristic of Subarachnoid hemorrhage and thunderclap headache?
  ...
  > **Answer: A** — CT first
  >
  > *Source:* - **CT first** is the key investigation rule
