...
```mermaid
flowchart TD
  A[Acute vertigo / dysphagia / crossed sensory signs] --> B[ABC + glucose + focused neuro exam]
  B --> C[Look for posterior circulation red flags]
  C --> D[Non-contrast CT head]
  D --> E[CTA or MRA posterior circulation]
  E --> F[Consider MRI-DWI if CT nondiagnostic]
  F --> G[Confirm lateral medullary / vertebral-PICA infarct]
  G --> H[Acute stroke pathway + swallow screen + aspiration precautions]
  H --> I[Secondary prevention after mechanism defined]
```
...
```mermaid
flowchart LR
  A[Acute vertigo +/- vomiting] --> B{Focal signs present?}
  B -->|Yes| C[Check dysphagia hoarseness crossed sensory loss Horner syndrome ataxia]
  C --> D[Suspect lateral medullary syndrome]
  D --> E[CT + CTA/MRA]
  E --> F[MRI-DWI if needed]
  F --> G[Stroke-unit treatment + swallow precautions + mechanism-based prevention]
  B -->|No| H[Consider peripheral vestibular causes]
```
