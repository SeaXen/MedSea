---
tags: [medicine, stroke-medicine, davidson, full-note, recovery-rehabilitation-prognosis, driving, work, recurrence-counseling]
...
topic: Driving, work, and recurrence counseling after stroke
...
```mermaid
flowchart TD
  A[Stroke survivor nearing discharge/follow-up] --> B[Assess residual motor, visual, cognitive, language, mood deficits]
  B --> C[Review job type, driving role, support system, recurrence risk factors]
  C --> D{Safe to resume driving/work now?}
  D -->|No/uncertain| E[Advise restriction + rehabilitation + specialist review]
  D -->|Possibly yes| F[Gradual return plan with clear precautions]
  E --> G[Optimize secondary prevention and document counseling]
  F --> G
```
...
```mermaid
flowchart LR
  A[Post-stroke follow-up] --> B[Assess residual deficits]
  B --> C[Assess driving/work demands]
  C --> D[Advise restriction or graded return]
  D --> E[Optimize secondary prevention]
  E --> F[Document counseling and follow-up]
```
