---
tags: [medicine, neurology, neuroophthalmology, iih, pseudotumour, fcps, mrcp]
chapter: Neurology
...
references:
  anatomy: ['Optic nerve', 'Optic nerve sheath', 'CSF circulation']
  physiology: ['CSF dynamics', 'ICP regulation']
  clinical: ['Davidson 24th Ed Ch25', 'Friesen et al 2016 revised criteria', 'NICE', 'Bradley 7th Ed', 'MRCP notes']
related: ['Papilloedema', 'Headache', 'Visual Field Defects', 'Cranial Nerve VI Palsy', 'CSF Disorders']
...
```mermaid
flowchart TD
    A[Headache ± Visual Symptoms] --> B[Fundoscopy: Papilloedema?]
    B -->|Yes| C[Urgent MRI/MRV Brain + Orbits]
    C --> D{Sinovenous Thrombosis?}
    D -->|Yes| E[Treat CVST — anticoagulation]
    D -->|No| F{Mass/Hydrocephalus?}
    F -->|Yes| G[Specific Rx]
    F -->|No| H[LP with OP measurement]
    H --> I{OP >25 cmH2O + Normal CSF?}
    I -->|Yes| J[IIH Diagnosis — Friedman 2013]
    I -->|No| K[Alternative Diagnosis]
    J --> L[Visual Assessment: Fields, VA, RAPD]
    L --> M{Vision Stable?}
    M -->|Yes| N[Medical: Weight loss + Acetazolamide]
    M -->|No/Fulminant| O[Surgical: ONSF or Shunt]
```
...
```mermaid
flowchart TD
    A[IIH Diagnosed] --> B{Visual Status}
    B -->|Stable, no vision loss| C[Medical: Weight Loss + Acetazolamide]
    C --> D[Response at 3-6 months]
    D -->|Improved| E[Continue + Taper]
    D -->|No response| F[Add Furosemide / Topiramate / Refer Surgery]
    B -->|Vision loss / Fulminant| G[Surgical: ONSF or Shunt]
    G --> H[Urgent Intervention within days]
    A --> I{Refractory Symptoms}
    I -->|Yes| J[Repeat LP, Bariatric Surgery, Stenting]
```
