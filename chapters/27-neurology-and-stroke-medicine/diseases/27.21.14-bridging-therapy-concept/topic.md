---
tags: [medicine, stroke-medicine, davidson, full-note, reperfusion, thrombectomy, bridging-therapy]
...
topic: Bridging therapy concept
...
```mermaid
flowchart TD
  A[Acute disabling stroke] --> B[NCCT excludes hemorrhage + CTA/MRA shows LVO]
  B --> C{Eligible for IV thrombolysis?}
  C -->|Yes| D[Give thrombolysis promptly]
  D --> E[Proceed immediately to thrombectomy pathway]
  C -->|No| F[Direct thrombectomy pathway if suitable]
  E --> G[Post-reperfusion monitoring]
  F --> G
```
...
```mermaid
flowchart LR
  A[LVO stroke] --> B{Eligible for IV thrombolysis?}
  B -->|Yes| C[Give lytic]
  C --> D[Proceed to thrombectomy]
  B -->|No| E[Direct thrombectomy]
  D --> F[Monitor]
  E --> F
```
