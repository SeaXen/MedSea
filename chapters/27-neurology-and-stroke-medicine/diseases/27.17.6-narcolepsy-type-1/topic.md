---
tags: [medicine, neurology, sleep, narcolepsy, fcps, mrcp]
chapter: Neurology
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 25: Neurology
topic: Narcolepsy Type 1
exam: [FCPS, MRCP Part 1, MRCP Part 2, PACES]
references:
  anatomy: [Hypothalamus, Lateral Hypothalamus]
  physiology: [Orexin/Hypocretin System]
  clinical: ['Davidson 24th Ed Ch25', 'ICSD-3 Criteria', 'AASM Practice Parameters', 'EFNS/ESRS Guidelines', 'PasTest', 'MRCP Notes']
related: ['Narcolepsy Type 2', 'Idiopathic Hypersomnia', 'Kleine-Levin Syndrome', 'Obstructive Sleep Apnoea', 'Cataplexy']
...
```mermaid
flowchart TD
    A[Genetic Susceptibility HLA-DQB1*06:02] --> B[Trigger Infection/Stress]
    B --> C[Autoimmune Attack on Orexin Neurons]
    C --> D[Loss of Lateral Hypothalamic Orexin Neurons >90%]
    D --> E[CSF Orexin 110 pg/mL]
    E --> F[Loss of Wake/Sleep Stability]
    F --> G[EDS + Cataplexy + Sleep Onset REM]
```
...
```mermaid
flowchart TD
    A[Suspected Narcolepsy] --> B[Detailed History + Epworth + Sleep Diary]
    B --> C[Exclude Other Causes - OSA/PLMD/Depression/Substance]
    C --> D[Nocturnal PSG]
    D --> E{MSLT Mean Sleep Latency 8 min + 2 SOREMPs?}
    E -->|Yes| F[CSF Orexin 110 pg/mL or HLA]
    E -->|No| G[Consider IH or Other]
    F --> H{Cataplexy?}
    H -->|Yes| I[Narcolepsy Type 1]
    H -->|No| J[Narcolepsy Type 2]
    F --> K{Hypocretin Low?}
    K -->|Yes| I
    K -->|No| J
```
...
```mermaid
flowchart TD
    A[Diagnosed NT1] --> B[Education + Behavioural Measures]
    B --> C[Pharmacotherapy for EDS]
    C --> D[Modafinil First-line]
    D --> E{Response?}
    E -->|Yes| F[Continue + Monitor]
    E -->|No| G[Add/Switch Armodafinil/Pitolisant/Solriamfetol]
    G --> H{Cataplexy Significant?}
    H -->|Yes| I[Sodium Oxybate - both EDS and Cataplexy]
    H -->|No| J[Stimulant Add-on]
    I --> K{Response?}
    K -->|Yes| F
    K -->|No| L[Second-line Venlafaxine/Clomipramine for Cataplexy]
```
