---
tags: [medicine, neurology, epilepsy, classification, fcps, mrcp]
chapter: Neurology
...
references:
  anatomy: []
  physiology: []
  clinical: ['Davidson 24th Ed Ch25', 'ILAE 2017 Classification (Fisher et al.)', 'ILAE 2017 Operational Classification', 'PasTest', 'MRCP Part 1/2 Notes']
related: ['Epilepsy & Seizure Disorders Hub', 'Seizure Classification & Diagnosis Hub', 'Status Epilepticus Management', 'Generalized Epilepsy Syndromes']
...
```mermaid
flowchart TD
    A[Seizure Suspected] --> B[Detailed History + Witness Account]
    B --> C{Onset Determined?}
    C -->|Yes| D{Focal vs Generalized}
    C -->|No| E[Unknown Onset → cEEG/Video-EEG]
    D -->|Focal| F{Awareness?}
    D -->|Generalized| G[Motor/Non-motor subtype]
    F -->|Aware| H[Focal Aware Seizure]
    F -->|Impaired| I[Focal Impaired Awareness]
    F -->|Unknown| J[Default: Impaired]
    F -->|Any| K{Focal→Bilateral Tonic-Clonic?}
    K -->|Yes| L[Document progression]
    G --> M[Specific generalized type]
    M --> N[ASM Choice + Syndrome Workup]
    L --> N
    H --> N
    I --> N
    E --> O[Reassess after EEG]
```
