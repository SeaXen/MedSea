---
tags: [medicine, neurology, peripheral-nerve, plexopathy, fcps, mrcp]
chapter: Neurology
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 25: Neurology
topic: Plexopathies (Brachial and Lumbosacral)
exam: [FCPS, MRCP Part 1, MRCP Part 2, PACES]
references:
  anatomy: [Brachial plexus C5-T1, Lumbosacral plexus L1-S4]
  physiology: []
  clinical: ['Davidson 24th Ed Ch25', "Adams and Victor's Principles of Neurology", 'Peripheral Nerve Society Guidelines', 'PasTest', 'MRCP Notes', 'Personal notes']
related: ['Peripheral Neuropathy Hub', 'Focal & Entrapment Neuropathies Hub', 'Parsonage-Turner Syndrome', 'Diabetic Amyotrophy']
...
```mermaid
flowchart TD
    A[Injury to Plexus] --> B{Type}
    B -->|Traction/Avulsion| C[Preganglionic: SNAP preserved, Horner ±, pseudomeningocele]
    B -->|Compression/Ischaemia| D[Postganglionic: SNAP absent, no Horner, Wallerian]
    C --> E[Permanent - Nerve Transfer]
    D --> F[Recovery Possible - Neurolysis/Rehab]
```
...
```mermaid
flowchart TD
    A[Multi-Nerve Weakness] --> B[MRI Plexus + EMG/NCS]
    B --> C{Preganglionic?}
    C -->|Yes - SNAP preserved, Horner| D[Root Avulsion: Myelography, Surgical Nerve Transfer]
    C -->|No| E[Postganglionic]
    E --> F{Context}
    F -->|Traumatic| G[Surgical + Rehab]
    F -->|Painless Progressive| H[CT chest + Mass workup: Neoplastic or Radiation]
    F -->|Acute Pain → Weakness| I[Parsonage: MRI exclude mass, Steroids/IVIG]
    F -->|DM + Thigh Pain| J[Diabetic Amyotrophy: MRI, DM control]
    F -->|Recurrent Multifocal| K[HNPP: PMP22 Genetic]
```
...
```mermaid
flowchart TD
    A[Plexopathy Confirmed] --> B{Cause?}
    B -->|Traumatic| C[Urgent MRI + Surgical]
    B -->|Neoplastic| D[Staging + Oncologic MDT]
    B -->|Radiation| E[Symptomatic + EMG myokymia]
    B -->|Parsonage| F[Analgesia ± Steroids/IVIG]
    B -->|Diabetic Amyotrophy| G[DM control + Pain Rx + Physio]
    B -->|HNPP| H[Counselling + Avoid Pressure]
    C --> I[Rehab ± Surgical Repair]
    D --> I
    E --> I
    F --> I
    G --> I
    ...
    I --> J{Recovery?}
    J -->|Yes| K[Maintenance]
    J -->|No| L[Nerve Transfer / Reconstruction]
```
...
mindmap
  root((Plexopathies))
    ...
    Aetiology
      ...
      Diabetic_Amyotrophy
    Localisation
      Preganglionic_SNAP_preserved
      Postganglionic_SNAP_lost
    Clinical
      Erb_Klumpke
      Pancoast_Horner
      Parsonage_Pain
...
```mermaid
flowchart TD
    A[Multi-Nerve Weakness] --> B[MRI Plexus + EMG]
    B --> C{Preganglionic?}
    C -->|SNAP preserved, Horner| D[Avulsion: Nerve Transfer]
    C -->|No| E[Postganglionic]
    E --> F{Context}
    F -->|Traumatic| G[Surgical + Rehab]
    F -->|Painless Progressive| H[CT chest: Pancoast or Radiation]
    F -->|Acute Pain → Weakness| I[Parsonage: Steroids/IVIG]
    F -->|DM + Thigh Pain| J[Diabetic Amyotrophy: DM Rx]
    F -->|Recurrent| K[HNPP: PMP22]
```
