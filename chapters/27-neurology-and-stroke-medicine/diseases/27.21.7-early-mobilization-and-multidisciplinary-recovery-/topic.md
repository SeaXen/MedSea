---
tags: [medicine, stroke-medicine, davidson, full-note, recovery-rehabilitation-prognosis, rehabilitation, mobilization]
...
topic: Early mobilization and multidisciplinary recovery planning
...
```mermaid
flowchart TD
  A[Stroke patient after acute stabilization] --> B[Assess medical stability + airway + oxygen + BP]
  B --> C[Assess strength, tone, balance, cognition, swallow]
  C --> D{Safe to begin mobilization?}
  D -->|Yes| E[Start graded activity: positioning -> sitting -> transfer -> stand/walk]
  D -->|Not yet| F[Treat instability and prevent bed-rest complications]
  E --> G[Multidisciplinary goal setting]
  G --> H[Daily review and progression]
  F --> H
  H --> I[Discharge planning + caregiver teaching]
```
...
```mermaid
flowchart LR
  A[Medically stable stroke patient] --> B[Assess vitals + neuro + balance + cognition]
  B --> C[Start bed mobility / sitting]
  C --> D[Progress to transfers / standing]
  D --> E[Advance to walking and ADL practice]
  E --> F[Review goals and discharge needs]
```
