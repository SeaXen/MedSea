---
tags: [medicine, stroke-medicine, davidson, full-note, stroke-unit-care, glucose, oxygen, temperature]
...
topic: Glucose, oxygen, and temperature control in stroke
...
```mermaid
flowchart TD
  A[Stroke patient arrives/stroke-unit review] --> B[Check glucose, SpO2, temperature]
  B --> C{Hypoglycaemia?}
  C -->|Yes| D[Correct immediately]
  C -->|No| E{Hyperglycaemia?}
  E -->|Yes| F[Controlled glucose management; avoid hypoglycaemia]
  E -->|No| G[Continue monitoring]
  G --> H{Hypoxia?}
  H -->|Yes| I[Give oxygen and treat cause]
  H -->|No| J[Avoid routine unnecessary oxygen]
  J --> K{Fever?}
  K -->|Yes| L[Antipyretics + search for infection/source]
  K -->|No| M[Continue stroke-unit optimization]
```
...
```mermaid
flowchart LR
  A[Stroke patient] --> B[Check glucose, SpO2, temp]
  B --> C[Correct hypoglycaemia immediately]
  B --> D[Manage hyperglycaemia carefully]
  B --> E[Give oxygen only if hypoxic]
  B --> F[Treat fever and identify source]
  C --> G[Repeat monitoring]
  D --> G
  E --> G
  F --> G
```
