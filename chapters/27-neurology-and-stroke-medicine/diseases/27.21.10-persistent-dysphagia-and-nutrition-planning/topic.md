---
tags: [medicine, stroke-medicine, davidson, full-note, recovery-rehabilitation-prognosis, dysphagia, nutrition]
...
topic: Persistent dysphagia and nutrition planning
...
```mermaid
flowchart TD
  A[Stroke patient] --> B[Keep NBM until swallow screen]
  B --> C{Screen passed?}
  C -->|Yes| D[Start appropriate oral intake and monitor]
  C -->|No| E[NBM + SLT assessment + hydration/nutrition plan]
  E --> F{Short-term recovery likely?}
  F -->|Yes| G[Texture modification / supervised oral trials / NGT if needed]
  F -->|No or prolonged unsafe intake| H[Consider longer-term enteral strategy e.g. PEG]
  G --> I[Monitor aspiration, weight, hydration, medication route]
  H --> I
```
...
```mermaid
flowchart LR
  A[Failed swallow screen] --> B[NBM + SLT review]
  B --> C{Safe modified oral intake possible?}
  C -->|Yes| D[Texture plan + supervision + monitor]
  C -->|No| E[Enteral feeding route]
  E --> F{Short-term?}
  F -->|Yes| G[NGT]
  F -->|No| H[Consider PEG]
```
