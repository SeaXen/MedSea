---
tags: [medicine, neurology, motor-neurone, flail-arm, flail-leg, fcps, mrcp]
chapter: Neurology
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 25: Neurology
topic: Flail Arm & Flail Leg Variants (MND)
exam: [FCPS, MRCP Part 1, MRCP Part 2, PACES]
references:
  anatomy: ['Cervical anterior horn cells (C5-T1)', 'Lumbosacral anterior horn cells (L2-S2)']
  physiology: ['LMN function', 'Denervation-reinnervation']
  clinical: ['Davidson 24th Ed Ch25', 'Gold Coast 2020', 'EFNS 2020', 'Adams & Victor', 'PasTest', 'MRCP Part 1/2 Notes']
related: ['Amyotrophic Lateral Sclerosis', 'Progressive Muscular Atrophy', 'Primary Lateral Sclerosis', 'Kennedys Disease', 'Multifocal Motor Neuropathy']
...
```mermaid
flowchart TD
    A[Genetic Susceptibility + Environmental] --> B[RNA Metabolism Defects]
    B --> C[TDP-43 Mislocalisation]
    C --> D[Protein Aggregation in Anterior Horn Cells]
    D --> E[Segmental Vulnerability]
    E -->|Cervical C5-C7| F[Flail Arm Syndrome]
    E -->|Lumbosacral L4-S1| G[Flail Leg Syndrome]
    E -->|Diffuse UMN+LMN| H[Classical ALS]
    F --> I[Progressive Proximal Arm Weakness]
    G --> J[Foot Drop, Distal Leg Weakness]
    H --> K[Rapid Widespread Weakness]
```
...
```mermaid
flowchart TD
    A[Progressive proximal arm weakness] --> B{UMN signs?}
    B -->|Yes| C[Possible ALS or PLS]
    B -->|No| D{LMN signs only}
    D --> E{Region involved}
    E -->|Both arms proximal| F[Flail Arm Syndrome]
    E -->|One or both legs distal| G[Flail Leg Syndrome]
    E -->|Symmetric proximal all limbs| H[Kennedy's, SMA, myopathy]
    F --> I{Examination of legs, bulbar, resp}
    I -->|All spared 12mo| J[Confirmed FAS]
    I -->|Spread| K[Possible progression to ALS]
    G --> L{Examination of arms, bulbar, resp}
    L -->|All spared 12mo| M[Confirmed FLS]
    L -->|Spread| N[Possible progression to ALS]
    J --> O[EMG + NCS + MRI + bloods]
    M --> O
    O --> P{NCS findings}
    P -->|Conduction block| Q[MMN - reconsider]
    P -->|Demyelinating| R[CIDP - reconsider]
    P -->|Normal sensory| S[Anterior horn cell]
    S --> T[Confirm FAS/FLS]
```
...
mindmap
  root((Flail Arm / Flail Leg))
    ...
    Diagnosis
      LMN_Only
      EMG_Chronic_Neurogenic
      NCS_No_Conduction_Block
      MRI_Normal
      Exclude_Mimics
    Differential
      MMN_Conduction_Block
      Kennedy_X_Linked
      Adult_SMA
      IBM_FDP
      Monomelic_Hirayama
    ...
    Prognosis
      Median_5_10y
      Better_than_ALS
      25_30_Progress_to_ALS
      Disability_Accumulates
```
...
> **Auto-generated PasTest/Mediscope-style scenario SBAs** grounded in the authored source. Each scenario tests a real clinical fact (triad, specific sign, contraindication, trial, first-line Rx) extracted from the topic. *Source: Ch 27: Neurology & Stroke — Flail Arm-Flail Leg Variants*

**Q1.** Which of the following features is most specific or characteristic of Flail Arm-Flail Leg Variants?
  ...
  > **Answer: A** — Progression:
  >
  > *Source:* ## History — Flail Arm
- **Onset:** Insidious, asymmetric proximal arm weakness
- **Symptoms:** Difficulty lifting arms overhead, reaching up, washing hair, getting dressed
- **Progression:** Spreads 

**Q2.** What is the most appropriate first-line therapy for Flail Arm-Flail Leg Variants?
  ...
  >
  > *Source:* **Occupational therapy:** Equipment provision (wheelchair, hoist), home modifications
