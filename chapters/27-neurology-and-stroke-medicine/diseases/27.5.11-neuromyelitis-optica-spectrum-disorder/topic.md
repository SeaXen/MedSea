---
tags: [medicine, neurology, demyelination, NMOSD, AQP4, MOG, optic-neuritis, LETM, fcps, mrcp]
chapter: Neurology
...
references:
  anatomy: []
  physiology: []
  clinical: ['Davidson 24th Ed Ch25', 'IPND 2015/2023 NMOSD Criteria', 'AQP4-IgG CBA', 'Eculizumab/Satralizumab Trials', 'PasTest', 'MRCP Part 1/2 Notes', 'Personal notes']
related: ['MOG Antibody-Associated Disease', 'Multiple Sclerosis (McDonald, DMTs)', 'Optic Neuritis', 'Transverse Myelitis', 'Acute Disseminated Encephalomyelitis']
...
```mermaid
flowchart TD
    A[Genetic Predisposition HLA-DPB1*05:01] --> B[AQP4-IgG Production]
    B --> C[BBB Disruption + AQP4-IgG Access]
    C --> D[Astrocyte Foot Process Binding]
    D --> E[Complement Activation CDC/ADCC]
    E --> F[Astrocyte Lysis + Necrosis]
    F --> G[Optic Nerve + Cord + Brainstem Injury]
    G --> H[LETM + ON + APS]
```
...
```mermaid
flowchart TD
    A[Optic Neuritis / Myelitis / APS] --> B[MRI Brain + Cord + Gd]
    B --> C{AQP4-IgG CBA}
    C -->|+ ≥1 core| D[NMOSD Confirmed]
    C -->|- ≥2 core| E{NMR meets criteria?}
    E -->|Yes| D
    E -->|No| F[Reconsider Dx - MS, MOGAD, vasculitis]
    D --> G[Initiate Acute Rx + Maintenance]
```
