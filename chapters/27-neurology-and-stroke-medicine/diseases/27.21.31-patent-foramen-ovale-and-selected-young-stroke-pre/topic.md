---
tags: [medicine, stroke-medicine, davidson, full-note, secondary-prevention, patent-foramen-ovale, young-stroke]
chapter: Stroke Medicine
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 29: Stroke medicine
heading: Secondary Prevention
topic_group: Vascular and cardiac source management
topic: Patent foramen ovale and selected young-stroke prevention issues
...
```mermaid
flowchart TD
  A[Young or cryptogenic ischaemic stroke] --> B[Confirm infarct pattern and exclude hemorrhage]
  B --> C[Search for common causes: AF, carotid disease, dissection, thrombophilia, vasculitis]
  C --> D{Alternative definite cause found?}
  D -->|Yes| E[Treat that cause; PFO may be incidental]
  D -->|No| F[Assess PFO significance on echo/shunt study]
  F --> G{High-risk PFO features / embolic context?}
  G -->|No| H[Medical secondary prevention]
  G -->|Yes| I[Consider specialist review for closure vs medical therapy]
  H --> J[Risk-factor and recurrence prevention]
  I --> J
```
...
```mermaid
flowchart LR
  A[Young or cryptogenic embolic stroke] --> B[Search for alternative causes]
  B --> C{Another definite cause found?}
  C -->|Yes| D[Treat true cause; PFO may be incidental]
  C -->|No| E[Assess PFO relevance and shunt features]
  E --> F{Likely relevant PFO?}
  F -->|No| G[Medical therapy]
  F -->|Yes| H[Consider closure vs tailored medical strategy]
  G --> I[Long-term secondary prevention]
  H --> I
```
