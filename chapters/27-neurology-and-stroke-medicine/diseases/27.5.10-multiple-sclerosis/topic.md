---
tags: [medicine, neurology, demyelination, multiple-sclerosis, fcps, mrcp]
chapter: Neurology
...
references:
  anatomy: []
  physiology: []
  clinical: ['Davidson 24th Ed Ch25', 'McDonald Criteria 2017', 'EAN/MAGNIMS 2021', 'AAN 2018', 'PasTest', 'MRCP Notes']
related: ['NMOSD', 'MOGAD', 'Optic Neuritis', 'Transverse Myelitis', 'ADEM']
...
```mermaid
flowchart TD
    A[Genetic Susceptibility + Env Trigger:EBV/Low VitD] --> B[Peripheral T-cell Activation]
    B --> C[CNS Migration via BBB]
    C --> D[Perivascular Inflammatory Infiltrate]
    D --> E[Myelin/Oligodendrocyte Damage]
    E --> F[Axonal Loss - Progressive Phase]
    F --> G[Atrophy: Brain/Spinal Cord]
```
...
```mermaid
flowchart TD
    A[Subacute CNS Event] --> B[Exclusion of Mimics]
    B --> C[MRI Brain + Cord with Gad]
    C --> D[McDonald 2017 Criteria]
    D --> E[DIS + DIT Demonstrated?]
    E -->|Yes| F[Diagnosis Confirmed]
    E -->|No| G[Follow-up / CIS]
```
...
```mermaid
mindmap
  root((Multiple Sclerosis))
    Diagnosis
      McDonald_2017
      DIS_DIT
      MRI_CSF_OCB
      Phenotypes_CIS_RR_SP_PP
    Pathology
      Demyelination_Axonal_Loss
      T_cells_B_cells
      EBV_VitD_Smoking
    Clinical
      Optic_Neuritis
