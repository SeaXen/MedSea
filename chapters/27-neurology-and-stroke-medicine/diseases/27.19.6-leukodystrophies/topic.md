---
tags: [medicine, neurology, genetic, leukodystrophy, fcps, mrcp]
chapter: Neurology
...
related: ['Mitochondrial Disorders', 'Multiple Sclerosis', 'Friedreichs Ataxia', 'Inborn Errors of Metabolism']
...
```mermaid
flowchart TD
    A[Genetic Mutation] --> B[Enzyme/Protein Defect]
    B --> C[Substrate Accumulation OR Loss of Function]
    C --> D[Myelin Damage / Loss]
    D --> E[White Matter Degeneration]
    E --> F[Clinical Phenotype]
```
...
```mermaid
flowchart TD
    A[White matter disease on MRI] --> B{Inheritance?}
    B -->|X-linked| C[Check VLCFA → ALD]
    B -->|AR| D[Lysosomal enzymes → MLD/Krabbe]
    B -->|AD| E[NOTCH3 → CADASIL]
    B -->|Sporadic| F[Genetic panel → Whole exome]
    C --> G[VLCFA Elevated]
    D --> H[Arylsulfatase A / Galactocerebrosidase]
    E --> I[NOTCH3 sequencing]
    F --> J[Leukodystrophy gene panel]
```
