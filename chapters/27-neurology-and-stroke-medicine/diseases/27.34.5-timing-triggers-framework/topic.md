---
tags: [medicine, neurology, davidson, vestibular, vertigo, timing-triggers, bedside-approach, localization, fcps, mrcp]
chapter: Neurology
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 28: Neurology
heading: Vestibular Disorders
topic_group: Approach to dizziness and vertigo
topic: Timing-triggers framework
exam: [FCPS, MRCP]
...
```mermaid
flowchart TD
A[Dizziness / vertigo complaint] --> B{Is it triggered?}
B -->|Yes| C{Specific positional trigger and brief seconds-long attacks?}
C -->|Yes| D[Consider BPPV]
C -->|No| E[Consider orthostatic or other movement-related causes]
B -->|No / spontaneous| F{Continuous for hours-days?}
F -->|Yes| G[Acute vestibular syndrome: vestibular neuritis vs central cause]
F -->|No| H{Recurrent episodes lasting minutes-hours?}
H -->|Yes| I[Consider Ménière disease or vestibular migraine]
H -->|No| J[Broader chronic dizziness / functional / multisensory review]
D --> K[Check red flags and nystagmus pattern]
G --> K
I --> K
K --> L{Neurological red flags or central ocular signs?}
L -->|Yes| M[Urgent neuro-evaluation and imaging]
L -->|No| N[Manage as likely peripheral/benign pattern with targeted confirmation]
```
...
```mermaid
flowchart LR
A[Duration] --> B[Trigger?]
B --> C[Auditory symptoms?]
C --> D[Neurological red flags?]
D --> E[Peripheral likely]
D --> F[Central concern]
```
