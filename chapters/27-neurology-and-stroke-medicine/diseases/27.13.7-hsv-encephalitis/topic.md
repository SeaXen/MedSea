---
tags: [medicine, neurology, cns-infections, encephalitis, hsv, fcps, mrcp]
chapter: Neurology
...
references:
  anatomy: ['Temporal lobes', 'Limbic system']
  physiology: ['Blood-brain barrier']
  clinical: ['Davidson 24th Ed Ch25', 'NICE NG240', 'IDSA 2008', 'EAN 2019', 'PasTest', 'MRCP Notes']
related: ['Autoimmune Encephalitis', 'Bacterial Meningitis', 'Brain Abscess', 'Cerebral Venous Thrombosis']
...
```mermaid
flowchart TD
    A[HSV Reactivation in Trigeminal Ganglion] --> B[Spread via CN V]
    B --> C[Temporal Lobe/Limbic System]
    C --> D[Neuronal Lysis + Inflammation]
    D --> E[Vasogenic Oedema + Necrosis]
    E --> F[Haemorrhagic Necrosis]
    F --> G[Permanent Brain Injury]
```
...
```mermaid
flowchart TD
    A[Suspected Encephalitis] --> B[ABC + IV Access]
    B --> C[Bloods + Cultures]
    C --> D[Empiric Aciclovir IMMEDIATELY]
    D --> E[CT Head - Exclude Mass Effect]
    E --> F[LP - CSF PCR for HSV]
    F --> G[MRI Brain DWI FLAIR]
    G --> H[EEG]
    H --> I[Continue/Stop Aciclovir Based on Results]
```
...
```mermaid
mindmap
  root((HSV Encephalitis))
    Pathology
      HSV1_Reactivation
      Trigeminal_Ganglion
      Temporal_Lobes
      Necrotising_Haemorrhagic
    Clinical
      Fever_AlteredConsciousness
      Behaviour_Personality
      Focal_Seizures
    Diagnosis
      CSF_PCR_GoldStandard
      MRI_Temporal_DWI_FLAIR
      EEG_PLEDs
      Repeat_LP_If_Negative
    Treatment
      Aciclovir_10mgkg_8h
      14_21_days
      Supportive_ICU
