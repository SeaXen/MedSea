---
tags: [medicine, stroke-medicine, davidson, full-note, recovery-rehabilitation-prognosis, rehabilitation]
...
topic: Stroke unit rehabilitation principles
...
```mermaid
flowchart TD
  A[Stroke patient medically stabilized] --> B[Early multidisciplinary assessment]
  B --> C[Swallow + mobility + cognition + communication + mood review]
  C --> D[Set individualized goals]
  D --> E[Begin structured therapy and complication prevention]
  E --> F[Review progress regularly]
  F --> G[Discharge planning + caregiver education + community follow-up]
```
...
```mermaid
flowchart LR
  A[Stroke stabilized] --> B[Assess swallow, mobility, cognition, communication]
  B --> C[Set rehab goals]
  C --> D[Begin multidisciplinary therapy + prevention plan]
  D --> E[Monitor progress and complications]
  E --> F[Discharge planning + community follow-up]
```
