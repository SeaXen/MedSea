---
tags: [medicine, stroke-medicine, davidson, full-note, stroke-unit-care, dvt, venous-thromboembolism]
chapter: Stroke Medicine
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 29: Stroke medicine
heading: Stroke Unit Care and Complications
topic_group: Common early complications
topic: Deep-vein thrombosis prevention after stroke
...
```mermaid
flowchart TD
  A[Stroke patient admitted] --> B[Assess mobility and bleeding risk]
  B --> C{Mobile and low VTE risk?}
  C -->|Yes| D[Encourage mobilization and hydration]
  C -->|No| E[Use stroke-appropriate prophylaxis strategy]
  E --> F{Bleeding risk high / ICH / recent thrombolysis?}
  F -->|Yes| G[Prefer mechanical methods first and reassess]
  F -->|No| H[Consider pharmacologic prophylaxis if safe]
  G --> I[Monitor for DVT/PE signs]
  H --> I[Monitor for DVT/PE signs and bleeding]
```
...
```mermaid
flowchart LR
  A[Immobile stroke patient] --> B[Assess bleed risk and stroke type]
  B --> C{Safe for anticoagulant prophylaxis?}
  C -->|No| D[Mechanical methods + mobilization + reassess]
  C -->|Yes| E[Add pharmacologic prophylaxis if appropriate]
  D --> F[Monitor for DVT/PE]
  E --> F
```
