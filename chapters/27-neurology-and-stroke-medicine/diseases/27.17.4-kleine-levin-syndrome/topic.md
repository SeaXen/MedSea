---
tags: [medicine, neurology, sleep, KLS, hypersomnia, fcps, mrcp]
chapter: Neurology
...
references:
  clinical: ["Davidson 24th Ed Ch25", "ICSD-3 criteria", "Arnulf et al. 2018 Lancet Neurol", "PasTest", "MRCP Notes"]
related: ['Narcolepsy Type 1', 'Narcolepsy Type 2', 'Insomnia', 'Bipolar Disorder']
...
```mermaid
flowchart TD
    A[Trigger - Infection] --> B[Autoimmune/hypocretin dysfunction]
    B --> C[Hypothalamic + Thalamic dysfunction]
    C --> D[Hypersomnia + Cognitive + Behavioural]
    D --> E[Episode lasting days-weeks]
    E --> F[Recovery, recurrent episodes]
```
...
```mermaid
flowchart TD
    A[KLS diagnosed] --> B[Education + Reassurance]
    B --> C[Trigger avoidance]
    C --> D{Episodes frequent?}
    D -->|No| E[Supportive]
    D -->|Yes| F[Prophylactic - Lithium first]
    F --> G{Responsive?}
    G -->|Yes| H[Continue]
    G -->|No| I[Valproate / Lamotrigine / Topiramate]
    I --> J{Responsive?}
    J -->|No| K[IV methylprednisolone / IVIG for severe]
```
