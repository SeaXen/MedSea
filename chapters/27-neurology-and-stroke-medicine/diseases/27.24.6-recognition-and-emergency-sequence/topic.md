---
tags: [medicine, neurology, davidson, epilepsy, status-epilepticus, emergency, fcps, mrcp]
chapter: Neurology
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 28: Neurology
heading: Epilepsy
topic_group: Status epilepticus
topic: Recognition and emergency sequence
exam: [FCPS, MRCP]
...
```mermaid
flowchart TD
  A[Seizure suspected] --> B{Ongoing >5 min or repeated without recovery?}
  B -->|Yes| C[Treat as status epilepticus]
  C --> D[ABC, lateral position, oxygen, monitors, IV access]
  D --> E[Capillary glucose + bloods]
  E --> F{Hypoglycaemia or immediate reversible cause?}
  F -->|Yes| G[Correct immediately]
  F -->|No| H[Give benzodiazepine]
  G --> H
  H --> I{Seizure stops?}
  I -->|Yes| J[Monitor, search cause, prevent recurrence]
  I -->|No| K[Give second-line antiseizure drug]
  K --> L{Seizure persists?}
  L -->|Yes| M[Refractory SE: ICU, intubation consideration, anesthetic therapy, EEG]
  L -->|No| J
```
...
```mermaid
flowchart LR
  A[Recognize ongoing seizure] --> B[Stabilize ABC]
  B --> C[Check glucose and bloods]
  C --> D[Benzodiazepine]
  D --> E{Stopped?}
  E -->|Yes| F[Observe + cause search]
  E -->|No| G[Second-line ASM]
  G --> H{Stopped?}
  H -->|Yes| F
  H -->|No| I[Refractory SE: ICU + EEG + anesthetic therapy]
```
