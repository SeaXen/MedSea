---
tags: [medicine, stroke-medicine, davidson, full-note, reperfusion, LVO, transfer-pathway]
chapter: Stroke Medicine
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 29: Stroke medicine
heading: Reperfusion Therapy
topic_group: Mechanical thrombectomy
topic: Large-vessel occlusion transfer pathway
...
```mermaid
flowchart TD
  A[Suspected disabling stroke] --> B[Prehospital LVO suspicion / stroke alert]
  B --> C[Urgent NCCT ± CTA at first capable centre]
  C --> D{LVO confirmed or highly suspected?}
  D -->|No| E[Standard stroke pathway]
  D -->|Yes| F[Assess IV thrombolysis eligibility]
  F --> G[Give lytic if indicated and no delay]
  G --> H[Activate thrombectomy-centre transfer immediately]
  F --> H
  H --> I[Transport with handover + imaging + onset details]
  I --> J[Arrival at thrombectomy-capable centre and endovascular activation]
```
...
```mermaid
mindmap
  root((LVO transfer pathway))
    Recognition
      Severe cortical syndrome
      Brainstem/BAO syndrome
    Imaging
      NCCT
      CTA/MRA
    Reperfusion
      Lytic if eligible
      Thrombectomy centre transfer
    Logistics
      Drip-and-ship
      Mothership
      Handover
...
```mermaid
flowchart LR
  A[Disabling stroke] --> B[CT ± CTA]
  B --> C{LVO?}
  C -->|Yes| D[Lytic if eligible]
  D --> E[Early transfer activation]
  C -->|Yes| E
  E --> F[Send imaging + onset + clinical details]
  F --> G[Thrombectomy centre arrival]
```
...
> **Auto-generated PasTest/Mediscope-style scenario SBAs** grounded in the authored source. Each scenario tests a real clinical fact (triad, specific sign, contraindication, trial, first-line Rx) extracted from the topic. *Source: Ch 27: Neurology & Stroke — Large-vessel occlusion transfer pathway*

**Q1.** What is the most appropriate first-line therapy for Large-vessel occlusion transfer pathway?

  - **A.** Give IV thrombolysis if indicated and it will not delay transfer
  - **B.** An advanced/surgical therapy reserved for refractory disease
  - **C.** Symptomatic treatment only, no disease-modifying therapy
  - **D.** Empiric broad-spectrum therapy without specific indication

  > **Answer: A** — Give IV thrombolysis if indicated and it will not delay transfer
  >
  > *Source:* Give IV thrombolysis if indicated and it will not delay transfer.
