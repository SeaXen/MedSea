---
tags: [medicine, neurology, davidson, epilepsy, first-seizure, provoked-seizure, unprovoked-seizure, fcps, mrcp]
chapter: Neurology
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 28: Neurology
heading: Epilepsy
topic_group: Evaluation of a first seizure
topic: Provoked vs unprovoked seizure
exam: [FCPS, MRCP]
...
```mermaid
flowchart TD
  A[First seizure] --> B[Confirm true seizure vs mimic]
  B --> C[ABC if ongoing / emergency review]
  C --> D[History + witness account + exam]
  D --> E[Bedside glucose, ECG, labs]
  E --> F{Immediate provoking factor found?}
  F -->|Yes| G[Classify as likely provoked / acute symptomatic]
  F -->|No| H[Evaluate for unprovoked seizure]
  G --> I[Treat underlying cause + recurrence prevention]
  H --> J[EEG + neuroimaging + recurrence risk assessment]
  J --> K[Decide need for long-term ASM and counseling]
```
...
```mermaid
flowchart LR
  A[First seizure] --> B[Confirm seizure]
  B --> C[History, witness, exam, ECG, glucose, labs]
  C --> D{Acute provoking factor present?}
  D -->|Yes| E[Provoked seizure → treat cause]
  D -->|No| F[Unprovoked seizure pathway]
  F --> G[EEG + MRI/CT + recurrence counseling]
```
