---
tags: [medicine, neurology, paraneoplastic, peripheral nerve hyperexcitability, fcps, mrcp]
chapter: Neurology
...
related: ['Lambert-Eaton Myasthenic Syndrome', 'Paraneoplastic Cerebellar Degeneration', 'Cancer Screening in PNS']
...
```mermaid
flowchart TD
    A[VGKC-complex Antibody] --> B[Peripheral Motor Nerve Terminal]
    B --> C[Reduced K+ Efflux]
    C --> D[Delayed Repolarisation]
    D --> E[Repetitive Firing]
    E --> F[Continuous Muscle Fibre Activity]
    F --> G[Myokymia, Cramps, Stiffness]
    A --> H[Autonomic Nerves]
    H --> I[Hyperhidrosis, Tachycardia]
```
...
```mermaid
flowchart TD
    A[Myokymia + Cramps + Stiffness] --> B[Clinical suspicion of PNH]
    B --> C[EMG: Spontaneous single units, doublets, multiplets, neuromyotonic discharges]
    C --> D[Antibody: Anti-VGKC-complex - CASPR2, LGI1, Contactin-2]
    D --> E[Tumour Screen: CT Chest +/ Abdomen/Pelvis - Thymoma, SCLC, Lymphoma]
    E --> F[Treat Symptom + Tumour]
```
...
```mermaid
flowchart TD
    A[PNH Confirmed] --> B[Tumour Screen - CT Chest]
    B --> C{Tumour Found?}
    C -->|Yes| D[Oncology + Carbamazepine +/ Immuno]
    C -->|No| E[Idiopathic: Carbamazepine +/ Pred]
    D --> F[Monitor]
    E --> F
    F --> G{Controlled?}
    G -->|Yes| H[Continue + Surveillance 3-6m]
    G -->|No| I[Escalate: IVIG, Rituximab, PLEX]
```
