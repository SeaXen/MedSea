---
tags: [medicine, neurology, motor-neurone, als-mimics, fcps, mrcp]
chapter: Neurology
...
references:
  anatomy: ['Anterior horn cells', 'Corticospinal tracts', 'Peripheral nerve roots']
  physiology: ['Motor unit', 'Nerve conduction', 'Denervation-reinnervation']
  clinical: ['Davidson 24th Ed Ch25', 'Gold Coast 2020', 'EFNS Guidelines on ALS Mimics', 'NICE NG42', 'Adams & Victor Principles of Neurology', 'PasTest', 'MRCP Part 1/2 Notes']
related: ['Amyotrophic Lateral Sclerosis', 'Multifocal Motor Neuropathy', 'Kennedy Disease', 'Primary Lateral Sclerosis', 'Progressive Muscular Atrophy', 'Inclusion Body Myositis', 'Cervical Myelopathy']
...
```mermaid
flowchart TD
    A[Clinical ALS Phenotype] --> B{Pathophysiology?}
    B --> C[Anterior horn cell loss]
    B --> D[Myelin - motor only]
    B --> E[Axon - motor only]
    B --> F[Muscle fibre loss]
    B --> G[Corticospinal tract compression]
    B --> H[Metabolic/toxic injury]
    B --> I[Autoimmune - paraneoplastic]
    C --> J[ALS / Kennedy / SMA / post-polio]
    D --> K[MMN / MADSAM / CIDP]
    E --> L[Lead / porphyria / MAMA]
    F --> M[IBM / polymyositis / DM]
    G --> N[Cervical myelopathy / syrinx / Chiari]
    H --> O[B12 / Cu / endocrine / heavy metal]
    I --> P[Anti-Hu / anti-Yo / anti-Ri]
```
...
```mermaid
flowchart TD
    A[Progressive Motor Weakness] --> B{Family Hx/Sex/Age clues}
    B -->|Male, gynaecomastia| C[Kennedy's screen]
    B -->|X-linked female carrier| D[Carrier testing]
    B -->|Adult onset, SMN1| E[SMA 4 genetic]
    A --> F{NCS Findings}
    F -->|Motor conduction block| G[MMN/MADSAM]
    F -->|Demyelinating, sensory| H[CIDP]
    F -->|Normal sensory, low CMAP| I[ALS/MND variant]
    F -->|Myopathic units| J[IBM/Myopathy]
    A --> K{CK}
    K -->|>5x| J
    K -->|Normal/mild| I
    A --> L{MRI Brain/Spine}
    L -->|Cord compression| M[Surgical referral]
    L -->|Pyramidal T2 hyperintensity| I
    L -->|Normal| I
    A --> N{Sensory signs?}
    N -->|Yes| O[CIDP/myelopathy/B12]
    N -->|No| P[Anterior horn cell]
    P -->|UMN+LMN| Q[Gold Coast 2020 ALS]
    P -->|Pure LMN slow| R[Kennedy/SMA/MMN]
    P -->|Pure UMN >4y| S[PLS]
    A --> T{Bulbar onset}
    T -->|+gynaecomastia| C
    T -->|No| I
    A --> U{Endocrine/Toxic screen}
    U -->|TFTs/Ca/Pb/B12/Cu abnormal| V[Treat cause]
    U -->|Normal| I
```
...
mindmap
  root((ALS Mimics))
    Treatable_First
      MMN_IVIG_No_Steroids
      CIDP_IVIG_Steroids
      Myelopathy_Surgery
      Endocrine_Treat
      B12_Copper_Replace
      Paraneoplastic_Tumour
      Lead_Chelation
    Motor_Only_LMN
      MMN
      MAMA
    ...
    Investigations
      ...
      EMG_Neurogenic_Myopathic
      MRI_Brain_Spine
      CK_High_Low
      Anti_GM1_cN1A_Hu
      Genetic_SMN1_AR_C9orf72
      Lead_Level
    Diagnosis_Rules
      Sensory_Not_ALS
      Conduction_Block_MMN
      MRI_Compression
      Gynaecomastia_Kennedy
      Finger_Flexor_IBM
      Treatable_First_Always
```
