---
tags: [medicine, neurology, functional, visual, fcps, mrcp]
chapter: Neurology
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 25: Neurology
topic: Functional Visual Symptoms
exam: [FCPS, MRCP Part 1, MRCP Part 2, PACES]
references:
  clinical: ["Davidson 24th Ed Ch25", "Stone et al. FND chapter", "PasTest", "MRCP Notes", "neurosymptoms.org"]
related: ['Functional Weakness', 'Functional Movement Disorders']
...
```mermaid
flowchart TD
    A[Trigger/Stress] --> B[Abnormal Visual Predictions]
    B --> C[Altered Visual Awareness]
    C --> D[Functional Visual Loss]
    D --> E[Reinforcement - attention/fear]
```
...
```mermaid
flowchart TD
    A[Visual symptoms] --> B[History: FND clues]
    B --> C[Full neuro-ophthalmic exam]
    C --> D[Visual field: Goldmann tubular?]
    D --> E[OKN test preserved?]
    E --> F[Pupils preserved?]
    F --> G[Screening MRI, OCT, VEP]
    G --> H{Organic lesion?}
    H -->|No| I[Diagnose FVD positively]
    H -->|Yes| J[Co-existing FVD possible]
    I --> K[Explanation + Reassurance + PT]
```
