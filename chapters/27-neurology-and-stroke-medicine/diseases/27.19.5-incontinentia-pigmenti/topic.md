---
tags: [medicine, neurology, genetic, neurocutaneous, fcps, mrcp]
chapter: Neurology
topic: Incontinentia Pigmenti
exam: [FCPS, MRCP Part 1, MRCP Part 2, PACES]
references:
  clinical: ['Davidson 24th Ed Ch25', 'Adams & Victor Principles of Neurology', 'GeneReviews', 'DermNet NZ']
related: ['Tuberous Sclerosis Complex', 'Von Hippel-Lindau', 'Neurofibromatosis', 'Phakomatoses']
...
```mermaid
flowchart TD
    A[IKBKG Mutation] --> B[NF-κB Pathway Defect]
    B --> C[Impaired Apoptosis of Damaged Cells]
    C --> D[Tissue Dysgenesis]
    D --> E[Multi-system Phenotype]
```
...
```mermaid
flowchart TD
    A[Newborn with Vesicular Rash along Blaschko] --> B{Sex?}
    B -->|Female| C[Suspect IP]
    B -->|Male| D[Klinefelter/Somatic Mosaicism]
    C --> E[Genetic Testing: IKBKG deletion/mutation]
    E -->|Positive| F[Diagnosis Confirmed]
    E -->|Negative| G[Sequence + Skin Biopsy]
    F --> H[MRI Brain + Ophthalmology + Skeletal Survey]
```
