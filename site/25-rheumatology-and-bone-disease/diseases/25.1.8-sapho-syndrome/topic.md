---
tags: [medicine, rheumatology, davidson, sapho, bone, skin, fcps, mrcp]
...
topic_group: Other musculoskeletal syndromes
...
```mermaid
flowchart TD
    A[SAPHO Diagnosis] --> B[Step 1: NSAIDs\n+ Topical Rx for skin]
    B --> C{Response?}
    C -->|Yes| D[Continue\nMonitor]
    C -->|No| E[Step 2: Add Antibiotics\nDoxycycline, Azithromycin, Cotrimoxazole]
    E --> F{Response?}
    F -->|Yes| D
    F -->|No| G[Step 3: Systemic Steroids\nPred 0.5-1 mg/kg → taper]
    G --> H{Response?}
    H -->|Yes| D
    H -->|No| I[Step 4: csDMARDs\nMTX, SSZ, AZA]
    I --> J{Response?}
    J -->|Yes| D
    J -->|No| K[Step 5: Biologics\nTNF-i, IL-17i, IL-1i, JAKi]
    K --> L[Refractory: Bisphosphonates IV, Surgery]
```
...
mindmap
  root((SAPHO Syndrome))
    ...
    Trigger
      ...
      Bone_PCR_positive
    Imaging
      Bull_head_sign_bone_scan
      Hyperostosis_CT
      Oedema_MRI
    Diagnosis
      Inclusion_skin_bone
      Exclude_infection_malignancy
      Bone_biopsy
    Differential
      Infectious_osteomyelitis
      TB
      AS
      PsA
      Paget
