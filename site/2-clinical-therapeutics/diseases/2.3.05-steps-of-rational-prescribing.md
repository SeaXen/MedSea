...
## 3. 3. Mermaid Algorithm — WHO 6-Step Cycle

```mermaid
flowchart TD
    A[1. Define patient's problem
Diagnosis + goals] --> B[2. Specify therapeutic objective
Cure / Control / Prevent / Palliate]
    B --> C[3a. Choose standard P-drug from guideline]
    C --> D{P-drug suitable for
this patient?}
    D -->|Yes| E[3b. Verify dose, route, duration]
    D -->|No| F[3c. Choose alternative P-drug]
    F --> D
    E --> G[4. Write the prescription
Generic, legible, signed]
    G --> H[5. Give information
name, indication, dosing, AEs, what to do if missed]
    H --> I[6. Monitor
efficacy and safety at planned visit]
    I --> J{Therapy effective
AND safe?}
    J -->|Yes| K[Continue OR Stop if course complete]
    J -->|No - failure| A
    J -->|No - ADR| L[Stop / Reduce / Switch]
    L --> A
```

## 4. 4. Comparison Tables

### 1. 4.1 Common Errors at Each Step
...
### 2. 4.2 SMART Therapeutic Objective
...
### 3. 4.3 Information to Give at Step 5 (Mnemonic: **"DIM AWARE"**)
