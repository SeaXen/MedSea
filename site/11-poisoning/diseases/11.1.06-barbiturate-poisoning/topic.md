...
```mermaid
mindmap
  root((Barbiturate Poisoning))
    Mechanism
      GABA-A Duration ↑ (Mimics GABA)
      NO Ceiling Effect
    Clinical
      Deep Coma (GCS 3)
      Absent Reflexes
      ...
      DIC
    Phenobarbital Specific
      Long Half-life (60-120h)
      Weak Acid (pKa 7.2)
...
```mermaid
flowchart TD
  A[Deep Coma, Severe Resp Depression,\nHypotension, Hypothermia] --> B[Barbiturate Poisoning]
  B --> C[ABCDE: Intubate Early\nMechanical Ventilation\nFluids + Vasopressors\nActive Warming]
  C --> D{Phenobarbital?}
  D -->|Yes| E[MDAC 1g/kg q4-6h\nUrinary Alkalinization\npH 7.5-8.0 + KCl\nHemodialysis if Severe]
  D -->|No| F[Supportive Care Only\nMDAC if Recent/Large Dose]
  E --> G[Monitor: CK, Coagulation,\nRenal, Electrolytes,\nSkin for Bullae]
  F --> G
  G --> H{Prolonged Ventilation?}
  H -->|Yes| I[Tracheostomy Consideration\nDVT/Stress Ulcer Prophylaxis\nPressure Area Care]
  H -->|No| J[Wean Ventilation\nObserve 24h Post-Extubation]
```
