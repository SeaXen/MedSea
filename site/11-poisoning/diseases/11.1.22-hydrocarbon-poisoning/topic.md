...
```mermaid
mindmap
  root((Hydrocarbon Poisoning))
    Main Danger
      Aspiration Pneumonitis
      Chemical Injury → Surfactant Loss → Hypoxia
    Contraindications
      No Emesis
      No Lavage
      No Charcoal
    Observation
      CXR at 6h (Lags 2-6h)
      Asymptomatic + Normal CXR = Discharge
    Specific Types
      Aliphatic: Kerosene, Gasoline → Aspiration Main
      Halogenated: CCl₄ → Hepatic, Cardiac Sensitization
      Aromatic: Benzene → Aplastic Anemia; Toluene → Renal, Neuropathy
    Complications
      ARDS
      Secondary Pneumonia (Abx if Proven)
      VF (Cardiac Sensitization)
