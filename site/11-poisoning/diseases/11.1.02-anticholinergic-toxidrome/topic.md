...
```mermaid
mindmap
  root((Anticholinergic Toxidrome))
    Mechanism
      Muscarinic Blockade
      Central + Peripheral
    Clinical
      ...
      Hyperthermia
    Agents
      Pure: Atropine, Scopolamine, Benztropine
      Mixed: TCA, Antihistamines, Antipsychotics, Plants
    DDx
      Sympathomimetic (Wet, Hyperreflexia)
      Serotonin (CLONUS, Hyperreflexia, Wet)
      NMS (Rigidity, Bradykinesia, Days)
      Cholinergic (Opposite)
    Management
      Supportive (Cooling, Benzo, Fluids)
      Physostigmine (Restricted)
      Charcoal (Longer Window)
    Physostigmine
      Indications: Severe refractory
      Contraindications: QRS, Brady, Asthma, Obstruction
      Dose: 0.5-1mg IV
```
...
```mermaid
flowchart TD
  A[Hot, Dry, Delirious, Tachycardic, Mydriasis, Urinary Retention] --> B[Anticholinergic Toxidrome]
  B --> C[ABCDE + Cooling + Benzodiazepines]
  C --> D{Severe Refractory<br/>Agitation/Hyperthermia/Seizures?}
  D -->|No| E[Observation 6h<br/>Psych Assessment]
  D -->|Yes| F{Contraindications?\nQRS>100, Brady, Asthma,\nBowel/Urinary Obstruction}
  F -->|Yes| G[DO NOT GIVE PHYSOSTIGMINE\nContinue Supportive\nICU Admission]
  F -->|No| H[Physostigmine 0.5-1mg IV\nOver 2-5 min\nMonitor ECG]
  H --> I[Repeat if needed q20-30min\nMax 2mg/hr]
  G --> J[ICU: Cooling, Benzo,\nFoley, Monitor]
  I --> J
  E --> K[Discharge if Resolved\n+ Psych Safe]
```
