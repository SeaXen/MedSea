---
tags: [hepatology, nafld, nash, hcc, surveillance, risk, fcps, mrcp]
davidson_chapter: Chapter 24
status: full-fcps-mrcp-note
priority: high
exam_relevance: "NAFLD-related HCC risk, surveillance criteria, non-cirrhotic HCC - FCPS/MRCP emerging concept"
see_also: ["Non-Alcoholic Fatty Liver Disease/NAFLD Spectrum (NAFL vs NASH)", "Non-Alcoholic Fatty Liver Disease/NAFLD Risk Factors and Pathophysiology", "Non-Alcoholic Fatty Liver Disease/NAFLD Diagnosis (FIB-4, NFS, ELF, FibroScan)", "Non-Alcoholic Fatty Liver Disease/NAFLD Management (Lifestyle, Pharmacotherapy, Bariatric Surgery)", "Liver Tumours/HCC (Hepatocellular Carcinoma)"]
created: 2025-06-13
modified: 2025-06-13
---

# NAFLD-Related HCC Risk and Surveillance
...
## NAFLD-Related HCC: Key Concepts
...
## Non-Cirrhotic HCC in NAFLD: Emerging Concept
...
## HCC Risk Factors in NAFLD (Independent)
...
## Surveillance Protocols by Guidelines
...
## Surveillance Modality & Technique
...
## HCC Surveillance Algorithm (NAFLD)

```mermaid
flowchart TD
    A[NAFLD Patient] --> B{Fibrosis Stage}
    B -->|F4 Cirrhosis| C[6-Monthly US ± AFP — LIFELONG]
    B -->|F3 Advanced Fibrosis| D[6-Monthly US ± AFP — CONSIDER]
    B -->|F2| E[No Routine Surveillance — Risk Stratify]
    B -->|F0-F1| F[No Surveillance]
    C --> G[If US Inadequate → CT/MRI]
    D --> G
    G --> H[If Lesion Found → LI-RADS Assessment]
    H --> I[LR-1/2: Continue Surveillance]
    H --> J[LR-3: 3-6mo FU]
    H --> K[LR-4: MDT → Treat as HCC]
    H --> L[LR-5: Treat as HCC]
    H --> M[LR-M: Biopsy / MDT]
```

---

## Post-SVR HCV with NAFLD Background
...
```mermaid
mindmap
  root((NAFLD HCC Surveillance))
    Risk by Fibrosis
      F0-F1: <0.1%/yr → No Surveillance
      F2: 0.1-0.3%/yr → No Routine
      F3: 0.5-1.5%/yr → EASL: 6m US±AFP
      F4: 1-3%/yr → MANDATORY 6m US±AFP
    Non-Cirrhotic HCC
      ~30% of NAFLD HCC
      F2-F3 + DM + Obesity + Male + PNPLA3
    Risk Factors
      Cirrhosis (10-20x)
      F3 (5-10x)
      DM (2-4x)
    ...
    Surveillance
      ...
      AFP >200 + Imaging = HCC
    Guidelines
      EASL: F3+F4 Surveillance
      AASLD: F4 Only, F3 Consider
      NICE: F4 Only, F3 Consider
    Post-SVR
      F4: Lifelong
      F3: Consider
```
