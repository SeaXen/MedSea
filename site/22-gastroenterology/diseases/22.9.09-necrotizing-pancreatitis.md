...
## 16. Infected or complicated necrosis
...
```mermaid
mindmap
  root((Necrotizing Pancreatitis))
    Definition
      Non-viable pancreatic/peripancreatic tissue
    Key Features
      Organ failure, CT necrosis, persistent SIRS
    Diagnosis
      Contrast CT for necrosis detection
    Management
      Supportive = sterile; step-up = infected
    Complications
      Infected necrosis, organ failure, collections
```

## 25. Flowchart
```mermaid
flowchart TD
  A[Severe acute pancreatitis] --> B[Contrast CT shows necrosis?]
  B -- Yes --> C[Classify sterile vs infected]
  B -- No --> D[Treat as interstitial edematous]
  C --> E[Manage by necrosis type]
  D --> E
```
