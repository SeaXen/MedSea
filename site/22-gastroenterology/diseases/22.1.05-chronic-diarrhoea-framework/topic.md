...
```mermaid
flowchart TD
A[Chronic diarrhoea >4 weeks] --> B{Type?}
B -- Watery --> C[Functional / secretory / osmotic pathway]
B -- Fatty --> D[Malabsorption pathway]
B -- Inflammatory --> E[IBD / colitis pathway]
C --> F[Check red flags + baseline tests]
D --> F
E --> F
F --> G[Targeted tests: coeliac serology, calprotectin, stool tests, colonoscopy]
```
