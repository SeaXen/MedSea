...
```mermaid
mindmap
  root((Protein-Losing Enteropathy))
    Definition
      PLE = GI protein loss > synthesis → hypoalbuminaem...
    Key Features
      Key clue: normal LFTs + normal renal function + lo...
    Diagnosis
      Causes: lymphangiectasia, IBD, coeliac, infection,...
    Management
      Alpha-1-antitrypsin stool clearance = diagnostic...
    Complications
      Management: treat cause + low-fat high-protein + M...
```

## 13. Flowchart
```mermaid
flowchart TD
  A[Protein-Losing Enteropathy Presentation] --> B{Key Diagnostic Feature?}
