...
```mermaid
mindmap
  root((Pancreatic Exocrine Insufficiency))
    Definition
      Inadequate enzymes → maldigestion → steatorrhoea
    Key Features
      Steatorrhoea, weight loss, fat-soluble vitamin deficiency
    Diagnosis
      Fecal elastase, clinical context, pancreatic history
    Management
      PERT dosing with meals + acid suppression + vitamins
    Complications
      Malnutrition, sarcopenia, osteoporosis, poor quality of life
```

## 23. Flowchart
```mermaid
flowchart TD
  A[Steatorrhoea + weight loss + pancreatic history] --> B[Fecal elastase <200?]
  B -- Yes --> C[Start PERT + monitor response]
  B -- No --> D[Consider other malabsorption causes]
  C --> E[Symptom control + nutritional repletion]
  D --> E
```
