...
### 1. Section 1: HAI — Definition, Scope, and Burden
...
### 2. Section 2: Catheter-Associated Urinary Tract Infection (CAUTI)
...
### 3. Section 3: Surgical Site Infection (SSI)
...
### 4. Section 4: Ventilator-Associated Pneumonia (VAP) and Ventilator-Associated Events (VAE)
...
### 5. Section 5: Central Line-Associated Bloodstream Infection (CLABSI)
...
### 6. Section 6: *Clostridioides difficile* Infection (CDI)
...
### 7. Section 7: Hospital-Acquired Pneumonia (HAP) and Non-Ventilator HAP
...
### 8. Section 8: HAI Surveillance — Methodology and Metrics
...
### 9. Section 9: The Big 5 HAI at a Glance — Comparison Table
...
### 10. Section 10: Quality Improvement and Implementation Science
...
### 11. Section 11: Special Populations and Settings
...
### 12. Section 12: Environmental Cleaning, Engineering Controls, and Novel Technologies
...
### 13. Section 13: HAI Economics and Policy
...
```mermaid
mindmap
  root((HAI Surveillance & Prevention))
    Definitions
      HAI 48h rule
      SSI 30d 90d rule
      Day 1 is admission day
    Big 5 HAIs
      CAUTI
        Most common
        Avoid + early removal
      ...
      SSI
        ...
        Prophylaxis 60 min before
        6 H bundle
        24h stop
      VAP VAE
        MV 48h + infiltrate + signs
        HOB 30-45
        Sedation vacation
        Oral CHG
        SSD cuff pressure
      CLABSI
        Central line 48h + BSI
        HMCO D bundle
        Subclavian preferred
        Line removal for Staph Candida
      CDI
        3 stools + toxin or PCR
        Vancomycin or fidaxomicin
        FMT VOWST REBYOTA
        Soap water hand wash
        Bleach cleaning
    Surveillance
      NHSN ECDC GLASS
      DAIR device days
    ...
    Antimicrobial Stewardship
      Restrict clindamycin FQ cephalosporins
      De-escalation
      DDD DOT
    Bundles
...
## 9. 9. Flowchart — HAI Investigation and Bundle Implementation

```mermaid
flowchart TD
    A[Admit Patient] --> B{Device or Procedure Needed?}
    B -->|No| C[Reassess Daily]
    B -->|Yes| D[Insert with Aseptic Bundle]
    D --> E[Daily Review Necessity]
    E --> F{Still Needed?}
    F -->|Yes| G[Maintenance Bundle]
    F -->|No| H[Remove Device Early]
    G --> I{New Fever Infection Signs?}
    I -->|No| E
    I -->|Yes| J[Apply HAI Criteria]
    J --> K[Bundle Type?]
    K -->|CAUTI| L[Urine culture + replace Foley + targeted abx]
    K -->|CLABSI| M[Paired BC + line removal + empirical vanco + GNR cover]
    K -->|VAP| N[Respiratory sample + anti-pseudomonal + MRSA cover]
    K -->|SSI| O[Inspect + drain + culture + targeted abx]
    K -->|CDI| P[Stool testing + stop offending abx + vanco or fidaxomicin]
    L --> Q[Surveillance + Feedback]
    M --> Q
    N --> Q
    O --> Q
    P --> Q
    Q --> R[PDSA Cycle]
    R --> S[Bundle Compliance Audit]
    S --> T[Improved HAI Rate]
```

---

## 10. 10. Flowchart — SSI Prophylaxis Decision

```mermaid
flowchart TD
    A[Patient for Surgery] --> B{Indication for Prophylaxis?}
    B -->|Clean implant or clean-contaminated| C[Give Prophylaxis]
    B -->|Clean no implant| D[Consider No Prophylaxis]
    B -->|Contaminated dirty| E[Therapeutic Antibiotics not Prophylaxis]
    C --> F[Choose Agent by Surgery Type]
    F --> G{Cefazolin Allergy?}
    G -->|No| H[Cefazolin 2 g IV 3 g if >120 kg]
    G -->|Yes non-severe| I[Still Cefazolin]
    G -->|Severe anaphylaxis| J[Vancomycin or Clindamycin]
    H --> K[Timing Within 60 min Before Incision]
    J --> L[Timing Within 120 min Before]
    K --> M[Redose q4h or blood loss 1500 mL]
    L --> M
    M --> N[Stop Within 24 h Post-op]
    N --> O[Bundle: Normothermia + Glucose <180 + Clipper + CHG-Alcohol Skin + O2]
    O --> P[Surveillance 30 d 90 d Implant]
```

---

## 11. 11. Flowchart — CDI Treatment Algorithm

```mermaid
flowchart TD
    A[3+ Unformed Stools/24h] --> B[Stop Inciting Antibiotic If Possible]
    B --> C[GDH + Toxin EIA Two-step]
    C --> D{GDH and Toxin Results?}
    D -->|Both positive| E[CDI Confirmed]
    D -->|GDH+ Toxin-| F[NAAT PCR]
    F -->|Positive| E
    F -->|Negative| G[CDI Unlikely]
    E --> H{Severity?}
    H -->|Non-severe| I[Fidaxomicin 200 mg BD 10 d]
    H -->|Severe| I
    H -->|Fulminant| J[Vancomycin 500 mg PO QDS + Metronidazole IV]
    I --> K{Response at 48-72 h?}
    K -->|Yes| L[Complete Course]
    K -->|No| M[Consider FMT or Bezlotoxumab or Live Biotherapeutic]
    J --> N[Surgical Consult Total Colectomy or Ileostomy]
    M --> O[Recurrence 2nd Fidaxomicin 200 mg BD 10 d]
    O --> P[Recurrence 3rd FMT VOWST REBYOTA]
    P --> Q[Bezlotoxumab as Adjunct in High Risk]
```
