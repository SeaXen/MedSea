...
davidson_topic_id: "14.1.19"
topic: "Fungal Pneumonias"
...
source: "/mnt/tb/Medicine/Infectious Disease/Fungal Pneumonias.md"
...
```mermaid
mindmap
  root((Fungal Pneumonias))
    By Host
      Neutropenic/HSCT
        IPA (Aspergillus)
        Mucormycosis
        Resistant moulds
      Non-neutropenic immunocompromised
        IPA / CPA / ABPA
        PCP
        Cryptococcus
        Endemic mycoses
    ...
    By Pathogen
      ...
      Endemic
        ...
        Paracoccidioides
    Diagnosis
      CT (halo, air-crescent, reverse halo, GGO)
      Biomarkers (GM, BDG, CrAg, Histo Ag, Coccidioides serology)
      Microscopy (KOH, GMS, PAS)
      Culture / PCR / Histology
    Treatment
      Voriconazole (IPA)
      Isavuconazole (IPA alt, Mucor secondary)
      L-AmB (Mucor, severe)
      Itraconazole (CPA, endemic, ABPA)
      TMP-SMX (PCP)
      Flucytosine + L-AmB (Crypto)
    Prophylaxis
      Posaconazole (AML/HSCT)
      TMP-SMX (HIV, steroids)
```

## Flowchart
```mermaid
flowchart TD
  A[Suspected fungal pneumonia] --> B{Neutropenic / HSCT?}
  B -->|Yes| C[CT: halo sign?]
  C -->|Yes| D[Serum GM, BDG]
  D --> E[BAL: GM, PCR, culture, GMS]
  E --> F[Empirical voriconazole / isavuconazole]
  C -->|Reverse halo| G[Surgical debridement + L-AmB 5-10mg/kg]
  B -->|No| H[Non-neutropenic immunocompromised?]
  H -->|Yes| I[CT + GM/BDG/CrAg/Histo Ag/PCP PCR]
  I --> J[BAL directed by syndrome]
  J --> K[Targeted therapy]
  H -->|No| L[Immunocompetent / endemic exposure]
  L --> M[Serology, urine Ag, culture]
  M --> N[Itraconazole/fluconazole mild; L-AmB severe]
```
