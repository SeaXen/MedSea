---
tags: [medicine, dermatology, davidson, fcps, mrcp, disease]
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 29: Dermatology
heading: Disorders of Pigmentation
topic: Melasma (chloasma)
status: full-fcps-mrcp-note
priority: high
created: 2026-06-16
modified: 2026-06-16
exam_relevance: [FCPS, MRCP Part 1, MRCP Part 2, PACES]
---

# Melasma (chloasma)

Related: [[../../../Dermatology MOC]], [[../../../Davidson Chapter 29 - Dermatology Hierarchy]]

> [!info] Davidson alignment: **Chapter 29: Dermatology** (24th Edition, Ibbotson) — Disorders of Pigmentation

## Definition

Melasma (chloasma) is a common acquired hyperpigmentation disorder, predominantly affecting women, with symmetric brown macules on face (cheeks, forehead, upper lip, nose). Driven by UV exposure, hormonal changes (pregnancy, OCP, HRT), and genetic predisposition. Epidermal (most common), dermal, or mixed. Treatment: sun protection (essential), topical hydroquinone, tretinoin, corticosteroids (Kligman formula), azelaic acid, tranexamic acid.

## Clinical Features and Presentation

Melasma (chloasma) presents with characteristic features as defined in Davidson 24e Ch 29. The key clinical features are summarised below. The disease is recognised by its morphology, distribution pattern, and course. Severity may be assessed using disease-specific scoring systems where applicable. Always consider the full differential when assessing a patient.

## Investigation and Management Summary

### Investigations
- Clinical diagnosis based on morphology, distribution, and history
- Skin biopsy (H&E) for histological confirmation in atypical or refractory cases
- Disease-specific tests as clinically indicated (autoantibodies, DIF for autoimmune bullous, culture for infection, KOH for fungal)
- Baseline bloods (FBC, LFT, renal) and pre-biologic screening (TB IGRA, hepatitis B/C, HIV) before systemic therapy

### Management
- **First-line:** General measures (emollients, soap substitutes, trigger avoidance) and topical therapy (corticosteroids, calcineurin inhibitors, vitamin D analogues, antifungals as appropriate)
- **Second-line:** Phototherapy (NB-UVB) or conventional systemic agents (methotrexate, ciclosporin, acitretin, azathioprine) for moderate-to-severe disease
- **Third-line:** Biologics or targeted small molecules (anti-TNF, anti-IL17/23, anti-IL4/13, JAK inhibitors) for refractory disease
- **Supportive:** Patient education, psychological support, quality-of-life assessment

## FCPS/MRCP High-Yield Summary

| Category | Key Points |
|----------|------------|
| Distribution | Symmetric face: cheeks, forehead, upper lip, nose |
| Sex | Women more common (F:M 9:1) |
| Triggers | UV, pregnancy, OCP, HRT, genetics |
| Type | Epidermal (most common), dermal, mixed (Wood's lamp) |
| Pregnancy mask | Common in pregnancy; often resolves postpartum |
| Diagnosis | Clinical; Wood's lamp (epidermal enhanced) |
| Treatment | Sun protection, hydroquinone, tretinoin, Kligman, azelaic acid, tranexamic acid |
| Hydroquinone | Tyrosinase inhibitor; 2-4% topical |
| Kligman | Hydroquinone + tretinoin + corticosteroid |
| Differential | Post-inflammatory hyperpigmentation, ephelides, lentigo, naevus, drug-induced |

## Differential Diagnosis

| Differential | Distinguishing Feature |
|--------------|------------------------|
| PIH | Post-inflammatory, asymmetric |

## Viva Questions

1. **Q:** Define Melasma (chloasma) and outline its classification.
   **A:** Melasma (chloasma) is a common acquired hyperpigmentation disorder, predominantly affecting women, with symmetric brown macules on face (cheeks, forehead, upper lip, nose). Driven by UV exposure, horm

2. **Q:** What are the key clinical features of Melasma (chloasma)?
   **A:** See the high-yield table above.

3. **Q:** Describe the stepwise management of Melasma (chloasma).
   **A:** Topical → phototherapy → conventional systemic → biologic.

4. **Q:** What scoring system is used for severity assessment?
   **A:** Disease-specific (see high-yield table).

5. **Q:** What are the key complications to monitor?
   **A:** Secondary infection, treatment toxicity, disease-specific complications.

## Confusions and Mnemonics

1. **Melasma** = Symmetric face; pregnancy/OCP/UV; hydroquinone

## Special Situations


| Situation | Consideration |
|-----------|---------------|
| **Pregnancy** | Avoid hydroquinone; use azelaic acid, sun protection |

## One-Page Revision Card

| **Topic** | **Melasma (chloasma)** |
|-----------|-----------|
| **Definition** | Melasma (chloasma) is a common acquired hyperpigmentation disorder, predominantly affecting women, w |
| **Key Features** | Symmetric face: cheeks, forehead, upper lip, nose |
| **Investigation** | Clinical ± biopsy + disease-specific tests |
| **Management** | Topical → phototherapy → systemic → biologic |
| **Red Flags** | Secondary infection, systemic involvement |
| **Viva Pearl** | See viva section |

## Spaced Repetition Trackers

- [ ] Day 1, 3, 7, 15, 30, 90

## Self-Test Scorecard

| Section | Score /5 |
|---------|----------|
| Understanding | /5 |
| Recall | /5 |
| MCQ Performance | /5 |
| SBA Performance | /5 |
| Viva Confidence | /5 |
| Total | /25 |

> <35 = weak, 35-44 = acceptable, 45+ = strong

## MCQs (5)
1. **Q:** Which of the following is a key feature of Melasma (chloasma)?
   A. Symmetric face: cheeks, forehead, upper lip, nose
   B. Always self-limiting
   C. Caused by bacterial infection only
   D. Restricted to palms and soles only
   E. No investigation needed
   **Answer: A**  *Distribution: Symmetric face: cheeks, forehead, upper lip, nose*
2. **Q:** What is the first-line treatment for Melasma (chloasma)?
   A. Topical therapy + general measures
   B. Surgical excision
   C. IVIG
   D. No treatment
   E. Antibiotics
   **Answer: A**  *Topical therapy first-line*
3. **Q:** Which pre-treatment screening is essential before systemic therapy for Melasma (chloasma)?
   A. TB IGRA, hepatitis B/C, HIV, pregnancy test
   B. Pregnancy test only
   C. ECG only
   D. No screening
   E. Eye exam only
   **Answer: A**  *Pre-biologic screening standard*
4. **Q:** What is the most important complication of Melasma (chloasma)?
   A. Secondary infection and disease-specific complications
   B. Malignancy transformation
   C. Renal failure
   D. Cardiac arrest
   E. No complications
   **Answer: A**
5. **Q:** What is the key histological/serological feature of Melasma (chloasma)?
   A. Post-inflammatory hyperpigmentation, ephelides, lentigo, naevus, drug-induced
   B. Always normal histology
   C. Non-specific findings
   D. Only positive in late disease
   E. No test exists
   **Answer: A**

## SBA Questions (5)
1. **Scenario:** Patient with Melasma (chloasma); first-line investigation?
   A. Clinical diagnosis ± biopsy if atypical  B. Antibiotics  C. CT  D. None  E. Steroids only
   **Answer: A**
2. **Scenario:** Melasma (chloasma) failed topical therapy; next step?
   A. Phototherapy or systemic  B. More topical  C. Observation  D. Surgery  E. Antifungals
   **Answer: A**
3. **Scenario:** Complication of Melasma (chloasma)?
   A. Secondary infection and complications  B. Malignancy  C. Renal failure  D. Cardiac arrest  E. None
   **Answer: A**
4. **Scenario:** Pre-treatment screening before systemic therapy for Melasma (chloasma)?
   A. TB, hep B/C, HIV  B. Pregnancy  C. ECG  D. None  E. Eye exam
   **Answer: A**
5. **Scenario:** Most characteristic histological finding in Melasma (chloasma)?
   A. Post-inflammatory hyperpigmentation, ephelides, lentigo, naevus, drug-induced
   B. Non-specific  C. Always normal  D. Only late  E. No test
   **Answer: A**

## Flashcards

- **Q:** Key fact about Melasma (chloasma)?
  **A:** Symmetric face: cheeks, forehead, upper lip, nose
- **Q:** Another key point?
  **A:** Women more common (F:M 9:1)
- **Q:** Most important complication?
  **A:** UV, pregnancy, OCP, HRT, genetics
- **Q:** First-line treatment?
  **A:** Epidermal (most common), dermal, mixed (Wood's lamp)
- **Q:** Key histological/serological feature?
  **A:** Common in pregnancy; often resolves postpartum

## Answer Key with Explanations

### MCQs
1. **A** — Distribution: Symmetric face: cheeks, forehead, upper lip, nose
2. **A** — Topical first-line
3. **A** — Pre-biologic screening
4. **A** — Secondary infection most common
5. **A** — Differential: Post-inflammatory hyperpigmentation, ephelides, lentigo, naevus, drug-induced

### SBAs
1. **A** — Clinical + biopsy if atypical
2. **A** — Phototherapy/systemic for moderate-severe
3. **A** — Secondary infection most common
4. **A** — Pre-biologic screening
5. **A** — Post-inflammatory hyperpigmentation, ephelides, lentigo, naevus, drug-induced

## Local Navigation

**Parent Heading Hub:** [[../../Disorders of Pigmentation Hub]]
**Chapter Hierarchy:** [[../../../Davidson Chapter 29 - Dermatology Hierarchy]]
**Chapter MOC:** [[../../../Dermatology MOC]]

---

> [!tip] **Exam focus:** Know definition, key clinical features, first-line management, and the most important complication. Always compare with the main differential.
