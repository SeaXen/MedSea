...
```mermaid
flowchart TD
    A[BP Diagnosed] --> B{Severity}
    B -->|Mild <20 bullae| C[Superpotent TCS whole body OR Doxycycline+Nicotinamide]
    B -->|Moderate-Severe| D[Prednisolone 0.5mg/kg + TCS ± Steroid-sparing Agent]
    C --> E{Remission 4-8w?}
    D --> E
    E -->|Yes| F[Taper Steroids over 6-12m; Maintain TCS/Doxy]
    E -->|No| G[Escalate: Increase Steroid, Add Azathioprine/MMF]
    G --> H{Remission?}
    H -->|No| I[Rituximab ± IVIG]
    H -->|Yes| J[Taper + Maintenance]
    F & J --> K[Long-term Follow-up: Relapse monitoring, Bone protection]
```
...
mindmap
  root((Bullous Pemphigoid))
    ...
    Clinical
      ...
      Nikolsky-
    Diagnosis
      DIF: Linear IgG/C3 BMZ
      Salt-split: Roof
      BP180 NC16A ELISA
      Histology: Subepidermal
    Differential
      Pemphigus Vulgaris
      EBA (Salt-split floor)
      Linear IgA (IgA DIF)
      MMP (Floor, mucosal)
    Management
      TCS (Clobetasol)
