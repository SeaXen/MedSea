---
tags: [medicine, dermatology, davidson, fcps, mrcp, disease]
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 29: Dermatology
heading: Genodermatoses
topic: Lamellar ichthyosis
status: full-fcps-mrcp-note
priority: high
created: 2026-06-16
modified: 2026-06-16
exam_relevance: [FCPS, MRCP Part 1, MRCP Part 2, PACES]
---

# Lamellar ichthyosis

Related: [[../../../Dermatology MOC]], [[../../../Davidson Chapter 29 - Dermatology Hierarchy]]

> [!info] Davidson alignment: **Chapter 29: Dermatology** (24th Edition, Ibbotson) — Genodermatoses

## Definition

Lamellar ichthyosis is a rare autosomal recessive ichthyosis (ARCI) presenting at birth as collodion baby (shiny, taut, parchment-like membrane), with generalised erythroderma and large dark plate-like scales, ectropion, eclabium. Caused by mutations in TGM1 (transglutaminase 1, most common), NIPAL4, ALOX12B, ALOXE3, others. Histology: massive hyperkeratosis, normal granular layer, increased mitoses. Treatment: emollients, keratolytics, oral retinoids (acitretin), topical retinoids; supportive for neonatal complications (temperature regulation, infection).

## Clinical Features and Presentation

Lamellar ichthyosis presents with characteristic features as defined in Davidson 24e Ch 29. The key clinical features are summarised below. The disease is recognised by its morphology, distribution pattern, and course. Severity may be assessed using disease-specific scoring systems where applicable. Always consider the full differential when assessing a patient.

## Investigation and Management Summary

### Investigations
- Clinical diagnosis based on morphology, distribution, and history
- Skin biopsy (H&E) for histological confirmation in atypical or refractory cases
- Disease-specific tests as clinically indicated (autoantibodies, DIF for autoimmune bullous, culture for infection, KOH for fungal)
- Baseline bloods (FBC, LFT, renal) and pre-biologic screening (TB IGRA, hepatitis B/C, HIV) before systemic therapy

### Management
- **First-line:** General measures (emollients, soap substitutes, trigger avoidance) and topical therapy (corticosteroids, calcineurin inhibitors, vitamin D analogues, antifungals as appropriate)
- **Second-line:** Phototherapy (NB-UVB) or conventional systemic agents (methotrexate, ciclosporin, acitretin, azathioprine) for moderate-to-severe disease
- **Third-line:** Biologics or targeted small molecules (anti-TNF, anti-IL17/23, anti-IL4/13, JAK inhibitors) for refractory disease
- **Supportive:** Patient education, psychological support, quality-of-life assessment

## FCPS/MRCP High-Yield Summary

| Category | Key Points |
|----------|------------|
| Inheritance | Autosomal recessive |
| Gene | TGM1 (most common), NIPAL4, ALOX12B, ALOXE3, CYP4F22 |
| Birth | Collodion baby (parchment membrane) |
| Scale | Large, dark, plate-like, generalised |
| Face | Ectropion, eclabium (everted lips) |
| Histology | Massive hyperkeratosis, normal granular layer |
| Complications | Temperature regulation (skin barrier), infection, dehydration |
| Treatment | Emollients, keratolytics, oral retinoids (acitretin), topical retinoids |
| Prognosis | Chronic, lifelong; retinoids helpful |

## Differential Diagnosis

| Differential | Distinguishing Feature |
|--------------|------------------------|
| CIE | Congenital ichthyosiform erythroderma, similar |

## Viva Questions

1. **Q:** Define Lamellar ichthyosis and outline its classification.
   **A:** Lamellar ichthyosis is a rare autosomal recessive ichthyosis (ARCI) presenting at birth as collodion baby (shiny, taut, parchment-like membrane), with generalised erythroderma and large dark plate-lik

2. **Q:** What are the key clinical features of Lamellar ichthyosis?
   **A:** See the high-yield table above.

3. **Q:** Describe the stepwise management of Lamellar ichthyosis.
   **A:** Topical → phototherapy → conventional systemic → biologic.

4. **Q:** What scoring system is used for severity assessment?
   **A:** Disease-specific (see high-yield table).

5. **Q:** What are the key complications to monitor?
   **A:** Secondary infection, treatment toxicity, disease-specific complications.

## Confusions and Mnemonics

- See clinical features

## Special Situations


| Situation | Consideration |
|-----------|---------------|
| **Pregnancy** | Avoid teratogens (retinoids, MTX, MMF) |


## One-Page Revision Card

| **Topic** | **Lamellar ichthyosis** |
|-----------|-----------|
| **Definition** | Lamellar ichthyosis is a rare autosomal recessive ichthyosis (ARCI) presenting at birth as collodion |
| **Key Features** | Autosomal recessive |
| **Investigation** | Clinical ± biopsy + disease-specific tests |
| **Management** | Topical → phototherapy → systemic → biologic |
| **Red Flags** | Secondary infection, systemic involvement |
| **Viva Pearl** | See viva section |

## Spaced Repetition Trackers

- [ ] Day 1, 3, 7, 15, 30, 90

## Self-Test Scorecard

| Section | Score /5 |
|---------|----------|
| Understanding | /5 |
| Recall | /5 |
| MCQ Performance | /5 |
| SBA Performance | /5 |
| Viva Confidence | /5 |
| Total | /25 |

> <35 = weak, 35-44 = acceptable, 45+ = strong

## MCQs (5)
1. **Q:** Which of the following is a key feature of Lamellar ichthyosis?
   A. Autosomal recessive
   B. Always self-limiting
   C. Caused by bacterial infection only
   D. Restricted to palms and soles only
   E. No investigation needed
   **Answer: A**
2. **Q:** What is the first-line treatment for Lamellar ichthyosis?
   A. Topical therapy + general measures
   B. Surgical excision
   C. IVIG
   D. No treatment
   E. Antibiotics
   **Answer: A**
3. **Q:** Which pre-treatment screening is essential before systemic therapy?
   A. TB IGRA, hepatitis B/C, HIV, pregnancy test
   B. Pregnancy test only
   C. ECG only
   D. No screening
   E. Eye exam only
   **Answer: A**
4. **Q:** What is the most important complication of Lamellar ichthyosis?
   A. Secondary infection and disease-specific complications
   B. Malignancy transformation
   C. Renal failure
   D. Cardiac arrest
   E. No complications
   **Answer: A**
5. **Q:** What is the key histological/serological feature?
   A. Chronic, lifelong; retinoids helpful
   B. Always normal
   C. Non-specific
   D. Only late
   E. No test
   **Answer: A**

## SBA Questions (5)
1. **Scenario:** Patient with Lamellar ichthyosis; first-line investigation?
   A. Clinical diagnosis ± biopsy if atypical  B. Antibiotics  C. CT  D. None  E. Steroids only
   **Answer: A**
2. **Scenario:** Failed topical therapy; next step?
   A. Phototherapy or systemic  B. More topical  C. Observation  D. Surgery  E. Antifungals
   **Answer: A**
3. **Scenario:** Complication of Lamellar ichthyosis?
   A. Secondary infection and complications  B. Malignancy  C. Renal failure  D. Cardiac arrest  E. None
   **Answer: A**
4. **Scenario:** Pre-treatment screening before systemic therapy?
   A. TB, hep B/C, HIV  B. Pregnancy  C. ECG  D. None  E. Eye exam
   **Answer: A**
5. **Scenario:** Most characteristic histological finding in Lamellar ichthyosis?
   A. Chronic, lifelong; retinoids helpful
   B. Non-specific  C. Always normal  D. Only late  E. No test
   **Answer: A**

## Flashcards

- **Q:** Key fact about Lamellar ichthyosis?
  **A:** Autosomal recessive
- **Q:** Another key point?
  **A:** TGM1 (most common), NIPAL4, ALOX12B, ALOXE3, CYP4F22
- **Q:** Most important complication?
  **A:** Collodion baby (parchment membrane)
- **Q:** First-line treatment?
  **A:** Large, dark, plate-like, generalised
- **Q:** Key histological/serological feature?
  **A:** Ectropion, eclabium (everted lips)

## Answer Key with Explanations

### MCQs
1. **A** — Inheritance: Autosomal recessive
2. **A** — Topical first-line
3. **A** — Pre-biologic screening
4. **A** — Secondary infection most common
5. **A** — Prognosis: Chronic, lifelong; retinoids helpful

### SBAs
1. **A** — Clinical + biopsy if atypical
2. **A** — Phototherapy/systemic for moderate-severe
3. **A** — Secondary infection most common
4. **A** — Pre-biologic screening
5. **A** — Chronic, lifelong; retinoids helpful

## Local Navigation

**Parent Heading Hub:** [[../../Genodermatoses Hub]]
**Chapter Hierarchy:** [[../../../Davidson Chapter 29 - Dermatology Hierarchy]]
**Chapter MOC:** [[../../../Dermatology MOC]]

---

> [!tip] **Exam focus:** Know definition, key clinical features, first-line management, and the most important complication. Always compare with the main differential.
