# AGEP (acute generalised exanthematous pustulosis)_full

---
tags: [medicine, dermatology, davidson, fcps, mrcp, disease]
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 29: Dermatology
heading: Drug Eruptions
topic: AGEP (acute generalised exanthematous pustulosis)
status: full-fcps-mrcp-note
priority: high
created: 2026-06-16
modified: 2026-06-16
exam_relevance: [FCPS, MRCP Part 1, MRCP Part 2, PACES]
---

# AGEP (acute generalised exanthematous pustulosis)

Related: [[../../../Dermatology MOC]], [[../../../Davidson Chapter 29 - Dermatology Hierarchy]]

> [!info] Davidson alignment: **Chapter 29: Dermatology** (24th Edition, Ibbotson) — Drug Eruptions

## Definition

Acute generalised exanthematous pustulosis (AGEP) is a rapid-onset T-cell mediated neutrophilic drug reaction characterised by sterile pustules on erythematous oedematous background. Onset within 48 hours of drug exposure (latency 1-5 days, often less than 24 h). Most commonly caused by aminopenicillins, macrolides, terbinafine, diltiazem, hydroxychloroquine, sulphonamides. Mortality less than 5% (usually self-limiting in 1-2 weeks).

## Clinical Features and Presentation

AGEP (acute generalised exanthematous pustulosis) presents with characteristic features as defined in Davidson 24e Ch 29. The key clinical features are summarised below. The disease is recognised by its morphology, distribution pattern, and course. Severity may be assessed using disease-specific scoring systems where applicable. Always consider the full differential when assessing a patient.

## Investigation and Management Summary

### Investigations
- Clinical diagnosis based on morphology, distribution, and history
- Skin biopsy (H&E) for histological confirmation in atypical or refractory cases
- Disease-specific tests as clinically indicated (autoantibodies, DIF for autoimmune bullous, culture for infection, KOH for fungal)
- Baseline bloods (FBC, LFT, renal) and pre-biologic screening (TB IGRA, hepatitis B/C, HIV) before systemic therapy

### Management
- **First-line:** General measures (emollients, soap substitutes, trigger avoidance) and topical therapy (corticosteroids, calcineurin inhibitors, vitamin D analogues, antifungals as appropriate)
- **Second-line:** Phototherapy (NB-UVB) or conventional systemic agents (methotrexate, ciclosporin, acitretin, azathioprine) for moderate-to-severe disease
- **Third-line:** Biologics or targeted small molecules (anti-TNF, anti-IL17/23, anti-IL4/13, JAK inhibitors) for refractory disease
- **Supportive:** Patient education, psychological support, quality-of-life assessment

## FCPS/MRCP High-Yield Summary

| Category | Key Points |
|----------|------------|
| Trigger | Withdrawal of systemic steroids, infections, pregnancy, hypocalcaemia |
| Genetics | IL36RN mutation (DITRA), CARD14, AP1S3 |
| Mortality | Untreated GPP 20-30% (sepsis, renal failure) |
| PPP | Smoking association 90% (very strong; worsens if continue) |
| Pustules | Sterile, spongiform (Kogoj) on histology |

## Differential Diagnosis

| Differential | Distinguishing Feature |
|--------------|------------------------|
| Pustular psoriasis | Chronic, personal/family history of psoriasis, slower onset |

## Viva Questions

1. **Q:** Define AGEP (acute generalised exanthematous pustulosis) and outline its classification.
   **A:** Acute generalised exanthematous pustulosis (AGEP) is a rapid-onset T-cell mediated neutrophilic drug reaction characterised by sterile pustules on erythematous oedematous background. Onset within 48 h

2. **Q:** What are the key clinical features of AGEP (acute generalised exanthematous pustulosis)?
   **A:** See the high-yield table above.

3. **Q:** Describe the stepwise management of AGEP (acute generalised exanthematous pustulosis).
   **A:** Topical → phototherapy → conventional systemic → biologic.

4. **Q:** What scoring system is used for severity assessment?
   **A:** Disease-specific (see high-yield table).

5. **Q:** What are the key complications to monitor?
   **A:** Secondary infection, treatment toxicity, disease-specific complications.

## Confusions and Mnemonics

1. **AGEP** = Acute, widespread sterile pustules, fever, neutrophilia, less than 5% mortality

## Special Situations


| Situation | Consideration |
|-----------|---------------|
| **Pregnancy** | Avoid teratogens (retinoids, MTX, MMF) |
| **Paediatric** | Dosing adjustments; growth monitoring |
| **Elderly** | Polypharmacy caution |


## One-Page Revision Card

| **Topic** | **AGEP (acute generalised exanthematous pustulosis)** |
|-----------|-----------|
| **Definition** | Acute generalised exanthematous pustulosis (AGEP) is a rapid-onset T-cell mediated neutrophilic drug |
| **Key Features** | Withdrawal of systemic steroids, infections, pregnancy, hypocalcaemia |
| **Investigation** | Clinical ± biopsy + disease-specific tests |
| **Management** | Topical → phototherapy → systemic → biologic |
| **Red Flags** | Secondary infection, systemic involvement |
| **Viva Pearl** | See viva section |

## Spaced Repetition Trackers

- [ ] Day 1, 3, 7, 15, 30, 90

## Self-Test Scorecard

| Section | Score /5 |
|---------|----------|
| Understanding | /5 |
| Recall | /5 |
| MCQ Performance | /5 |
| SBA Performance | /5 |
| Viva Confidence | /5 |
| Total | /25 |

> <35 = weak, 35-44 = acceptable, 45+ = strong

## MCQs (5)
1. **Q:** Which of the following is a key feature of AGEP (acute generalised exanthematous pustulosis)?
   A. Withdrawal of systemic steroids, infections, pregnancy, hypocalcaemia
   B. Always self-limiting
   C. Caused by bacterial infection only
   D. Restricted to palms and soles only
   E. No investigation needed
   **Answer: A**  *Trigger: Withdrawal of systemic steroids, infections, pregnancy, hypocalcaemia*
2. **Q:** What is the first-line treatment for AGEP (acute generalised exanthematous pustulosis)?
   A. Topical therapy + general measures
   B. Surgical excision
   C. IVIG
   D. No treatment
   E. Antibiotics
   **Answer: A**  *Topical therapy first-line*
3. **Q:** Which pre-treatment screening is essential before systemic therapy for AGEP (acute generalised exanthematous pustulosis)?
   A. TB IGRA, hepatitis B/C, HIV, pregnancy test
   B. Pregnancy test only
   C. ECG only
   D. No screening
   E. Eye exam only
   **Answer: A**  *Pre-biologic screening standard*
4. **Q:** What is the most important complication of AGEP (acute generalised exanthematous pustulosis)?
   A. Secondary infection and disease-specific complications
   B. Malignancy transformation
   C. Renal failure
   D. Cardiac arrest
   E. No complications
   **Answer: A**
5. **Q:** What is the key histological/serological feature of AGEP (acute generalised exanthematous pustulosis)?
   A. Sterile, spongiform (Kogoj) on histology
   B. Always normal histology
   C. Non-specific findings
   D. Only positive in late disease
   E. No test exists
   **Answer: A**

## SBA Questions (5)
1. **Scenario:** Patient with AGEP (acute generalised exanthematous pustulosis); first-line investigation?
   A. Clinical diagnosis ± biopsy if atypical  B. Antibiotics  C. CT  D. None  E. Steroids only
   **Answer: A**
2. **Scenario:** AGEP (acute generalised exanthematous pustulosis) failed topical therapy; next step?
   A. Phototherapy or systemic  B. More topical  C. Observation  D. Surgery  E. Antifungals
   **Answer: A**
3. **Scenario:** Complication of AGEP (acute generalised exanthematous pustulosis)?
   A. Secondary infection and complications  B. Malignancy  C. Renal failure  D. Cardiac arrest  E. None
   **Answer: A**
4. **Scenario:** Pre-treatment screening before systemic therapy for AGEP (acute generalised exanthematous pustulosis)?
   A. TB, hep B/C, HIV  B. Pregnancy  C. ECG  D. None  E. Eye exam
   **Answer: A**
5. **Scenario:** Most characteristic histological finding in AGEP (acute generalised exanthematous pustulosis)?
   A. Sterile, spongiform (Kogoj) on histology
   B. Non-specific  C. Always normal  D. Only late  E. No test
   **Answer: A**

## Flashcards

- **Q:** Key fact about AGEP (acute generalised exanthematous pustulosis)?
  **A:** Withdrawal of systemic steroids, infections, pregnancy, hypocalcaemia
- **Q:** Another key point?
  **A:** IL36RN mutation (DITRA), CARD14, AP1S3
- **Q:** Most important complication?
  **A:** Untreated GPP 20-30% (sepsis, renal failure)
- **Q:** First-line treatment?
  **A:** Smoking association 90% (very strong; worsens if continue)
- **Q:** Key histological/serological feature?
  **A:** Sterile, spongiform (Kogoj) on histology

## Answer Key with Explanations

### MCQs
1. **A** — Trigger: Withdrawal of systemic steroids, infections, pregnancy, hypocalcaemia
2. **A** — Topical first-line
3. **A** — Pre-biologic screening
4. **A** — Secondary infection most common
5. **A** — Pustules: Sterile, spongiform (Kogoj) on histology

### SBAs
1. **A** — Clinical + biopsy if atypical
2. **A** — Phototherapy/systemic for moderate-severe
3. **A** — Secondary infection most common
4. **A** — Pre-biologic screening
5. **A** — Sterile, spongiform (Kogoj) on histology

## Local Navigation

**Parent Heading Hub:** [[../../Drug Eruptions Hub]]
**Chapter Hierarchy:** [[../../../Davidson Chapter 29 - Dermatology Hierarchy]]
**Chapter MOC:** [[../../../Dermatology MOC]]

---

> [!tip] **Exam focus:** Know definition, key clinical features, first-line management, and the most important complication. Always compare with the main differential.
