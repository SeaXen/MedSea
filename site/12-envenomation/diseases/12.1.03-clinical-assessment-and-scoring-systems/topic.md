...
```mermaid
flowchart TD
    A[Suspected Envenomation] --> B[Triage ABCDE]
    B --> C[History + Examination]
    C --> D[Identify Animal]
    D --> E{Type?}
    E -->|Snake| F[Snake Protocol]
    E -->|Scorpion| G[Scorpion Protocol]
    E -->|Spider| H[Spider Protocol]
    E -->|Marine| I[Marine Protocol]
    E -->|Hymenoptera| J[Anaphylaxis Protocol]
    F --> K[Baseline Labs + 20WBCT]
    G --> L[Vitals + ECG + CXR]
    H --> M[Pain Score + Neuro Exam]
    I --> N[Marine Labs + ECG]
    J --> O[Anaphylaxis Workup]
    K --> P{Severity Score}
    L --> P
    M --> P
    N --> P
    P -->|Mild| Q[Observe]
    P -->|Moderate| R[Admit + Consider AV]
    P -->|Severe| S[ICU + AV + Monitoring]
```
...
```mermaid
mindmap
  root((Clinical Assessment & Scoring))
    History
      AMPLE
      Snake_description
      First_aid
      Symptoms_onset
    Examination
      Local[Fang marks, swelling, necrosis]
      Systemic[Neuro, Coag, CVS, Renal]
      ABCDE
    Scoring_Systems
      WHO_Grading[0-4]
      SSS[Snakebite Severity Score 0-22]
      SCORPION[0-8]
      RADS[Redback 0-6]
    Investigations
      Bedside[20WBCT, ECG]
      Coagulation[INR, aPTT, fibrinogen, platelets, D-dimer]
      Renal[U&E, CK, myoglobin]
      Other[FBC, LFTs, ABG, CXR, VDK]
    Management
      Mild[Observe, ward]
      Moderate[HDU, consider AV]
      Severe[ICU, AV, monitoring]
    Reassessment
      Vitals_q15min
      Coag_q6h
      Neuro_q30-60min
