...
```mermaid
mindmap
  root((General Envenomation Principles))
    Classification
      Snakes[Elapid, Viperid, Colubrid]
      Scorpions
      Spiders
      Marine[Jellyfish, Fish, Octopus]
      Hymenoptera[Bee, Wasp, Ant]
    Venom_Classes
      Neurotoxic[Postsynaptic, Presynaptic, Ion channel]
      Haemotoxic[Procoagulant, Anticoagulant, Platelet]
      Cytotoxic[PLA2, Cytolysins, Proteases]
      Cardiotoxic
      Allergenic
    First_Aid
      DO[PIB, Immobilise, Hot water, Vinegar]
      DONT[Tourniquet, Incision, Ice, Shock, Elevation]
    Assessment
      ABCDE[Airway, Breathing, Circulation, Disability, Exposure]
      Scoring[WHO grading, Snakebite severity, SCORPION]
    Antivenom
      Principles[Mechanism, Types, Timing, Dosing]
      Reactions[Early anaphylaxis, Pyrogenic, Late serum sickness]
      Monitoring[Coag, Renal, Neuro, Vitals]
    Disposition
      Dry_bite[Observe 12-24h]
      Mild[Observe 24h]
      Moderate-Severe[ICU, Antivenom, Serial labs]
```
...
```mermaid
flowchart TD
    A[Suspected Envenomation] --> B[ABCDE Stabilisation]
    B --> C{Identify Agent}
    C -->|Snake| D[Snake Protocol]
    C -->|Scorpion| E[Scorpion Protocol]
    C -->|Spider| F[Spider Protocol]
    C -->|Marine| G[Marine Protocol]
    C -->|Hymenoptera| H[Anaphylaxis Protocol]
    D --> I[Apply PIB if Elapid / Local Pressure if Viperid]
    I --> J[Immobilise, IV Access, Labs]
    J --> K{Systemic Envenomation?}
    K -->|Yes| L[Antivenom Protocol]
    K -->|No| M[Observe 12-24h]
    L --> N[Monitor: Coag, Renal, Neuro, Vitals]
    N --> O[Disposition]
```
