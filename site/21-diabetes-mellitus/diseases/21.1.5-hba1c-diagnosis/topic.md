...
```mermaid
flowchart TD
    A[HbA1c Result] --> B{Diagnostic Range?}
    B -->|≥48 mmol/mol| C[Diabetes - confirm if asymptomatic]
    B -->|39-47 mmol/mol| D[Prediabetes - lifestyle intervention]
    B -->|<39 mmol/mol| E[Normal - repeat per screening interval]
    C --> F{Conditions affecting accuracy?}
    F -->|Yes| G[Use FPG/OGTT instead]
    F -->|No| H[Diagnosis confirmed]
```
