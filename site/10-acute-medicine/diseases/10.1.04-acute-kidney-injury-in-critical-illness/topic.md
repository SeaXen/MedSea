...
```mermaid
flowchart TD
  A[Olive Urine Output / Rising Creatinine] --> B[KDIGO Criteria Met?]
  B -->|Yes| C[Stage AKI 1/2/3]
  C --> D{Exclude Post-Renal\nUS Kidneys}
  D -->|Hydronephrosis| E[Relieve Obstruction\nCatheter/Nephrostomy]
  D -->|No Obstruction| F[Pre-renal vs Intrinsic]
  F --> G{FENa <1%\nUrine Osm >500\nBland Sediment}
  G -->|Yes| H[Pre-renal: Volume Resuscitation\nOptimise Haemodynamics\nStop Diuretics/ACEi/NSAIDs]
  G -->|No| I[Intrinsic: ATN Most Likely\nStop Nephrotoxins\nSupportive Care\nMonitor for RRT Indications]
  H --> J[Monitor Response\nCr, UO, Fluid Balance\nElectrolytes Daily]
  I --> J
  J --> K{RRT Indications?\nAEIOU}
  K -->|Yes| L[CRRT if Unstable\nIHD/SLED if Stable]
  K -->|No| M[Continue Supportive\nDrug Dose Adjustment\nAvoid Fluid Overload]
```
