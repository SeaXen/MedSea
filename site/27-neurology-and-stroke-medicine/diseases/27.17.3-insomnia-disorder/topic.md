---
tags: [medicine, neurology, sleep, insomnia, fcps, mrcp]
chapter: Neurology
...
references:
  anatomy: []
  physiology: []
  clinical: ['Davidson 24th Ed Ch25', 'AASM ICSD-3 Criteria', 'NICE CKS Insomnia', 'ESRS/European Sleep Research Society', 'BAP Consensus 2019', 'PasTest', 'MRCP Notes']
related: ['Sleep Disorders Hub', 'Insomnia & Hypersomnia Hub', 'Idiopathic Hypersomnia', 'Narcolepsy', 'Obstructive Sleep Apnoea', 'Restless Legs Syndrome']
...
```mermaid
flowchart TD
    A[Predisposing Factors] --> B[Vulnerability]
    C[Precipitating Stressor] --> D[Acute Insomnia]
    B --> D
    D --> E[Maladaptive Behaviours]
    E --> F[Conditioned Arousal]
    F --> G[Cognitive Hyperarousal]
    G --> H[24-h Hyperarousal]
    H --> I[Chronic Insomnia]
```
...
```mermaid
flowchart TD
    A[Insomnia Symptoms] --> B[Detailed History + Diary]
    B --> C{Red Flags?}
    C -->|Yes OSA/RLS/PLMD/Narcolepsy| D[Targeted Investigation PSG/MSLT]
    C -->|No| E[Identify 3P Factors]
    E --> F[Comorbidity Screen - Mental/Medical/Substance]
    F --> G{Comorbidity Identified?}
    G -->|Yes| H[Treat Comorbidity + Insomnia]
    G -->|No| I[Chronic Insomnia Disorder]
    I --> J[Step 1: Sleep Hygiene + CBT-I]
    J --> K{Response?}
    K -->|No| L[Add Pharmacotherapy Short-term]
    L --> M{Response?}
    M -->|No| N[Specialist Referral]
```
...
```mermaid
flowchart TD
    A[Insomnia Disorder Confirmed] --> B[CBT-I + Sleep Hygiene]
    B --> C{Response 4-6 weeks?}
    C -->|Yes| D[Continue + Relapse Prevention]
    C -->|No| E[Add Short-term Hypnotic 4 weeks max]
    E --> F{Response?}
    F -->|Yes| D
    F -->|No| G[Specialist Referral]
    G --> H[Reassess / PSG if not done / Review Comorbidity]
```
