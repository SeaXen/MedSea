---
tags: [medicine, neurology, paraneoplastic, paediatric, fcps, mrcp]
chapter: Neurology
...
related: ['Paraneoplastic Cerebellar Degeneration', 'Cancer Screening in PNS', 'Paraneoplastic Encephalitis']
...
```mermaid
flowchart TD
    A[Tumour/Infection/Idiopathic] --> B[Immune Activation]
    B --> C[Antibody vs Brainstem/Cerebellar Antigens]
    C --> D[Disinhibition of Burst Neurons in PPRF/rostral SC]
    D --> E[Loss of Omnipause Neuron Function]
    E --> F[Opsoclonus - chaotic saccades]
    C --> G[Cerebellar Fastigial Nucleus Dysfunction]
    G --> H[Myoclonus + Ataxia]
```
...
```mermaid
flowchart TD
    A[Opsoclonus + Myoclonus + Ataxia] --> B[Clinical diagnosis of OMS]
    B --> C[Age Stratification]
    C --> D{Adult or Child?}
    D -->|Child| E[Urgent CT/MRI Adrenals + MIBG/PET for Neuroblastoma]
    D -->|Adult| F[Breast Exam + Mammogram, CT CAP, Pelvic Imaging]
    E --> G[Antibody Panel: Anti-Ri, Anti-Hu, Anti-Yo, Anti-Ma]
    F --> G
    G --> H[Tumour Treatment + Immunotherapy]
    B --> I[MRI Brain - exclude structural]
    I --> J[LP - exclude infection, OCB, antibodies]
    J --> K[EEG - exclude seizures]
```
...
```mermaid
flowchart TD
    A[OMS Confirmed] --> B[Urgent Tumour Screen]
    B --> C[Tumour Treatment if Found]
    B --> D[Immunotherapy: Steroids + IVIG]
    D --> E{Response at 4-6 weeks?}
    E -->|Yes| F[Taper + Maintenance: AZA/MMF/Rituximab]
    E -->|No| G[Escalate: PLEX, Rituximab, Cyclophosphamide]
    F --> H[Monitor for Relapse]
    G --> H
```
