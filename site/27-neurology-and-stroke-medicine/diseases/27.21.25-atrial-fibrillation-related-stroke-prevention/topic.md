---
tags: [medicine, stroke-medicine, davidson, full-note, secondary-prevention, atrial-fibrillation, cardioembolic-stroke]
...
topic: Atrial fibrillation-related stroke prevention
...
```mermaid
flowchart TD
  A[TIA or ischaemic stroke] --> B[Look for cardioembolic mechanism]
  B --> C{AF known or newly detected?}
  C -->|No| D[Continue rhythm monitoring / alternative workup]
  C -->|Yes| E[Assess infarct size, bleed risk, renal function, valve status]
  E --> F{TIA or very small infarct?}
  F -->|Yes| G[Earlier anticoagulation may be appropriate]
  F -->|No| H{Moderate or large infarct / hemorrhagic risk?}
  H -->|Yes| I[Delay, repeat imaging, then anticoagulate]
  H -->|No| J[Intermediate timing]
  G --> K[Long-term anticoagulation]
  I --> K
  J --> K
  K --> L[Optimize BP, diabetes, lipids, adherence, lifestyle]
```
...
```mermaid
flowchart LR
  A[AF-related TIA/stroke] --> B[Define infarct size and hemorrhage risk]
  B --> C{Safe to anticoagulate now?}
  C -->|Yes| D[Start long-term anticoagulation]
  C -->|No| E[Delay + repeat imaging/monitoring]
  E --> D
  D --> F[Control BP, diabetes, lipids, lifestyle]
```
