---
tags: [medicine, neurology, davidson, epilepsy, emergency, fcps, mrcp]
chapter: Neurology
...
```mermaid
flowchart TD
  A[Seizing patient] --> B[Call for help, note time, ABC approach]
  B --> C[Airway positioning, suction, high-flow oxygen, protect from injury]
  C --> D[Check capillary glucose immediately]
  D --> E{Hypoglycemia?}
  E -->|Yes| F[Give IV glucose +/- thiamine if indicated]
  E -->|No| G[IV/IO access, bloods, monitor ECG/BP/O2]
  G --> H[First-line benzodiazepine]
  H --> I{Seizure stops?}
  I -->|Yes| J[Observe, investigate cause, prevent recurrence]
  I -->|No| K[Second-line antiseizure drug: levetiracetam / valproate / phenytoin-fosphenytoin]
  K --> L{Still seizing?}
  L -->|Yes| M[ICU, intubation, anesthetic therapy, EEG]
  L -->|No| J
```
...
```mermaid
mindmap
  root((Status epilepticus))
    Recognition
      Convulsive >=5 min
      Recurrent seizures no recovery
      Non-convulsive altered mental state
    Immediate steps
      ABC
      Oxygen
      Glucose
