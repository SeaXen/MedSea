---
tags: [medicine, stroke-medicine, davidson, full-note, secondary-prevention, dapt]
...
topic: Dual antiplatelet therapy after minor stroke or TIA
...
```mermaid
flowchart TD
  A[Minor stroke or high-risk TIA] --> B[Exclude hemorrhage]
  B --> C[Determine mechanism]
  C --> D{Cardioembolic source?}
  D -->|Yes| E[Usually anticoagulation pathway, not routine DAPT]
  D -->|No| F[Assess bleeding risk + stroke severity]
  F --> G{Minor stroke / high-risk TIA and acceptable bleeding risk?}
  G -->|Yes| H[Start short-course DAPT]
  G -->|No| I[Use single antiplatelet strategy]
  H --> J[Step down to single antiplatelet long term]
  I --> J
```
...
```mermaid
mindmap
  root((DAPT after minor stroke/TIA))
    When to use
      Minor stroke
      High-risk TIA
      Non-cardioembolic
...
```mermaid
flowchart LR
  A[Minor stroke/high-risk TIA] --> B[Exclude bleed]
  B --> C[Check mechanism]
  C --> D{Non-cardioembolic?}
  D -->|Yes| E[Assess bleeding risk]
  E --> F[Short-course DAPT]
  F --> G[Return to single antiplatelet]
  D -->|No| H[Alternative pathway e.g. anticoagulation]
```
