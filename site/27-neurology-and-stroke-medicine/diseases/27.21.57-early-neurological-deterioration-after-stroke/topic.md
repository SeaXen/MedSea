---
tags: [medicine, stroke-medicine, davidson, full-note, stroke-unit-care, early-neurological-deterioration, END]
...
topic: Early neurological deterioration after stroke
...
```mermaid
flowchart TD
  A[Stroke patient worsens] --> B[Check ABC, glucose, oxygenation, BP]
  B --> C[Urgent focused neuro exam]
  C --> D[Review timing: post-lysis? large infarct? ICH?]
  D --> E[Urgent non-contrast CT head]
  E --> F{Bleed / edema / hydrocephalus / mass effect?}
  F -->|Yes| G[Treat structural complication + escalate]
  F -->|No| H[Search for seizure, hypoxia, infection, metabolic or hemodynamic cause]
  G --> I[Repeat monitoring and targeted therapy]
  H --> I[Repeat monitoring and targeted therapy]
```
...
```mermaid
flowchart LR
  A[Stroke worsens] --> B[ABC + glucose + vitals]
  B --> C[Urgent neuro reassessment]
  C --> D[Urgent CT head]
  D --> E{Structural cause?}
  E -->|Yes| F[Treat bleed/edema/ICP issue]
  E -->|No| G[Treat systemic cause]
  F --> H[Close monitoring]
  G --> H
```
