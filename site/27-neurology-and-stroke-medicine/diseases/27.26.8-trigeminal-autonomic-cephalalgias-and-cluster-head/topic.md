...
```mermaid
flowchart TD
  A[Patient with unilateral severe headache] --> B{Thunderclap, fever, focal deficit, papilledema, eye signs, new onset, cancer/immunosuppression?}
  B -->|Yes| C[Evaluate urgently for secondary headache: CT/MRI, vascular imaging, eye exam, LP if indicated]
  B -->|No| D{Short attacks with autonomic signs?}
  D -->|No| E[Consider migraine, trigeminal neuralgia, sinus/ocular disease, cervicogenic, temporal arteritis]
  D -->|Yes| F{Attack duration pattern}
  F -->|15-180 min| G[Think cluster headache]
  F -->|2-30 min and many/day| H[Think paroxysmal hemicrania]
  F -->|Seconds-minutes| I[Think SUNCT/SUNA or trigeminal neuralgia mimic]
  F -->|Continuous with flares| J[Think hemicrania continua]
  G --> K[Assess bout pattern, alcohol trigger, nocturnal attacks, restlessness]
  H --> L[Trial of indomethacin if appropriate]
  I --> M[Consider MRI to exclude posterior fossa/pituitary or other structural lesion]
  J --> N[Indomethacin response supports diagnosis]
  K --> O[Acute therapy: high-flow oxygen or SC/intranasal triptan]
  O --> P[Start preventive therapy e.g. verapamil; bridge if needed]
```
