...
```mermaid
flowchart TD
  A[First suspected seizure] --> B[Stabilize if ongoing/emergency]
  B --> C[Patient history + witness history + exam]
  C --> D[Capillary glucose + ECG + basic labs]
  D --> E{Acute provoker found?}
  E -->|Yes| F[Provoked seizure pathway]
  E -->|No| G[Unprovoked seizure pathway]
  F --> H[Treat cause + recurrence prevention]
  G --> I[EEG + MRI/CT depending context]
  I --> J[Recurrence risk / epilepsy counseling]
```
...
```mermaid
flowchart LR
  A[Suspected first seizure] --> B[History + witness + exam]
  B --> C[Glucose, labs, ECG]
  C --> D{Provoked cause found?}
  D -->|Yes| E[Treat cause]
  D -->|No| F[EEG + MRI/CT as indicated]
  F --> G[Recurrence risk and counseling]
```
