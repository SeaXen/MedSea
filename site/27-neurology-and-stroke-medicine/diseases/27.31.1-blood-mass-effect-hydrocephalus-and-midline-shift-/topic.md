...
```mermaid
flowchart TD
A[Open CT head] --> B[Check patient details, side markers, image quality]
B --> C[Look for blood]
C --> D[Assess ventricles and cisterns]
D --> E[Look for mass effect and edema]
E --> F[Assess midline structures for shift]
F --> G[Check posterior fossa and hydrocephalus]
G --> H[Correlate with clinical urgency: ICP/herniation/neurosurgical referral]
```
