---
tags: [medicine, neurology, history-taking, examination, fcps, mrcp]
chapter: Neurology
...
related: ['Neurological Examination', 'Cranial Nerve Examination', 'Motor System Examination', 'Sensory System Examination', 'Higher Cortical Function Assessment']
...
```mermaid
flowchart TD
    A[Symptom] --> B{Pathophysiology}
    B -->|V| C[Vascular: stroke/TIA/SAH/CVT]
    B -->|I| D[Infectious: meningitis/encephalitis/abscess]
    B -->|T| E[Traumatic/Toxic: TBI, drugs, alcohol]
    B -->|A| F[Autoimmune: MS, GBS, NMOSD, AE]
    B -->|M| G[Metabolic: Na, Ca, glucose, B12, thyroid]
    B -->|I| H[Inherited: HD, CMT, DMD, mitochondrial]
    B -->|N| I[Neoplastic: primary, mets, paraneoplastic]
    B -->|S| S[Structural: hydrocephalus, syrinx, disc]
    B -->|C| J[Convulsive/idiopathic: epilepsy, migraine, FND]
```
...
```mermaid
flowchart TD
    A[Symptom Reported] --> B{Onset / Tempo}
    B -->|Hyperacute| C[Stroke / TIA / SAH pathway]
    B -->|Acute| D[Infection / Inflammation / Trauma]
    B -->|Subacute| E[Neoplasm / Chronic infection / Autoimmune]
    B -->|Chronic-Progressive| F[Neurodegenerative / Inherited / Toxic]
    B -->|Relapsing| G[MS / Vasculitis / Metabolic]
    B -->|Paroxysmal| H[Epilepsy / Migraine / Movement / FND]
    C --> I[Localise: vascular territory?]
    D --> J[Localise: diffuse vs focal]
    E --> K[Localise: mass / cord / nerve]
    F --> L[Localise: system involvement]
    G --> M[Localise: CNS / PNS / systemic]
    H --> N[Localise: cortical / subcortical / brainstem / peripheral]
    I --> O[Differential + Investigations]
    J --> O
    K --> O
    L --> O
    M --> O
...
```mermaid
mindmap
  root((Neurological History Taking))
    Structure
      PC
      HPC_Tempo
      Systemic_Review
...
```mermaid
flowchart TD
    A[Presenting Symptom] --> B{Open Question: Tell me more}
    B --> C[Clarify Symptom Descriptor]
    C --> D{Tempo?}
    D -->|Hyperacute| E[Vascular/Seizure/Syncope]
    D -->|Acute| F[Infection/Inflammation/Trauma]
    D -->|Subacute| G[Neoplasm/Chronic infection/AE]
    D -->|Chronic-Progressive| H[Neurodegenerative/Hereditary]
    D -->|Relapsing| I[MS/Vasculitis]
    D -->|Paroxysmal| J[Epilepsy/Migraine/FND]
    E --> K[Localise: vascular territory/network]
    F --> K
    G --> K
    H --> K
    I --> K
    J --> K
    K --> L[Formulate Differential]
    L --> M[Targeted Investigations]
    M --> N[Diagnosis + Severity/Prognosis]
    N --> O[Management Plan + Communication]
```
