---
tags: [medicine, stroke-medicine, davidson, full-note, secondary-prevention, lipids, statins]
...
topic: Lipid lowering after stroke
...
```mermaid
flowchart TD
  A[Stroke/TIA survivor] --> B[Define likely mechanism and vascular risk]
  B --> C[Check lipid profile and prior statin use]
  C --> D[Start/continue high-intensity statin if appropriate]
  D --> E[Recheck lipids and tolerance]
  E --> F{LDL still above target/high-risk?}
  F -->|Yes| G[Improve adherence and consider add-on therapy]
  F -->|No| H[Continue long-term therapy + lifestyle measures]
```
...
```mermaid
flowchart LR
  A[Ischaemic stroke/TIA] --> B[Check vascular mechanism + LDL]
  B --> C[High-intensity statin]
  C --> D[Recheck LDL + tolerance]
  D --> E{Above target/high risk?}
  E -->|Yes| F[Optimize adherence + add ezetimibe if needed]
  E -->|No| G[Continue long-term therapy]
```
