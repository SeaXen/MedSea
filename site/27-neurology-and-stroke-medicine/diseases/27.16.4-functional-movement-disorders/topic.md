---
tags: [medicine, neurology, functional, movement, fcps, mrcp]
chapter: Neurology
...
references:
  clinical: ["Davidson 24th Ed Ch25", "Stone & Carson 2014 Mov Disord", "PasTest", "MRCP Notes", "neurosymptoms.org"]
related: ['Functional Weakness', 'Functional Visual Symptoms', 'PNES']
...
```mermaid
flowchart TD
    A[Trigger - Injury/Stress] --> B[Abnormal Motor Predictions]
    B --> C[Altered Sense of Agency]
    C --> D[Movement without conscious will]
    D --> E[Reinforcement - attention]
    E --> F[Functional Movement]
```
...
```mermaid
flowchart TD
    A[Movement disorder] --> B[History: FND clues]
    B --> C[Examination: positive signs]
    C --> D[Distractibility, entrainment present?]
    D --> E[Inconsistency/incongruence?]
    E --> F[Screening MRI, bloods]
    F --> G{DaTscan if needed}
    G --> H{Organic?}
    H -->|No| I[Diagnose FMD positively]
    H -->|Yes| J[Co-existing FMD possible]
    I --> K[Explanation + PT + CBT]
```
