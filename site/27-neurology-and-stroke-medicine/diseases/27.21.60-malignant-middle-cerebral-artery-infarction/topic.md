---
tags: [medicine, stroke-medicine, davidson, full-note, stroke-unit-care, malignant-mca, decompressive-surgery]
chapter: Stroke Medicine
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 29: Stroke medicine
heading: Stroke Unit Care and Complications
topic_group: Malignant stroke and deterioration
topic: Malignant middle cerebral artery infarction
...
```mermaid
flowchart TD
  A[Large MCA infarct identified] --> B[Admit to monitored stroke-unit / neurocritical setting]
  B --> C[Frequent neuro observations + repeat imaging vigilance]
  C --> D{Signs of malignant swelling / drowsiness?}
  D -->|No| E[Continue aggressive supportive care and edema surveillance]
  D -->|Yes| F[Urgent CT head + senior/neurosurgical escalation]
  F --> G[Head up, airway support, optimize physiology]
  G --> H[Assess for decompressive hemicraniectomy]
```
...
```mermaid
flowchart LR
  A[Large MCA infarct] --> B[Observe closely]
  B --> C{Drowsiness / CT swelling?}
  C -->|No| D[Continue stroke-unit monitoring]
  C -->|Yes| E[Urgent CT + neurosurgical review]
  E --> F[Head up, optimize physiology, airway support]
  F --> G[Consider decompressive hemicraniectomy]
```
