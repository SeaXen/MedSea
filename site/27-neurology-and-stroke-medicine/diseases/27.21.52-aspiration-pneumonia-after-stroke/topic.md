---
tags: [medicine, stroke-medicine, davidson, full-note, stroke-unit-care, aspiration-pneumonia, dysphagia]
...
topic: Aspiration pneumonia after stroke
...
```mermaid
flowchart TD
  A[Stroke patient] --> B[Swallow screen before oral intake]
  B --> C{Swallow safe?}
  C -->|Yes| D[Modified safe feeding + monitor]
  C -->|No| E[NBM + speech/swallow review + alternate hydration/nutrition]
  D --> F{Cough, fever, desaturation, wet voice?}
  E --> F
  F -->|Yes| G[Assess airway/oxygenation + examine chest]
  G --> H[CXR, blood tests, cultures if indicated]
  H --> I[Start aspiration pneumonia management + supportive care]
  F -->|No| J[Continue prevention and reassessment]
```
...
```mermaid
flowchart LR
  A[Stroke patient] --> B[Swallow screen]
  B --> C{Unsafe?}
  C -->|Yes| D[NBM + swallow review]
  C -->|No| E[Safe modified oral intake]
  D --> F[Monitor for cough, fever, desaturation]
  E --> F
  F -->|Aspiration suspected| G[CXR + oxygenation assessment + antibiotics if pneumonia]
```
