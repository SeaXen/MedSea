---
tags: [medicine, stroke-medicine, davidson, full-note, special-stroke-scenarios, young-stroke, cvst]
chapter: Stroke Medicine
davidson_part: Part 3: Clinical Medicine
davidson_chapter: Chapter 29: Stroke medicine
heading: Special Stroke Scenarios
topic_group: Young stroke and uncommon mechanisms
...
```mermaid
flowchart TD
  A[Headache +/- seizure +/- focal deficit] --> B[Assess ABCs, neuro status, glucose]
  B --> C[Do CT head]
  C --> D{CT normal or atypical hemorrhagic infarct?}
  D -->|Yes| E[Consider CVST risk factors and order CTV/MRV]
  D -->|No| F[Still consider CTV/MRV if suspicion persists]
  E --> G[Confirm venous sinus thrombosis]
  F --> G
  G --> H[Start anticoagulation if appropriate + treat raised ICP/seizures/cause]
```
...
```mermaid
flowchart LR
  A[Young patient with headache +/- seizure +/- focal deficit] --> B[CT head]
  B --> C[Suspicion persists / atypical hemorrhagic lesion]
  C --> D[CTV or MRV]
  D --> E[Confirm CVST]
  E --> F[Anticoagulate + manage ICP/seizures + treat cause]
```
