...
```mermaid
flowchart TD
A[Seizure-like event] --> B[ABC and capillary glucose]
B --> C{Unstable, prolonged, or recurrent?}
C -->|Yes| D[Treat as emergency / status / metabolic collapse]
C -->|No| E[Focused history + witness account + ECG + labs]
E --> F{Typical vasovagal/orthostatic features?}
F -->|Yes| G[Syncope likely]
F -->|No| H{Positive epileptic features?}
H -->|Yes| I[Seizure likely]
H -->|No| J{Positive functional features?}
J -->|Yes| K[FND / dissociative attack likely]
J -->|No| L{Metabolic / toxic clues?}
L -->|Yes| M[Provoked/metabolic event likely]
L -->|No| N[Keep broad differential and reassess]
```
